...
export const escrowRoutes = new Elysia({ prefix: "/escrow" })
  ...
  .post(
    ...
    async ({ body, headers }) => {
      ...
      const result = await escrowService.configurePropertyEscrowSplit(
        region,
        propertyId,
        agentId,
        agentPayoutRate,
  ...
  .post(
    ...
    async ({ body, headers }) => {
      ...
      const result = await escrowService.processRentPayment(region, paymentId);
      return result;
    },
    {
      body: t.Object({
  ...
  .post(
    ...
    async ({ headers }) => {
      const region = headers["x-region"] || headers["X-Region"] || "US";
      const result = await escrowService.releaseScheduledCommissions(region);
  ...
  .post(
    ...
    async ({ body, headers }) => {
      ...
      const result = await escrowService.withdrawCommissions(region, agentId, amount);
      return result;
    },
    {
      body: t.Object({
  ...
  .get(
    ...
    async ({ params, headers }) => {
      ...
      const wallet = await escrowService.getAgentWallet(region, agentId);
  ...
  .post(
    ...
    async ({ body, headers }) => {
      ...
      const result = await escrowService.createCrossBorderEscrow(
        buyerEmail,
        buyerRegion,
        sellerRegion,
        amount,
