...
export const paymentRoutes = new Elysia({ prefix: "/payments" })
  ...
  .patch("/:id", async ({ orgId, db, params, body, set }) => {
    try {
      ...
      // --- ML Feedback Loop: Payment Reliability ---
      if (oldData && data.status !== oldData.status && data.tenantId) {
        if (data.status === 'PAID') {
          MLBridgeService.sendFeedback("tenant-screening", "PAYMENT_ON_TIME", +2.0, { 
            userId: data.tenantId, 
            paymentId: data.id 
          }).catch(console.error);
        } else if (data.status === 'OVERDUE' || data.status === 'FAILED') {
          MLBridgeService.sendFeedback("tenant-screening", "PAYMENT_LATE_OR_FAILED", -5.0, { 
            userId: data.tenantId, 
            paymentId: data.id 
          }).catch(console.error);
        }
