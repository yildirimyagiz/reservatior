...
export const tax1099FormRoutes = new Elysia({ prefix: "/tax-1099-forms" })
  ...
  .get("/", async ({ query }) => {
    ...
    return tax1099FormService.getAll({
      where,
   ...
   */
  .post("/", async ({ body, set }) => {
    const data = await tax1099FormService.create(body);
    set.status = 201;
    return { data };
  }, {
  ...
  .get("/:id", async ({ params, set }) => {
    const data = await tax1099FormService.getById(params.id);
    if (!data) {
      set.status = 404;
  ...
  .patch("/:id", async ({ params, body, set }) => {
    try {
      const data = await tax1099FormService.update(params.id, body);
      return { data };
    } catch (e) {
      set.status = 404;
  ...
  .delete("/:id", async ({ params, set }) => {
    try {
      await tax1099FormService.delete(params.id);
