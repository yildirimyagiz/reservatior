import { Elysia, t } from "elysia";
import { authMiddleware } from "../middleware/auth";
import { aIChatMessageService } from "../services/aichatmessage";
import { 
  AIChatMessagePlainInputCreate, 
  AIChatMessagePlainInputUpdate 
} from "../../generated/prismabox/AIChatMessage";
import { regionMiddleware } from "../middleware/region";

export const aichatMessageRoutes = new Elysia({ prefix: "/ai-chat-messages" })
  .use(authMiddleware)
  .use(regionMiddleware)

  /**
   * GET /aichat-message
   * Retrieves all AIChatMessage with pagination and basic filtering.
   */
  .get("/", async ({ orgId, db, query }) => {
    const { page = "1", limit = "20", ...where } = query as any;
      if (orgId) where.orgId = orgId;
    return aIChatMessageService.withDB(db as any).getAll({
      where,
      skip: (parseInt(page) - 1) * parseInt(limit),
      take: parseInt(limit),
      orderBy: { createdAt: "desc" }
    });
  }, {
    query: t.Partial(t.Object({
      page: t.Optional(t.String()),
      limit: t.Optional(t.String()),
      orgId: t.Optional(t.String()),
    }))
  })

  /**
   * POST /aichat-message
   * Creates a new AIChatMessage.
   */
  .post("/", async ({ orgId, db, body, set }) => {
    const data = await aIChatMessageService.withDB(db as any).create(body);
    set.status = 201;
    return { data };
  }, {
    body: AIChatMessagePlainInputCreate
  })

  /**
   * GET /aichat-message/:id
   * Retrieves a single AIChatMessage by ID.
   */
  .get("/:id", async ({ orgId, db, params, set }) => {
    const data = await aIChatMessageService.withDB(db as any).getById(params.id);
    if (!data) {
      set.status = 404;
      return { error: "AIChatMessage not found" };
    }
    return { data };
  }, {
    params: t.Object({ id: t.String() })
  })

  /**
   * PATCH /aichat-message/:id
   * Updates an existing AIChatMessage.
   */
  .patch("/:id", async ({ orgId, db, params, body, set }) => {
    try {
      const data = await aIChatMessageService.withDB(db as any).update(params.id, body);
      return { data };
    } catch (e) {
      set.status = 404;
      return { error: "AIChatMessage not found or update failed" };
    }
  }, {
    params: t.Object({ id: t.String() }),
    body: AIChatMessagePlainInputUpdate
  })

  /**
   * DELETE /aichat-message/:id
   * Deletes a AIChatMessage.
   */
  .delete("/:id", async ({ orgId, db, params, set }) => {
    try {
      await aIChatMessageService.withDB(db as any).delete(params.id);
      return { success: true, message: "AIChatMessage deleted successfully" };
    } catch (e) {
      set.status = 404;
      return { error: "AIChatMessage not found or already deleted" };
    }
  }, {
    params: t.Object({ id: t.String() })
  });
