...
export const listingRoutes = new Elysia({ prefix: "/listing" })
  ...
  .get("/:id", async ({ orgId, db, params, set }) => {
    const data = await listingService.withDB(db as any).getById(params.id);
    if (!data) {
      set.status = 404;
  ...
  .patch("/:id", async ({ orgId, db, params, body, set }) => {
    try {
      const data = await listingService.withDB(db as any).update(params.id, body);
      return { data };
    } catch (e) {
      set.status = 404;
  ...
  .delete("/:id", async ({ orgId, db, params, set }) => {
    try {
      await listingService.withDB(db as any).delete(params.id);
