...
export const aiMigrationRoutes = new Elysia({ prefix: "/ai-migration" })
  ...
  .post("/analyze", async ({ orgId, db, body, set }) => {
    const { url, userId } = body;

    try {
      // First, import the property
      const importResult = await neuralImporterService.withDB(db as any).importFromUrl(url);
  ...
  .post("/execute", async ({ orgId, db, body, set }) => {
  ...
  .get("/status/:propertyId", async ({ orgId, db, params }) => {
   ...
   */
  .post("/drive-sync", async ({ orgId, db, body, set }) => {
    const { folderUrl, orgId, userId } = body;

    try {
      // Step 1: Scan Drive folder
      const scanResult = await neuralImporterService.withDB(db as any).importFromDriveFolder(folderUrl);
