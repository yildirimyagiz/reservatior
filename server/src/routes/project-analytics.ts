...
export const projectAnalyticsRoutes = new Elysia({ prefix: "/project-analyticses" })
  ...
  .get("/", async ({ orgId, db, query }) => {
    ...
    return projectAnalyticsService.withDB(db as any).getAll({
      where,
  ...
  .post("/", async ({ orgId, db, body, set }) => {
    const data = await projectAnalyticsService.withDB(db as any).create(body);
    set.status = 201;
    return { data };
  }, {
  ...
  .get("/:id", async ({ orgId, db, params, set }) => {
    const data = await projectAnalyticsService.withDB(db as any).getById(params.id);
    if (!data) {
      set.status = 404;
  ...
  .patch("/:id", async ({ orgId, db, params, body, set }) => {
    try {
      const data = await projectAnalyticsService.withDB(db as any).update(params.id, body);
      return { data };
    } catch (e) {
      set.status = 404;
  ...
  .delete("/:id", async ({ orgId, db, params, set }) => {
    try {
      await projectAnalyticsService.withDB(db as any).delete(params.id);
