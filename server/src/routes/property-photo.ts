...
export const propertyPhotoRoutes = new Elysia({ prefix: "/property-photo" })
   ...
   */
  .post("/", async ({ body, set }) => {
    const data = await propertyPhotoService.create(body);
    set.status = 201;
    return { data };
  }, {
  ...
  .get("/:id", async ({ params, set }) => {
    const data = await propertyPhotoService.getById(params.id);
    if (!data) {
      set.status = 404;
  ...
  .patch("/:id", async ({ params, body, set }) => {
    try {
      const data = await propertyPhotoService.update(params.id, body);
      return { data };
    } catch (e) {
      set.status = 404;
  ...
  .delete("/:id", async ({ params, set }) => {
    try {
      await propertyPhotoService.delete(params.id);
