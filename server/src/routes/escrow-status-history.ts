...
export const escrowStatusHistoryRoutes = new Elysia({ prefix: "/escrow-status-history" })
  ...
  .get("/", async ({ query }) => {
    ...
    return escrowStatusHistoryService.getAll({
      where,
   ...
   */
  .post("/", async ({ body, set }) => {
    const data = await escrowStatusHistoryService.create(body);
    set.status = 201;
    return { data };
  }, {
  ...
  .get("/:id", async ({ params, set }) => {
    const data = await escrowStatusHistoryService.getById(params.id);
    if (!data) {
      set.status = 404;
  ...
  .patch("/:id", async ({ params, body, set }) => {
    try {
      const data = await escrowStatusHistoryService.update(params.id, body);
      return { data };
    } catch (e) {
      set.status = 404;
  ...
  .delete("/:id", async ({ params, set }) => {
    try {
      await escrowStatusHistoryService.delete(params.id);
