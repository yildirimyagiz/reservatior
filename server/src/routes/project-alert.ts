...
export const projectAlertRoutes = new Elysia({ prefix: "/project-alerts" })
  ...
  .get("/", async ({ orgId, db, query }) => {
    ...
    return projectAlertService.withDB(db as any).getAll({
      where,
  ...
  .post("/", async ({ orgId, db, body, set }) => {
    const data = await projectAlertService.withDB(db as any).create(body);
    set.status = 201;
    return { data };
  }, {
  ...
  .get("/:id", async ({ orgId, db, params, set }) => {
    const data = await projectAlertService.withDB(db as any).getById(params.id);
    if (!data) {
      set.status = 404;
  ...
  .patch("/:id", async ({ orgId, db, params, body, set }) => {
    try {
      const data = await projectAlertService.withDB(db as any).update(params.id, body);
      return { data };
    } catch (e) {
      set.status = 404;
  ...
  .delete("/:id", async ({ orgId, db, params, set }) => {
    try {
      await projectAlertService.withDB(db as any).delete(params.id);
