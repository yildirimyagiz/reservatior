...
const DIRECTORY = "/Users/os2026/Documents/validebag_konaklari";

async function main() {
  console.log("Validebağ Konakları data ingestion started...");
  ...
  const org = await prisma.organization.upsert({
    where: { id: "org_validebag" },
    update: {},
    create: {
      id: "org_validebag",
      name: "Validebağ Konakları Yönetimi",
      type: "AGENCY",
      region: "TR",
      defaultCurrency: "TRY",
      defaultLocale: "tr-TR",
  ...
  const project = await prisma.project.create({
    data: {
      orgId: org.id,
      name: "Validebağ Konakları",
      description: "Üsküdar / İstanbul lüks konut projesi",
  ...
  for (const file of pdfFiles) {
    // Expected format: NAME SURNAME BLOCK-UNIT.pdf
    ...
    // Find the block-unit pattern, e.g., A1-13, B3-17, E-6, D2-53
    ...
    const contactId = `contact_${ownerName.replace(/[^A-Z0-9]/ig, "")}`;
    ...
    // Extract block and unit from blockUnit, e.g. "D1-38"
    let block = "";
    let unit = "";
    ...
    const propId = `validebag_${block}_${unit}`;
    ...
    const property = await prisma.property.upsert({
      ...
      create: {
        id: propId,
        orgId: org.id,
        name: `Validebağ Konakları Blok ${block} Daire ${unit}`,
        type: "APARTMENT",
        ...
        region: "TR",
        currency: "TRY",
        addressLine1: "Validebağ Konakları",
        city: "İstanbul",
        country: "TR",
        zip: "34662",
    ...
    await prisma.document.create({
      data: {
        ...
        title: `${ownerName} - ${blockUnit} Sözleşme`,
  ...
  console.log(`Successfully ingested ${propertiesAdded.size} properties from Validebağ Konakları.`);
}
