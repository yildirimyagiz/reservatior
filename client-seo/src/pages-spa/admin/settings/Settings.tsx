...
export default function Settings() {
  ...
  const saveSettings = async (category: string, settings: any) => {
    ...
    try {
      ...
      toast({ title: t("admin.settings.synchronization_complete"), description: `${category.charAt(0).toUpperCase() + category.slice(1)} configurations updated successfully` });
    } catch (error) {
      toast({ title: t("admin.settings.update_failed"), description: `Neural handshake failed for ${category} settings.`, variant: "destructive" });
    } finally { setLoading(false); }
  };
  ...
  return <div className="min-h-screen bg-background">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="bg-white/5 p-6 rounded-2xl border border-white/10">
          <h1 className="text-xl font-bold text-white">{t("admin.settings.neuroconfiguration_hub")}</h1>
          <p className="text-sm text-slate-400">{t("admin.settings.finetune_your_neural_handshake")}</p>
        </div>

        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-10 focus-visible:ring-0">
          <TabsList className="bg-white/5 border border-white/10 p-1.5 rounded-2xl h-18 w-full flex overflow-x-auto gap-1">
            <TabsTrigger value="profile" className="flex-1 min-w-[100px] rounded-xl font-bold text-[10px] data-[state=active]:bg-white/10 data-[state=active]:text-white text-slate-400 transition-all gap-2">
              <User className="w-4 h-4" />{t("admin.settings.profile")}</TabsTrigger>
            <TabsTrigger value="organization" className="flex-1 min-w-[100px] rounded-xl font-bold text-[10px] data-[state=active]:bg-white/10 data-[state=active]:text-white text-slate-400 transition-all gap-2">
              <Building className="w-4 h-4" />{t("admin.settings.org")}</TabsTrigger>
            <TabsTrigger value="notifications" className="flex-1 min-w-[100px] rounded-xl font-bold text-[10px] data-[state=active]:bg-white/10 data-[state=active]:text-white text-slate-400 transition-all gap-2">
              <Bell className="w-4 h-4" />{t("admin.settings.alerts")}</TabsTrigger>
            <TabsTrigger value="security" className="flex-1 min-w-[100px] rounded-xl font-bold text-[10px] data-[state=active]:bg-white/10 data-[state=active]:text-white text-slate-400 transition-all gap-2">
              <Shield className="w-4 h-4" />{t("admin.settings.security")}</TabsTrigger>
            <TabsTrigger value="appearance" className="flex-1 min-w-[100px] rounded-xl font-bold text-[10px] data-[state=active]:bg-white/10 data-[state=active]:text-white text-slate-400 transition-all gap-2">
              <Palette className="w-4 h-4" />{t("admin.settings.design")}</TabsTrigger>
            <TabsTrigger value="api" className="flex-1 min-w-[100px] rounded-xl font-bold text-[10px] data-[state=active]:bg-white/10 data-[state=active]:text-white text-slate-400 transition-all gap-2">
             ...
             <Card className="bg-white/5 border-white/10 rounded-4xl overflow-hidden shadow-2xl border-l border-t relative">
                <CardHeader className="pt-8 px-8">
                   <div className="space-y-1">
                      <CardTitle className="text-xs font-bold text-white">{t("admin.settings.entity_parameters")}</CardTitle>
                      <p className="text-[10px] font-bold text-slate-400">{t("admin.settings.personalized_neural_identification")}</p>
                   </div>
                </CardHeader>
                   ...
                   <form onSubmit={handleProfileSubmit} className="space-y-10">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                         ...
                         <div className="space-y-3">
                            ...
                            <Input type="email" value={profileSettings.email} readOnly className="bg-white/5 border-white/10 rounded-2xl h-14 text-slate-400 cursor-not-allowed" />
                         </div>
                         <div className="space-y-3">
                      ...
                      <div className="flex justify-end pt-4 border-t border-white/10">
                         <Button type="submit" disabled={loading} className="bg-white/5 hover:bg-white/10 text-white rounded-2xl h-16 px-10 font-bold text-[12px] transition-all border border-white/10">
             ...
             <Card className="bg-white/5 border-white/10 rounded-4xl overflow-hidden shadow-2xl border-l border-t relative">
                ...
                <CardContent className="p-8 space-y-6">
                   {[{ id: "emailNotifications", label: t("admin.settings.email_dispatch"), sub: "Mission logs via secondary node", icon: Globe },
                     { id: "pushNotifications", label: t("admin.settings.neural_interface_pushes"), sub: "Real-time tactical nodes", icon: Smartphone },
                     { id: "leadAlerts", label: t("admin.settings.request_velocity_alerts"), sub: "Immediate portfolio responses", icon: Bell }
                         ...
                         <div className="flex items-center gap-6">
                            ...
                            <div>
                               <h5 className="font-bold text-white text-xs leading-none">{pref.label}</h5>
             ...
             <Card className="bg-white/5 border-white/10 rounded-4xl overflow-hidden shadow-2xl border-l border-t relative">
                ...
                <CardContent className="p-8 space-y-10">
                   <div className="p-8 rounded-3xl bg-rose-500/5 border border-rose-500/10 flex items-start gap-6">
                      ...
                      <div>
                        <h4 className="text-sm font-bold text-white mb-1 leading-none">{t("admin.settings.security_level_standard_handshake")}</h4>
                        <p className="text-[10px] font-bold text-slate-400 leading-relaxed">{t("admin.settings.system_protected_by_rsa4096")}</p>
                      </div>
                   </div>
                   <div className="space-y-6">
                      <div className="flex items-center justify-between p-6 rounded-3xl bg-white/5 border border-white/10">
                         <div className="flex items-center gap-6">
                            ...
                            <div>
                               <h5 className="font-bold text-white text-xs leading-none">{t("admin.settings.multifactor_sync")}</h5>
                               <p className="text-[10px] font-bold text-slate-400 mt-1">{t("admin.settings.enhance_verification_with_biometric")}</p>
                            </div>
                         </div>
             ...
             <Card className="bg-white/5 border-white/10 rounded-4xl overflow-hidden shadow-2xl border-l border-t relative">
                <CardHeader className="pt-8 px-8">
                   <div className="space-y-1">
                      ...
                      <p className="text-[10px] font-bold text-slate-400">{t("admin.settings.finetune_your_visual_and")}</p>
                   </div>
                </CardHeader>
                <CardContent className="p-8 space-y-8">
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                      <div className="space-y-3">
                         <Label className="text-[10px] font-bold text-slate-400 ml-1">{t("admin.settings.visual_theme")}</Label>
                      ...
                      <div className="space-y-3">
                         <Label className="text-[10px] font-bold text-slate-400 ml-1">{t("admin.settings.language_node")}</Label>
                      ...
                      <div className="space-y-3">
                         <Label className="text-[10px] font-bold text-slate-400 ml-1">{t("admin.settings.currency_standard")}</Label>
