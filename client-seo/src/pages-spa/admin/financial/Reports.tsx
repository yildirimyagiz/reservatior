...
export default function FinancialReports() {
  ...
  return (
    <div className="space-y-6 min-h-screen">
      <div className="flex items-center justify-between bg-white/5 p-6 rounded-2xl border border-slate-200 dark:border-white/10">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">{t("admin_financial_financial_reports")}</h1>
          <p className="text-slate-500 dark:text-slate-400">{t("admin_financial_comprehensive_financial_analysis_and")}</p>
        </div>
        <Button className="bg-white/5 border-slate-200 dark:border-white/10 text-slate-900 dark:text-white hover:bg-white/10">
          <Download className="w-4 h-4 mr-2" />{t("admin_financial_export_reports")}
        </Button>
      </div>

      {dashLoading ? (
        <div className="flex justify-center py-16"><Loader2 className="w-8 h-8 animate-spin text-slate-500 dark:text-slate-400" /></div>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="bg-white/5 border-slate-200 dark:border-white/10">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-sm font-medium text-slate-500 dark:text-slate-400">{t("admin_financial_total_revenue")}</CardTitle>
                <TrendingUp className="h-4 w-4 text-emerald-400" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold text-slate-900 dark:text-white">${(d?.totalRevenue || 0).toLocaleString()}</div>
                <p className="text-xs text-slate-500">{t("admin_financial_152_from_last_quarter")}</p>
              </CardContent>
            </Card>
            <Card className="bg-white/5 border-slate-200 dark:border-white/10">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-sm font-medium text-slate-500 dark:text-slate-400">{t("admin_financial_net_profit")}</CardTitle>
                <DollarSign className="h-4 w-4 text-emerald-400" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold text-slate-900 dark:text-white">${(d?.totalProfit || 0).toLocaleString()}</div>
                <p className="text-xs text-slate-500">{t("admin_financial_87_from_last_quarter")}</p>
              </CardContent>
            </Card>
            <Card className="bg-white/5 border-slate-200 dark:border-white/10">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-sm font-medium text-slate-500 dark:text-slate-400">{t("admin_financial_operating_costs")}</CardTitle>
                <TrendingDown className="h-4 w-4 text-red-400" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold text-slate-900 dark:text-white">${(d?.totalExpenses || 0).toLocaleString()}</div>
                <p className="text-xs text-slate-500">{t("admin_financial_53_from_last_quarter")}</p>
              </CardContent>
            </Card>
            <Card className="bg-white/5 border-slate-200 dark:border-white/10">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-sm font-medium text-slate-500 dark:text-slate-400">{t("admin_financial_roi")}</CardTitle>
                <FileText className="h-4 w-4 text-slate-500 dark:text-slate-400" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold text-slate-900 dark:text-white">18.5%</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="bg-white/5 border-slate-200 dark:border-white/10">
              <CardHeader>
                <CardTitle className="text-slate-900 dark:text-white">{t("admin_financial_revenue_breakdown")}</CardTitle>
              </CardHeader>
              <CardContent>
                    ...
                    <div key={i} className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <p className="font-medium text-slate-900 dark:text-white">${Math.round(item.amount).toLocaleString()}</p>
            ...
            <Card className="bg-white/5 border-slate-200 dark:border-white/10">
              <CardHeader>
                <CardTitle className="text-slate-900 dark:text-white">{t("admin_financial_expense_analysis")}</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {expenses.map((expense, i) => (
                    <div key={i} className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <p className="font-medium text-slate-900 dark:text-white">${Math.round(expense.amount).toLocaleString()}</p>
          ...
          <Card className="bg-white/5 border-slate-200 dark:border-white/10">
            <CardHeader>
              <CardTitle className="text-slate-900 dark:text-white">{t("admin_financial_monthly_performance_trends")}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {(d?.revenueData || []).length === 0 ? (
                  ...
                  <div key={i} className="flex items-center justify-between border-b border-slate-200 dark:border-white/10 pb-4 last:border-0">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-medium text-slate-900 dark:text-white">{month.month}</p>
                    ...
                    <div className="grid grid-cols-3 gap-8 text-right">
                      ...
                      <div>
                        <p className="font-medium text-slate-900 dark:text-white">${Math.round(month.profit).toLocaleString()}</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="bg-white/5 border-slate-200 dark:border-white/10">
              <CardHeader>
                <CardTitle className="text-slate-900 dark:text-white">{t("admin_financial_top_performing_properties")}</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
            ...
            <Card className="bg-white/5 border-slate-200 dark:border-white/10">
              <CardHeader>
                <CardTitle className="text-slate-900 dark:text-white">{t("admin_financial_payment_methods")}</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {[{ method: "Bank Transfer", count: 234, amount: d ? d.totalRevenue * 0.514 : 234567, percentage: 51.4 },
                    ...
                    <div key={i} className="flex items-center justify-between">
                      <div>
                        <p className="font-medium text-slate-900 dark:text-white">{method.method}</p>
                        <p className="text-sm text-slate-500">{method.count} {t("admin_financial_transactions")}</p>
                      </div>
                      <div className="text-right">
                        <p className="font-medium text-slate-900 dark:text-white">${Math.round(method.amount).toLocaleString()}</p>
            ...
            <Card className="bg-white/5 border-slate-200 dark:border-white/10">
              <CardHeader>
                <CardTitle className="text-slate-900 dark:text-white">{t("admin_financial_upcoming_reports")}</CardTitle>
              </CardHeader>
              <CardContent>
                {reportsLoading ? (
                  ...
                  <div className="space-y-4">
                    {reports.map((report) => (
                      <div key={report.id} className="flex items-center justify-between">
                        ...
                        <Badge className={report.status === "in-progress" ? "bg-amber-500/20 text-amber-400 border-0" : "bg-slate-500/20 text-slate-500 dark:text-slate-400 border-0"}>
                          {report.status}
                        </Badge>
                      </div>
                    ))}
