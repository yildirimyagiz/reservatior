...
const statusConfig: Record<string, { label: string; class: string }> = {
  ...
  APPROVED: { label: "Approved", class: "bg-slate-500/20 text-slate-500 dark:text-slate-400" },
  PAID: { label: "Paid", class: "bg-emerald-500/20 text-emerald-400" },
  CANCELLED: { label: "Cancelled", class: "bg-slate-500/20 text-slate-500 dark:text-slate-400" },
  OVERDUE: { label: "Overdue", class: "bg-red-500/20 text-red-400" },
  DISPUTED: { label: "Disputed", class: "bg-slate-500/20 text-slate-500 dark:text-slate-400" },
};
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                return (
                  <TableRow key={c.id} className="border-b border-slate-200 dark:border-white/10 hover:bg-white/5 transition-colors">
                    <TableCell className="py-4 px-6 font-mono text-xs text-slate-500 dark:text-slate-400">{c.id.slice(0, 8)}...</TableCell>
                    <TableCell className="px-6 font-bold text-slate-900 dark:text-white">${c.commissionAmount.toLocaleString()}</TableCell>
                    <TableCell className="px-6 text-sm text-slate-300">{c.commissionRate}%</TableCell>
                    <TableCell className="px-6 text-sm text-slate-500 dark:text-slate-400">${c.amountBase.toLocaleString()}</TableCell>
                    <TableCell className="px-6">
                      <Badge className={cn("border-0", cfg.class)}>{cfg.label}</Badge>
                    </TableCell>
                    <TableCell className="px-6 text-xs text-slate-500 dark:text-slate-400 font-mono">{new Date(c.createdAt).toLocaleDateString()}</TableCell>
                    <TableCell className="px-6">
                      <div className="flex gap-2">
                        <Button size="sm" variant="ghost" className="text-slate-500 dark:text-slate-400 hover:text-white" onClick={() => openEdit(c)}>
