...
export function Expenses() {
 ...
 label: t("admin_financial_total_amount"),
 ...
 <Table>
 <TableHeader><TableRow><TableHead>{t("admin_financial_category")}</TableHead><TableHead>{t("admin_financial_amount")}</TableHead><TableHead>{t("admin_financial_date")}</TableHead><TableHead>{t("admin_financial_description")}</TableHead><TableHead className="w-10" /></TableRow></TableHeader>
 <TableBody>
 ...
 <div className="space-y-1.5"><Label>{t("admin_financial_currency")}</Label>
 ...
 <div className="space-y-1.5"><Label>{t("admin_financial_category")}</Label>
 ...
 <DialogFooter><Button type="submit" disabled={createMutation.isPending}>{createMutation.isPending ? t("commonSaving") : t("admin_financial_record_expense")}</Button></DialogFooter>
 </form>
 </DialogContent>
 </Dialog>
 </>;
...
export function FinancialRecords() {
 ...
 return <PageShell title={t("admin_financial_financial_records")} description={t("admin_financial_income_expenses_and_profit")} stats={[{
 label: t("admin_financial_total_income"),
 value: `$${totalIncome.toLocaleString()}`
 }, {
 label: t("admin_financial_total_expenses"),
 value: `$${totalExpenses.toLocaleString()}`
 }, {
 label: t("admin_financial_net_profit"),
 value: `$${netProfit.toLocaleString()}`
 }, {
 label: t("admin_financial_transactions"),
 value: records.length
 }]} actions={<Button variant="outline" size="sm" onClick={() => refetch()} disabled={isLoading}>
 <RefreshCw className={`w-4 h-4 mr-2 ${isLoading ? 'animate-spin' : ''}`} />{t("admin_financial_refresh")}</Button>}>
 <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
 <Table>
 <TableHeader><TableRow><TableHead>{t("admin_financial_date")}</TableHead><TableHead>{t("admin_financial_description")}</TableHead><TableHead>{t("admin_financial_category")}</TableHead><TableHead>{t("admin_financial_type")}</TableHead><TableHead className="text-right">{t("admin_financial_amount")}</TableHead></TableRow></TableHeader>
 <TableBody>
 {isLoading ? <TableRow><TableCell colSpan={5} className="text-center py-12"><Loader2 className="w-6 h-6 animate-spin mx-auto" /></TableCell></TableRow> : records.length === 0 ? <TableRow><TableCell colSpan={5} className="text-center py-12 text-muted-foreground">{t("admin_financial_no_records_found")}</TableCell></TableRow> : records.map((r: FinancialRecord) => <TableRow key={r.id} className="hover:bg-muted/40 transition-colors">
...
export function TaxRecords() {
 const { t } = useTranslation();
 return <PageShell title={t("admin_financial_tax_records")} description={t("admin_financial_property_and_income_tax")} stats={[{
 label: t("admin_financial_total_tax"),
 value:"$8,400"
 }]}>
 <div className="bg-card border border-border rounded-xl p-8 text-center text-muted-foreground">{t("admin_financial_tax_records_are_being")}</div>
 </PageShell>;
}
...
export function Budgets() {
 ...
 return <PageShell title={t("admin_financial_budgets")} description={t("admin_financial_manage_and_track_budget")} stats={[{
 label: t("admin_financial_total_budgets"),
 ...
 <RefreshCw className={`w-4 h-4 mr-2 ${isLoading ? 'animate-spin' : ''}`} />{t("admin_financial_refresh")}</Button>}>
 {isLoading ? <div className="flex justify-center py-24"><Loader2 className="w-8 h-8 animate-spin" /></div> : budgets.length === 0 ? <div className="bg-card border border-border rounded-xl p-12 text-center text-muted-foreground">{t("admin_financial_no_budgets_found")}</div> : <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 ...
 {over && <Badge className="bg-red-100 text-red-700 border-0 text-xs">{t("admin_financial_over_budget")}</Badge>}
 </div>
 <div>
 ...
 <p className="text-[10px] text-muted-foreground mt-1">{pct.toFixed(0)}{t("admin_financial_utilized")}</p>
 </div>
 </div>;
 })}
 </div>}
...
export function Payouts() {
 ...
 return <PageShell title={t("admin_financial_payouts")} description={t("admin_financial_commission_payments_vendor_payouts")} stats={[{
 label: t("admin_financial_total_payouts"),
