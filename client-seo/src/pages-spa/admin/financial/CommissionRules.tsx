          ...
          <DialogContent className="sm:max-w-[425px] bg-slate-900 border-white/10 text-white">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 pt-4">
              <div className="space-y-2">
                ...
                <Input id="providerId" className="bg-white/5 border-white/10 text-white" value={formData.providerId} onChange={e => setFormData({ ...formData, providerId: e.target.value })} required />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input id="commission" type="number" className="bg-white/5 border-white/10 text-white" value={formData.commission} onChange={e => setFormData({ ...formData, commission: Number(e.target.value) })} required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="minVolume">{t("admin.financial.min_volume", "Min Volume")}</Label>
                <Input id="minVolume" type="number" className="bg-white/5 border-white/10 text-white" value={formData.minVolume} onChange={e => setFormData({ ...formData, minVolume: Number(e.target.value) })} />
              </div>
              <DialogFooter>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="bg-white/5 border-white/10">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-medium text-slate-400">{t("admin.financial.total_payouts", "Total Payouts")}</p>
                <h3 className="text-2xl font-bold text-white mt-1">${(rules.reduce((s, r) => s + r.commission, 0)).toLocaleString()}</h3>
              </div>
          ...
          <Table>
            <TableHeader className="bg-white/5 border-b border-white/10">
              <TableRow className="hover:bg-transparent border-none">
                <TableHead className="text-xs font-medium text-slate-400 py-4 px-6">{t("admin.financial.provider_source", "Provider")}</TableHead>
                <TableHead className="text-xs font-medium text-slate-400 px-6">{t("admin.financial.rule_type", "Type")}</TableHead>
                <TableHead className="text-xs font-medium text-slate-400 px-6">{t("admin.financial.commission", "Commission")}</TableHead>
                <TableHead className="text-xs font-medium text-slate-400 px-6">{t("admin.financial.min_volume", "Min Volume")}</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow key={rule.id} className="border-b border-white/10 hover:bg-white/5 transition-colors">
                  <TableCell className="py-4 px-6 font-medium text-white">{rule.providerId}</TableCell>
                  ...
                  <TableCell className="px-6 font-bold text-white">
                    {rule.ruleType === "PERCENTAGE" ? `${rule.commission}%` : `$${rule.commission.toLocaleString()}`}
                  </TableCell>
                  <TableCell className="px-6 text-sm text-slate-400">
                    {rule.minVolume ? `$${rule.minVolume.toLocaleString()}+` : "—"}
                  </TableCell>
                  <TableCell className="px-6 text-xs text-slate-400 font-mono">
                    {new Date(rule.createdAt).toLocaleDateString()}
                  </TableCell>
                  <TableCell className="px-6">
                    <div className="flex gap-2">
                      <Button size="sm" variant="ghost" className="text-slate-400 hover:text-white" onClick={() => openEdit(rule)}>
