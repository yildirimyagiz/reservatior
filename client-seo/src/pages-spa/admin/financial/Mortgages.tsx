...
export default function Mortgages() {
 ...
 <DialogHeader>
 <DialogTitle>{t("admin_auto_add_new_mortgage", "Yeni İpotek Ekle")}</DialogTitle>
 <DialogDescription>{t("admin_auto_register_a_new_mortgage_for_a_property_m", "Arka uçta bir mülk eşlemesi için yeni bir ipotek kaydedin.")}</DialogDescription>
 </DialogHeader>
 <div className="grid gap-4 py-4">
 <div className="grid grid-cols-4 items-center gap-4">
 <Label htmlFor="propertyId" className="text-right text-xs">{t("admin_auto_property", "Mülk")}</Label>
 ...
 <div className="grid grid-cols-4 items-center gap-4">
 <Label htmlFor="lender" className="text-right text-xs">{t("admin_auto_lender_bank", "Borç Veren Banka")}</Label>
 <Input id="lender" className="col-span-3 h-10" value={newMortgage.lender} onChange={e => setNewMortgage({...newMortgage, lender: e.target.value})} placeholder={t("admin_auto_e_g_chase_bank", "örneğin Takip Bankası")} />
 </div>
 <div className="grid grid-cols-4 items-center gap-4">
 <Label htmlFor="principal" className="text-right text-xs">{t("admin_auto_principal", "Anapara ($)")}</Label>
 ...
 <div className="grid grid-cols-4 items-center gap-4">
 <Label htmlFor="interestRate" className="text-right text-xs">{t("admin_auto_interest_rate", "Faiz Oranı (%)")}</Label>
 ...
 <div className="grid grid-cols-4 items-center gap-4">
 <Label htmlFor="startDate" className="text-right text-xs">{t("admin_auto_start_date", "Başlangıç ​​Tarihi")}</Label>
 ...
 <SelectItem value="PAID_OFF">{t("admin_auto_paid_off", "Ödendi")}</SelectItem>
 <SelectItem value="REFINANCED">{t("admin_auto_refinanced", "Yeniden finanse edildi")}</SelectItem>
 </SelectContent>
 </Select>
 </div>
 </div>
 ...
 </TableRow> : mortgages.length === 0 ? <TableRow><TableCell colSpan={7} className="text-center py-12 text-muted-foreground">{t("admin_financial_no_mortgages_found", "İpotek bulunamadı")}</TableCell></TableRow> : mortgages.map(mort => <TableRow key={mort.id} className="hover:bg-muted/40 transition-colors">
 <TableCell>
