  ...
  return <div className="min-h-screen bg-background">
      <div className="p-6 space-y-6">
        <div className="bg-white/5 p-6 rounded-2xl border border-slate-200 dark:border-white/10">
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">{t("admin_security_fraud_assessment_hub")}</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">{t("admin_security_realtime_behavioral_analysis_and")}</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
           {[{ label: t("admin_security_total_screened"), val: screenings.length, icon: Shield, color: "text-slate-500 dark:text-slate-400" },
        ...
        <Card className="bg-white/5 border-slate-200 dark:border-white/10 rounded-4xl overflow-hidden shadow-2xl border-l border-t relative">
           <CardHeader className="p-8 border-b border-slate-200 dark:border-white/10 flex flex-row items-center justify-between">
              ...
              <div className="flex gap-2">
                 <Button variant="outline" className="h-10 rounded-xl border-slate-200 dark:border-white/10 bg-white/5 hover:bg-white/10 text-slate-500 dark:text-slate-400 hover:text-white font-bold text-[9px] gap-2">
           ...
           <CardContent className="p-0">
             {isLoading ? <div className="py-24 text-center">
                   ...
                   <p className="text-xs font-bold text-slate-500 dark:text-slate-400 animate-pulse">{t("admin_security_syncing_trust_matrix")}</p>
                </div> : <Table>
                 <TableHeader className="bg-white/5 border-b border-slate-200 dark:border-white/10">
                   <TableRow className="hover:bg-transparent border-none">
                     ...
                     <TableHead className="text-[10px] font-bold text-slate-500 dark:text-slate-400 px-8">{t("admin_security_asset_context")}</TableHead>
                     <TableHead className="text-[10px] font-bold text-slate-500 dark:text-slate-400 px-8">{t("admin_security_risk_profile")}</TableHead>
                     <TableHead className="text-[10px] font-bold text-slate-500 dark:text-slate-400 px-8">{t("admin_security_neural_score")}</TableHead>
                 ...
                 <TableBody>
                   {screenings.map((s: any) => <TableRow key={s.id} className="border-b border-slate-200 dark:border-white/10 hover:bg-white/5 transition-all group">
                       <TableCell className="py-8 px-8">
                         <div className="font-bold text-slate-500 dark:text-slate-400 text-[9px] mb-2">{s.booking?.id?.slice(0, 8) || 'PRE-BOOKING'}</div>
                         <div className="text-sm font-bold text-slate-900 dark:text-white leading-none">{s.contact?.fullName}</div>
                         <div className="text-[10px] font-bold text-slate-500 dark:text-slate-400 mt-1">{s.contact?.email}</div>
                       </TableCell>
                       <TableCell className="px-8">
                         <div className="text-xs font-bold text-slate-500 dark:text-slate-400">{s.property?.name}</div>
                       ...
                       <TableCell className="px-8">
                          <div className="flex items-center gap-4">
                             ...
                             <span className="text-xs font-bold text-slate-900 dark:text-white">{(s.riskScore * 100).toFixed(0)}</span>
                          </div>
                       </TableCell>
                       <TableCell className="px-8 text-right">
                         <Button variant="ghost" className="h-12 w-12 rounded-2xl hover:bg-white/5 text-slate-500 dark:text-slate-400 hover:text-white">
