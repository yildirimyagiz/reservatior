...
export default function PropertyInventory() {
  ...
  if (isLoading) {
    return <div className="min-h-screen bg-background p-6">
        <div className="bg-white/5 p-6 rounded-2xl border border-slate-200 dark:border-white/10">
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">{t("admin_inventory_title")}</h1>
        </div>
  ...
  return <div className="min-h-screen bg-background">
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card className="bg-white/5 border-slate-200 dark:border-white/10">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-slate-500 dark:text-slate-400">{t("available")}</CardTitle>
              <Home className="h-4 w-4 text-slate-500 dark:text-slate-400" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-slate-900 dark:text-white">{availableProperties}</div>
              <p className="text-xs text-slate-500 dark:text-slate-400">{t("admin_property_ready_for_showing")}</p>
            </CardContent>
          </Card>

          <Card className="bg-white/5 border-slate-200 dark:border-white/10">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-slate-500 dark:text-slate-400">{t("admin_inventory_portfolioValue")}</CardTitle>
              <DollarSign className="h-4 w-4 text-slate-500 dark:text-slate-400" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-slate-900 dark:text-white">${totalValue.toLocaleString()}</div>
          ...
          <Card className="bg-white/5 border-slate-200 dark:border-white/10">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-slate-500 dark:text-slate-400">{t("admin_inventory_avgPrice")}</CardTitle>
              <TrendingUp className="h-4 w-4 text-slate-500 dark:text-slate-400" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-slate-900 dark:text-white">${avgPrice.toLocaleString()}</div>
          ...
          <Card className="bg-white/5 border-slate-200 dark:border-white/10">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-slate-500 dark:text-slate-400">{t("admin_inventory_avgDOM")}</CardTitle>
              <Calendar className="h-4 w-4 text-slate-500 dark:text-slate-400" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-slate-900 dark:text-white">{Math.round(avgDaysOnMarket)}</div>
            ...
            <div className="flex gap-4 items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                <Select value={typeFilter} onValueChange={setTypeFilter}>
                  ...
                  <SelectContent className="max-h-[400px] bg-[#14151a] border-slate-200 dark:border-white/10 text-slate-900 dark:text-white">
                    <SelectItem value="all">{t("admin_inventory_types_all")}</SelectItem>
                    <SelectGroup>
                      <SelectLabel className="text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest text-[9px] px-2 py-1">{t("client.property.portfolio.filters.type.residential")} - HOUSES</SelectLabel>
                    ...
                    <SelectGroup>
                      <SelectLabel className="text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest text-[9px] px-2 py-1 mt-2">{t("client.property.portfolio.filters.type.residential")} - APARTMENTS</SelectLabel>
                    ...
                    <SelectGroup>
                      <SelectLabel className="text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest text-[9px] px-2 py-1 mt-2">{t("client.property.portfolio.filters.type.commercial")}</SelectLabel>
        ...
        <Card className="bg-white/5 border-slate-200 dark:border-white/10">
          <CardHeader>
            <CardTitle className="text-slate-900 dark:text-white">{t("admin_property_property_inventory")}</CardTitle>
            <p className="text-sm text-slate-500 dark:text-slate-400">{t("admin_property_manage_your_property_listings")}</p>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow className="border-slate-200 dark:border-white/10">
                  ...
                  <TableHead className="text-slate-500 dark:text-slate-400">{t("admin_property_price")}</TableHead>
                  <TableHead className="text-slate-500 dark:text-slate-400">{t("admin_property_size")}</TableHead>
                  <TableHead className="text-slate-500 dark:text-slate-400">{t("admin_property_agent")}</TableHead>
                  <TableHead className="text-slate-500 dark:text-slate-400">{t("admin_property_days_on_market")}</TableHead>
                  <TableHead className="text-slate-500 dark:text-slate-400">{t("admin_property_views")}</TableHead>
                  <TableHead className="text-slate-500 dark:text-slate-400">{t("admin_property_offers")}</TableHead>
                  <TableHead className="text-right text-slate-500 dark:text-slate-400">{t("admin_property_actions")}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {properties.map(property => <TableRow key={property.id} className="border-slate-200 dark:border-white/10">
                    <TableCell className="font-medium text-slate-900 dark:text-white">
                      <div className="flex items-center gap-2">
                        <Building className="h-4 w-4 text-slate-500 dark:text-slate-400" />
                        <div>
                          <div className="text-slate-900 dark:text-white">{property.address}</div>
                    ...
                    <TableCell>
                      <Badge variant="outline" className="text-slate-500 dark:text-slate-400 border-slate-200 dark:border-white/10">{getLocalizedType(property.propertyType)}</Badge>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2 text-slate-900 dark:text-white">
                        <div className={`w-2 h-2 rounded-full ${property.status === 'AVAILABLE' ? 'bg-green-500' : property.status === 'PENDING' ? 'bg-yellow-500' : property.status === 'UNDER_CONTRACT' ? 'bg-slate-500' : property.status === 'SOLD' ? 'bg-slate-500' : property.status === 'WITHDRAWN' ? 'bg-red-500' : 'bg-white/10'}`} />
                        <span className="capitalize">{getStatusLabel(property.status)}</span>
                      </div>
                    </TableCell>
                    <TableCell className="font-medium text-slate-900 dark:text-white">${property.price.toLocaleString()}</TableCell>
                    <TableCell className="text-slate-500 dark:text-slate-400">
                      <div className="text-sm">
                        <div className="text-slate-900 dark:text-white">{property.bedrooms}{t("admin_property_bed")}{property.bathrooms}{t("admin_property_bath")}</div>
                        <div className="text-slate-500 dark:text-slate-400">{property.squareFeet?.toLocaleString()}{t("admin_property_sq_ft")}</div>
                      </div>
                    </TableCell>
                    <TableCell className="text-slate-500 dark:text-slate-400">
                    ...
                    <TableCell>
                      <div className="text-center">
                        <div className="font-medium text-slate-900 dark:text-white">{property.daysOnMarket}</div>
                        <div className="text-xs text-slate-500 dark:text-slate-400">{t("admin_property_days")}</div>
                      </div>
                    </TableCell>
                    <TableCell>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="bg-[#14151a] border-slate-200 dark:border-white/10 text-slate-900 dark:text-white">
                          ...
                          {property.status === 'AVAILABLE' && <>
                              <DropdownMenuItem className="hover:bg-white/5" onClick={() => updateStatusMutation.mutate({ propertyId: property.id, status: 'PENDING' })}>{t("admin_property_mark_as_pending")}</DropdownMenuItem>
                              <DropdownMenuItem className="hover:bg-white/5" onClick={() => updateStatusMutation.mutate({ propertyId: property.id, status: 'OFF_MARKET' })}>{t("admin_property_take_off_market")}</DropdownMenuItem>
                            </>}
                          <DropdownMenuSeparator className="border-slate-200 dark:border-white/10" />
                          <DropdownMenuItem className="text-red-600 hover:bg-white/5">{t("admin_property_withdraw_listing")}</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
                  </TableRow>)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="bg-white/5 border-slate-200 dark:border-white/10">
            <CardHeader>
              <CardTitle className="text-slate-900 dark:text-white">{t("admin_property_status_distribution")}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-medium text-slate-900 dark:text-white">{properties.filter(p => p.status === 'AVAILABLE').length}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-900 dark:text-white flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-yellow-500" />{t("admin_property_pending")}</span>
                  <span className="font-medium text-slate-900 dark:text-white">{properties.filter(p => p.status === 'PENDING').length}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-900 dark:text-white flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-slate-500" />{t("admin_property_under_contract")}</span>
                  <span className="font-medium text-slate-900 dark:text-white">{properties.filter(p => p.status === 'UNDER_CONTRACT').length}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-900 dark:text-white flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-slate-500" />{t("admin_property_sold")}</span>
                  <span className="font-medium text-slate-900 dark:text-white">{properties.filter(p => p.status === 'SOLD').length}</span>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/5 border-slate-200 dark:border-white/10">
            <CardHeader>
              <CardTitle className="text-slate-900 dark:text-white">{t("admin_property_property_types")}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-900 dark:text-white">{t("admin_property_houses")}</span>
                  <span className="font-medium text-slate-900 dark:text-white">{properties.filter(p => p.propertyType === 'HOUSE').length}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-900 dark:text-white">{t("admin_property_condos")}</span>
                  <span className="font-medium text-slate-900 dark:text-white">{properties.filter(p => p.propertyType === 'CONDO').length}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-900 dark:text-white">{t("admin_property_townhouses")}</span>
                  <span className="font-medium text-slate-900 dark:text-white">{properties.filter(p => p.propertyType === 'TOWNHOUSE').length}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-900 dark:text-white">{t("admin_property_apartments")}</span>
                  <span className="font-medium text-slate-900 dark:text-white">{properties.filter(p => p.propertyType === 'APARTMENT').length}</span>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/5 border-slate-200 dark:border-white/10">
            <CardHeader>
              <CardTitle className="text-slate-900 dark:text-white">{t("admin_property_performance_metrics")}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-slate-500 dark:text-slate-400">{Math.round(avgDaysOnMarket)}</div>
                  <p className="text-sm text-slate-500 dark:text-slate-400">{t("admin_property_avg_days_on_market")}</p>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-400">{totalViews}</div>
                  <p className="text-sm text-slate-500 dark:text-slate-400">{t("admin_property_total_views")}</p>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-slate-500 dark:text-slate-400">{properties.reduce((acc, p) => acc + p.offers, 0)}</div>
                  <p className="text-sm text-slate-500 dark:text-slate-400">{t("admin_property_total_offers")}</p>
                </div>
              </div>
            </CardContent>
          </Card>
