...
export default function AdminProperties() {
  ...
  const getStatusColor = (status: string) => {
    switch (status.toUpperCase()) {
      ...
      case 'RENTED': return 'bg-slate-500/10 text-slate-400 border-slate-500/20';
      ...
      <div className="p-6 space-y-10 pb-20">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
          <div className="xl:col-span-3 space-y-6">
            ...
            <Card className="bg-gradient-to-br from-slate-600/20 to-transparent border-white/10 rounded-4xl p-8 relative overflow-hidden shadow-2xl">
              <div className="relative z-10 space-y-4">
                ...
                <Button className="w-full bg-slate-600 hover:bg-slate-500 text-white font-bold h-12 rounded-2xl text-[10px] shadow-xl shadow-slate-600/20">{t('viewAnalytics')}</Button>
              </div>
              <Activity className="absolute -bottom-10 -right-10 w-40 h-40 text-slate-400/5 rotate-12" />
            </Card>
          </div>
                             ...
                             <DropdownMenu>
                               ...
                               <DropdownMenuContent align="end" className="bg-[#14151a] border-white/10 rounded-2xl shadow-2xl p-2 min-w-[180px]">
                                 ...
                                 <DropdownMenuItem onClick={() => navigate('/admin/leads')} className="rounded-xl px-4 py-3 text-[10px] font-bold text-slate-400 hover:text-white transition-all cursor-pointer">
                                    <Users className="w-4 h-4 mr-3 text-slate-400" /> {t('manageRequests')}
                                 </DropdownMenuItem>
                                 ...
                                 <DropdownMenuItem onClick={() => navigate('/admin/maintenance')} className="rounded-xl px-4 py-3 text-[10px] font-bold text-slate-400 hover:text-white transition-all cursor-pointer">
                                    <Calendar className="w-4 h-4 mr-3 text-slate-400" /> {t('scheduleMaintenance')}
                                 </DropdownMenuItem>
