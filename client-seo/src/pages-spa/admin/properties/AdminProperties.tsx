...
export default function AdminProperties() {
  ...
  const getStatusColor = (status: string) => {
    switch (status.toUpperCase()) {
      ...
      case 'SOLD': return 'bg-slate-500/10 text-slate-500 dark:text-slate-400 border-slate-500/20';
      case 'PENDING': return 'bg-orange-500/10 text-orange-400 border-orange-500/20';
      case 'RENTED': return 'bg-slate-500/10 text-slate-500 dark:text-slate-400 border-slate-500/20';
  ...
  const getComplianceBadge = (status: string) => {
    switch (status.toUpperCase()) {
      ...
      default: return <Badge className="bg-slate-500/20 text-slate-500 dark:text-slate-400 border-none font-bold leading-none">{status}</Badge>;
    }
  };
      ...
      <div className="p-6 space-y-10 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <Card className="bg-white/5 border-slate-200 dark:border-white/10 rounded-3xl overflow-hidden shadow-2xl relative group">
            <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-20 transition-all">
              <Building className="w-12 h-12 text-slate-900 dark:text-white" />
            </div>
            <CardContent className="p-8">
              <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 mb-1">{t('globalAssets')}</p>
              <h3 className="text-xl font-bold text-slate-900 dark:text-white leading-none">{stats.totalProperties}</h3>
          ...
          <Card className="bg-white/5 border-slate-200 dark:border-white/10 rounded-3xl overflow-hidden shadow-2xl relative group">
            ...
            <CardContent className="p-8">
              <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 mb-1">{t('projectedYield')}</p>
              <h3 className="text-xl font-bold text-slate-900 dark:text-white leading-none">${stats.totalRevenue.toLocaleString()}</h3>
          ...
          <Card className="bg-white/5 border-slate-200 dark:border-white/10 rounded-3xl overflow-hidden shadow-2xl relative group">
            ...
            <CardContent className="p-8">
              <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 mb-1">{t('propertyAvgoccupancy')}</p>
              <h3 className="text-xl font-bold text-slate-900 dark:text-white leading-none">{stats.avgOccupancy}%</h3>
          ...
          <Card className="bg-white/5 border-slate-200 dark:border-white/10 rounded-3xl overflow-hidden shadow-2xl relative group">
            ...
            <CardContent className="p-8">
              <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 mb-1">{t('pendingRequests')}</p>
              <h3 className="text-xl font-bold text-slate-900 dark:text-white leading-none">24</h3>
              <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 mt-4 flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
          <div className="xl:col-span-3 space-y-6">
            <Card className="bg-white/5 backdrop-blur-xl border-slate-200 dark:border-white/10 rounded-4xl overflow-hidden shadow-2xl">
              <CardHeader className="p-6 border-b border-slate-200 dark:border-white/10">
              ...
              <div className="p-4 space-y-2">
                {[{ label: "Escrow Havuzu", icon: ShieldCheck, path: "/admin/escrow" },
                 ...
                 { label: t('leadsProspects'), icon: Users, path: "/admin/leads" }].map((item, i) => <Button key={i} variant="ghost" onClick={() => window.location.href = item.path} className="w-full justify-between h-14 rounded-2xl px-4 hover:bg-white/5 text-slate-500 dark:text-slate-400 hover:text-white transition-all group">
            ...
            <Card className="bg-gradient-to-br from-slate-600/20 to-transparent border-slate-200 dark:border-white/10 rounded-4xl p-8 relative overflow-hidden shadow-2xl">
              ...
              <Activity className="absolute -bottom-10 -right-10 w-40 h-40 text-slate-500 dark:text-slate-400/5 rotate-12" />
            </Card>
          </div>

          <div className="xl:col-span-9 space-y-6">
            <Tabs defaultValue="ALL" value={listingFilter} onValueChange={setListingFilter} className="w-full">
              <TabsList className="bg-white/5 border border-slate-200 dark:border-white/10 p-1 rounded-2xl h-14 mb-4">
                <TabsTrigger value="ALL" className="rounded-xl h-10 px-6 data-[state=active]:bg-emerald-600 data-[state=active]:text-white transition-all text-xs font-bold tracking-widest text-slate-500 dark:text-slate-400">TÜM İLANLAR</TabsTrigger>
                <TabsTrigger value="SALE" className="rounded-xl h-10 px-6 data-[state=active]:bg-emerald-600 data-[state=active]:text-white transition-all text-xs font-bold tracking-widest text-slate-500 dark:text-slate-400">SATILIK</TabsTrigger>
                <TabsTrigger value="RENT" className="rounded-xl h-10 px-6 data-[state=active]:bg-emerald-600 data-[state=active]:text-white transition-all text-xs font-bold tracking-widest text-slate-500 dark:text-slate-400">KİRALIK</TabsTrigger>
                <TabsTrigger value="BOOKING" className="rounded-xl h-10 px-6 data-[state=active]:bg-emerald-600 data-[state=active]:text-white transition-all text-xs font-bold tracking-widest text-slate-500 dark:text-slate-400">GÜNLÜK / OTEL</TabsTrigger>
              </TabsList>
            </Tabs>

            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-4">
              <div className="relative flex-1 max-w-md group">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 dark:text-slate-400 group-focus-within:text-orange-500 transition-colors" />
                <Input placeholder={t('searchPlaceholder')} className="bg-white/5 border-slate-200 dark:border-white/10 rounded-2xl pl-12 h-14 text-slate-900 dark:text-white focus:ring-orange-500/20 focus:border-orange-500/40 transition-all font-medium shadow-2xl" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
              </div>
              <div className="flex gap-2">
                <Button variant="outline" className="h-14 px-6 rounded-2xl border-slate-200 dark:border-white/10 bg-white/5 text-slate-500 dark:text-slate-400 hover:text-white hover:bg-white/10 gap-2">
                ...
                <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
                  <DialogTrigger asChild>
                    <Button className="h-14 px-8 rounded-2xl bg-orange-600 hover:bg-orange-500 text-slate-900 dark:text-white font-bold text-[10px] shadow-xl shadow-orange-600/30 gap-2">
                  ...
                  <DialogContent className="sm:max-w-[425px] bg-[#14151a] border-slate-200 dark:border-white/10 text-slate-900 dark:text-white">
                    <DialogHeader>
                      <DialogTitle className="text-slate-900 dark:text-white">Create New Property</DialogTitle>
                      <DialogDescription className="text-slate-500 dark:text-slate-400">Enter the details for the new property.</DialogDescription>
                    </DialogHeader>
                    <div className="grid gap-4 py-4">
                      <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="title" className="text-right text-xs text-slate-500 dark:text-slate-400">Title</Label>
                        <Input id="title" className="col-span-3 h-10 bg-white/5 border-slate-200 dark:border-white/10 text-slate-900 dark:text-white" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} placeholder="Enter title" />
                      </div>
                      <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="type" className="text-right text-xs text-slate-500 dark:text-slate-400">Property Type</Label>
                        <Input id="type" className="col-span-3 h-10 bg-white/5 border-slate-200 dark:border-white/10 text-slate-900 dark:text-white" value={formData.type} onChange={e => setFormData({ ...formData, type: e.target.value })} placeholder="Enter property type" />
                      </div>
                      <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="price" className="text-right text-xs text-slate-500 dark:text-slate-400">Price</Label>
                        <Input id="price" className="col-span-3 h-10 bg-white/5 border-slate-200 dark:border-white/10 text-slate-900 dark:text-white" value={formData.price} onChange={e => setFormData({ ...formData, price: e.target.value })} placeholder="Enter price" />
                      </div>
                    </div>
                    <DialogFooter>
            ...
            <Card className="bg-white/5 border-slate-200 dark:border-white/10 rounded-4xl overflow-hidden shadow-2xl relative">
                  ...
                  <Table>
                    <TableHeader className="bg-white/5 border-b border-slate-200 dark:border-white/10">
                      <TableRow className="hover:bg-transparent border-none">
                        <TableHead className="text-[10px] font-bold text-slate-500 dark:text-slate-400 py-6 px-8">{t('propertyAssetprofile')}</TableHead>
                        <TableHead className="text-[10px] font-bold text-slate-500 dark:text-slate-400 px-8">{t('performanceYield')}</TableHead>
                        <TableHead className="text-[10px] font-bold text-slate-500 dark:text-slate-400 px-8 text-center">{t('propertyRequests')}</TableHead>
                        <TableHead className="text-[10px] font-bold text-slate-500 dark:text-slate-400 px-8">{t('propertyCompliance')}</TableHead>
                    ...
                    <TableBody>
                      {isLoading ? <TableRow>
                            <TableCell colSpan={5} className="py-20 text-center">
                               ...
                               <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 animate-pulse">{t('propertySyncing')}</p>
                            </TableCell>
                          </TableRow> : filteredProperties.length === 0 ? <TableRow>
                            <TableCell colSpan={5} className="py-20 text-center">
                               <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400">{t('noAssets')}</p>
                            </TableCell>
                         </TableRow> : filteredProperties.map(property => <TableRow key={property.id} className="border-b border-slate-200 dark:border-white/10 hover:bg-white/5 transition-all group">
                          <TableCell className="py-8 px-8">
                            <div className="flex items-center gap-6">
                              <div className="w-16 h-16 bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl flex items-center justify-center group-hover:scale-105 transition-all overflow-hidden relative">
                                 <Building className="w-8 h-8 text-slate-600" />
                              </div>
                              <div className="space-y-1">
                                <h4 className="text-lg font-bold text-slate-900 dark:text-white leading-tight">{property.name}</h4>
                                <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 flex items-center gap-1 leading-none">
                                ...
                                <div className="flex gap-2 mt-3 flex-wrap">
                                   ...
                                   <Badge variant="outline" className="text-[9px] font-bold text-slate-500 dark:text-slate-400 border-slate-200 dark:border-white/10 px-2">{getLocalizedType(property.type)}</Badge>
                                </div>
                              </div>
                            </div>
                          </TableCell>
                          <TableCell className="px-8">
                            <div className="space-y-3 min-w-[200px]">
                               <div className="flex justify-between items-end">
                                  <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400">{t("admin_property_yield_dna")}</span>
                                  <span className="text-sm font-bold text-emerald-400">${property.revenue?.toLocaleString()}</span>
                               </div>
                               <div className="flex justify-between items-end">
                                  <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400">{t("admin_property_occupancy")}</span>
                                  <span className="text-xs font-bold text-slate-900 dark:text-white">{property.occupancyRate}%</span>
                               </div>
                          ...
                          <TableCell className="px-8 text-center">
                             <div className="flex justify-center gap-4">
                                <div className="flex flex-col items-center">
                                   <span className="text-xl font-bold text-slate-900 dark:text-white leading-none">8</span>
                                   <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 mt-1">{t('newLeads')}</span>
                                </div>
                                <div className="flex flex-col items-center">
                                   <span className="text-xl font-bold text-slate-900 dark:text-white leading-none">3</span>
                                   <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 mt-1">{t('activeBookings')}</span>
                                </div>
                             </div>
                          </TableCell>
                          <TableCell className="px-8">
                          ...
                          <TableCell className="px-8 text-right">
                             <DropdownMenu>
                               <DropdownMenuTrigger asChild>
                                 <Button variant="ghost" className="h-12 w-12 rounded-2xl hover:bg-white/5 text-slate-500 dark:text-slate-400 hover:text-white transition-all">
                               ...
                               <DropdownMenuContent align="end" className="bg-[#14151a] border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl p-2 min-w-[180px]">
                                 ...
                                 <DropdownMenuItem onClick={() => navigate('/admin/maintenance')} className="rounded-xl px-4 py-3 text-[10px] font-bold text-slate-500 dark:text-slate-400 hover:text-white transition-all cursor-pointer">
                                    <Calendar className="w-4 h-4 mr-3 text-slate-500 dark:text-slate-400" /> {t('scheduleMaintenance')}
                                 </DropdownMenuItem>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
           <Card className="bg-white/5 border-slate-200 dark:border-white/10 rounded-4xl p-10 overflow-hidden shadow-2xl relative group">
              ...
              <div className="relative z-10 space-y-8">
                 ...
                 <div className="grid grid-cols-3 gap-6">
                    <div className="space-y-4">
                       <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400">{t('newLeads')}</span>
                       </div>
                       ...
                       <Progress value={78} className="h-1 bg-white/10 [&>div]:bg-slate-500" />
                    </div>
                    <div className="space-y-4">
                       <div className="flex items-center gap-2">
                          <Zap className="w-4 h-4 text-slate-500 dark:text-slate-400" />
                          <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400">{t('conversions')}</span>
                       </div>
                       <div className="flex items-end gap-2">
                       ...
                       <Progress value={62} className="h-1 bg-white/10 [&>div]:bg-slate-500" />
                    </div>
                    <div className="space-y-4">
                       <div className="flex items-center gap-2">
                          <Clock className="w-4 h-4 text-orange-400" />
                          <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400">{t('avgReplyTime')}</span>
                       </div>
                       <div className="flex items-end gap-2">
                          <span className="text-3xl font-bold text-slate-900 dark:text-white">{t("admin_property_12m")}</span>
                 ...
                 <Button variant="ghost" className="text-[10px] font-bold text-slate-500 dark:text-slate-400 hover:text-white flex items-center gap-2 px-0">
                 ...
                 <div className="space-y-6">
                    <div className="flex justify-between items-center group">
                       <div className="flex items-center gap-3">
                          ...
                          <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 tracking-tight">{t('netOperatingIncome')}</span>
                       </div>
                       <span className="text-lg font-bold text-slate-900 dark:text-white leading-none font-mono">$842,000</span>
                    </div>
                    <div className="flex justify-between items-center group">
                       <div className="flex items-center gap-3">
                          <div className="w-1.5 h-1.5 rounded-full bg-red-500 border border-red-400/50"></div>
                          <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 tracking-tight">{t('totalMonthlyExpenses')}</span>
                       </div>
                       <span className="text-lg font-bold text-slate-500 dark:text-slate-400 leading-none font-mono">$114,200</span>
                    </div>
                    <div className="flex justify-between items-center group">
                       <div className="flex items-center gap-3">
                          <div className="w-1.5 h-1.5 rounded-full bg-slate-500 border border-slate-400/50"></div>
                          <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 tracking-tight">{t('pendingCommissions')}</span>
                       </div>
                       <span className="text-lg font-bold text-slate-500 dark:text-slate-400 leading-none font-mono">$42,500</span>
                    </div>
                    <div className="flex justify-between items-center group">
                       <div className="flex items-center gap-3">
                          <div className="w-1.5 h-1.5 rounded-full bg-orange-500 border border-orange-400/50"></div>
                          <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 tracking-tight">{t('taxLiability')}</span>
                       </div>
                       <span className="text-lg font-bold text-slate-500 dark:text-slate-400 leading-none font-mono">$12,800</span>
                    </div>
                 </div>
                 <div className="pt-4 border-t border-slate-200 dark:border-white/10 flex gap-4">
                    <Button className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-slate-900 dark:text-white font-bold h-12 rounded-2xl text-[10px] shadow-xl shadow-emerald-600/20">{t('executeDisbursements')}</Button>
                    <Button variant="outline" className="flex-1 border-slate-200 dark:border-white/10 text-slate-500 dark:text-slate-400 hover:text-white h-12 rounded-2xl text-[10px] group">{t('viewFullLedger')}</Button>
                 </div>
              </div>
           </Card>
        </div>
