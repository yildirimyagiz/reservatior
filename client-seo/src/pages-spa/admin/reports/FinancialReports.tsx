...
export default function FinancialReports() {
  ...
  const stats = [
    ...
    { title: t("admin_reports_entity_subsystems"), value: "2,450", change: "+4.3%", trend: "up", icon: Zap, color: "text-slate-500 dark:text-slate-400" },
    { title: t("admin_reports_payout_queue"), value: "$12,400", change: "-2.1%", trend: "down", icon: Activity, color: "text-orange-400" },
    { title: t("admin_reports_system_net_margin"), value: "$84,200", change: "+8.7%", trend: "up", icon: PieChart, color: "text-slate-500 dark:text-slate-400" }
  ];
  return <div className="min-h-screen bg-background">
      <div className="p-6 space-y-6">
        <div className="bg-white/5 p-6 rounded-2xl border border-slate-200 dark:border-white/10">
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">{t("admin_reports_financial_neural_matrix")}</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">{t("admin_reports_systemwide_revenue_stream_analysis")}</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, i) => <Card key={i} className="bg-white/5 border-slate-200 dark:border-white/10 rounded-3xl overflow-hidden shadow-2xl relative group border-l-2 border-l-transparent hover:border-l-slate-500/50 transition-all">
              <div className={cn("absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-20 transition-all", stat.color)}>
                 <stat.icon className="w-12 h-12" />
              </div>
              <CardContent className="p-8">
                <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 mb-1">{stat.title}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <Card className="lg:col-span-8 bg-white/5 backdrop-blur-xl border-slate-200 dark:border-white/10 rounded-4xl overflow-hidden shadow-2xl border-l border-t relative group">
            <CardHeader className="p-8 border-b border-slate-200 dark:border-white/10 flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription className="text-[10px] font-bold text-slate-500 dark:text-slate-400 mt-1">{t("admin_reports_crosscluster_revenue_performance_metrics")}</CardDescription>
              </div>
              <Button variant="ghost" className="h-10 rounded-xl hover:bg-white/5 text-slate-500 dark:text-slate-400 font-bold text-[9px] gap-2">{t("admin_reports_synchronize_matrix")}<Download className="w-3.5 h-3.5" />
              </Button>
            </CardHeader>
          ...
          <Card className="lg:col-span-4 bg-white/5 backdrop-blur-xl border-slate-200 dark:border-white/10 rounded-4xl overflow-hidden shadow-2xl border-l border-t">
            ...
            <CardContent className="p-8 space-y-8">
              {[{ name: t("admin.reports.entity_agencies", "Varlık Acenteleri"), value: "$280k", pct: 62, color: "bg-slate-500 shadow-slate-500/40" },
                ...
                { name: t("admin.reports.protocol_fees", "Protokol Ücretleri"), value: "$26k", pct: 5, color: "bg-orange-500 shadow-orange-500/40" }
                    ...
                    <div className="flex justify-between items-end">
                      ...
                      <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400">{item.value}</span>
                    </div>
              ...
              <div className="pt-8 border-t border-slate-200 dark:border-white/10 mt-auto">
                 <div className="p-6 bg-white/5 rounded-3xl border border-slate-200 dark:border-white/10 group hover:bg-white/10 transition-all cursor-pointer relative">
                    <p className="text-[9px] font-bold text-slate-500 dark:text-slate-400 mb-1">{t("admin_reports_projection_buffer")}</p>
                    <p className="text-xl font-bold text-slate-900 dark:text-white whitespace-pre leading-none">{t("admin_reports_positive_cycle_forecast")}</p>
        ...
        <Card className="bg-white/5 border-slate-200 dark:border-white/10 rounded-4xl overflow-hidden shadow-2xl border-l border-t relative">
           <CardHeader className="p-8 border-b border-slate-200 dark:border-white/10 flex flex-row items-center justify-between">
              ...
              <Button variant="ghost" className="h-10 px-4 rounded-xl hover:bg-white/5 text-slate-500 dark:text-slate-400 font-bold text-[9px] gap-2">{t("admin_reports_view_global_ledger")}<ChevronRight className="w-4 h-4" />
              </Button>
           </CardHeader>
           <CardContent className="p-0">
             <div className="overflow-x-auto">
               <table className="w-full text-left">
                 <thead className="bg-white/5 border-b border-slate-200 dark:border-white/10">
                   <tr className="border-none hover:bg-transparent">
                     <th className="py-6 px-8 text-[10px] font-bold text-slate-500 dark:text-slate-400">{t("admin_reports_temporal_hash")}</th>
                     <th className="py-6 px-8 text-[10px] font-bold text-slate-500 dark:text-slate-400">{t("admin_reports_entity_origin")}</th>
                     <th className="py-6 px-8 text-[10px] font-bold text-slate-500 dark:text-slate-400">{t("admin_reports_sync_date")}</th>
                     <th className="py-6 px-8 text-[10px] font-bold text-slate-500 dark:text-slate-400">{t("admin_reports_type_class")}</th>
                     <th className="py-6 px-8 text-[10px] font-bold text-slate-500 dark:text-slate-400 text-right">{t("admin_reports_creditdebit")}</th>
                     <th className="py-6 px-8 text-[10px] font-bold text-slate-500 dark:text-slate-400 text-right">{t("admin_reports_node_state")}</th>
                   </tr>
                 </thead>
                 <tbody className="divide-y divide-white/5">
                   {[1, 2, 3, 4, 5, 6].map((_, i) => <tr key={i} className="group hover:bg-white/5 transition-all">
                       <td className="py-8 px-8 font-mono text-[10px] font-bold text-slate-500 dark:text-slate-400">{t("admin_reports_trx9482")}{i}{t("admin_reports_xls")}</td>
                       <td className="py-8 px-8">
                          <p className="text-sm font-bold text-slate-900 dark:text-white leading-none">{t("admin_reports_neural_realty_systems")}{i + 1}</p>
                          <p className="text-[9px] font-bold text-slate-600 mt-1">{t("admin_reports_orgclusterdelta")}</p>
                       </td>
                       <td className="py-8 px-8 text-[10px] font-bold text-slate-500 dark:text-slate-400">{t("admin_reports_mar")}{24 - i}, 2026</td>
                       <td className="py-8 px-8">
                         <Badge variant="outline" className="bg-white/5 border-slate-200 dark:border-white/10 text-[9px] font-bold px-2 py-0.5 text-slate-500 dark:text-slate-400">{t("admin_reports_systemsync")}</Badge>
                       </td>
                       <td className="py-8 px-8 text-right font-bold text-slate-900 dark:text-white">$1,250.00</td>
