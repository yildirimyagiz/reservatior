          ...
          {MOCK_RULES.map(rule => <div key={rule.id} className={`bg-card border rounded-xl p-4 flex items-center gap-4 ${rule.isActive ? "border-border" : "border-border opacity-60"}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {rule.isActive ? <Badge className="bg-green-100 text-green-700 border-0 text-xs">{t("admin_ai_active")}</Badge> : <Badge className="bg-white/5 text-slate-500 dark:text-slate-400 border-0 text-xs">{t("admin_ai_inactive")}</Badge>}
                </div>
