...
import { FileText, Database, Plus, Edit, Trash2, MoreHorizontal, Eye } from"lucide-react";
export default function AiExtractedDataPage() {
 ...
 const [selectedItemForView, setSelectedItemForView] = useState<AiExtractedData | null>(null);
 const {
 toast
 } = useToast();
 ...
 <div>
 <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-slate-200 to-slate-500">{t("admin_ai_ai_extracted_data")}</h1>
 ...
 <div className="bg-muted0 h-2 rounded-full" style={{
 ...
 <DialogContent className="max-w-2xl bg-card border-border text-foreground font-mono">
 <DialogHeader>
 <DialogTitle className="text-foreground">{t("admin_auto_extracted_metadata_payload", "Çıkarılan Meta Veri Yükü")}</DialogTitle>
 <DialogDescription className="text-muted-foreground">
 {t("admin_auto_json_output_generated_by", "Tarafından oluşturulan JSON çıktısı")}{selectedItemForView?.aiModel ||"AI model"}
 </DialogDescription>
 </DialogHeader>
 <div className="py-4 overflow-x-auto max-h-[60vh] bg-black/40 p-4 rounded-xl border border-border">
 <pre className="text-success text-sm">
 {selectedItemForView ? JSON.stringify(selectedItemForView.extractedJson, null, 2) :""}
 </pre>
 </div>
 <DialogFooter>
 <Button onClick={() => setIsViewDialogOpen(false)}>{t("admin_auto_close_buffer", "Tamponu Kapat")}</Button>
 </DialogFooter>
 </DialogContent>
 </Dialog>
 </div>
