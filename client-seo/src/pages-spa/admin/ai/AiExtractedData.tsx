...
export default function AiExtractedDataPage() {
 ...
 <DialogContent className="max-w-2xl bg-card border-white/5 text-foreground font-mono">
 <DialogHeader>
 <DialogTitle className="text-foreground">{t("admin_auto_extracted_metadata_payload", "Extracted Metadata Payload")}</DialogTitle>
 <DialogDescription className="text-muted-foreground">
 {t("admin_auto_json_output_generated_by", "JSON output generated by")}{selectedItemForView?.aiModel ||"AI model"}
 </DialogDescription>
 </DialogHeader>
 <div className="py-4 overflow-x-auto max-h-[60vh] bg-black/40 p-4 rounded-xl border border-white/5">
 <pre className="text-emerald-400 text-sm">
 {selectedItemForView ? JSON.stringify(selectedItemForView.extractedJson, null, 2) :""}
 </pre>
 </div>
 <DialogFooter>
 <Button onClick={() => setIsViewDialogOpen(false)}>{t("admin_auto_close_buffer", "Close Buffer")}</Button>
 </DialogFooter>
 </DialogContent>
 </Dialog>
 </div>
