    ...
    <div className="space-y-6 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-white/5 border-slate-200 dark:border-white/10">
          ...
          <CardContent className="space-y-4">
            <div className="flex justify-between items-center text-sm p-3 bg-white/5 rounded-lg">
              <span className="text-slate-500 dark:text-slate-400">{t("admin_ai_system_status", "System Status")}</span>
              <Badge className={cn(
                "border-0",
            ...
            <div className="flex justify-between items-center text-sm p-3 bg-white/5 rounded-lg">
              <span className="text-slate-500 dark:text-slate-400">{t("admin_ai_active_sessions", "Active Sessions")}</span>
              <span className="font-bold text-slate-900 dark:text-white">{status?.metrics?.activeSessions ?? "—"}</span>
            </div>
            <div className="flex justify-between items-center text-sm p-3 bg-white/5 rounded-lg">
              <span className="text-slate-500 dark:text-slate-400">{t("admin_ai_pending_tasks", "Pending Tasks")}</span>
        ...
        <Card className="bg-white/5 border-slate-200 dark:border-white/10">
          ...
          <CardContent className="space-y-4">
            <div className="flex justify-between items-center text-sm p-3 bg-white/5 rounded-lg">
              <span className="text-slate-500 dark:text-slate-400">{t("admin_ai_total_sessions", "Total Sessions")}</span>
              <span className="font-bold text-slate-900 dark:text-white">{status?.metrics?.totalSessions ?? "—"}</span>
            </div>
            <div className="flex justify-between items-center text-sm p-3 bg-white/5 rounded-lg">
              <span className="text-slate-500 dark:text-slate-400">{t("admin_ai_total_valuations", "Total Valuations")}</span>
              <span className="font-bold text-slate-900 dark:text-white">{status?.metrics?.totalValuations ?? "—"}</span>
            </div>
            <div className="flex justify-between items-center text-sm p-3 bg-white/5 rounded-lg">
              <span className="text-slate-500 dark:text-slate-400">{t("admin_ai_uptime", "Uptime")}</span>
        ...
        <Card className="bg-white/5 border-slate-200 dark:border-white/10">
          ...
          <CardContent className="space-y-4">
            <div className="flex justify-between items-center text-sm p-3 bg-white/5 rounded-lg">
              <span className="text-slate-500 dark:text-slate-400">{t("admin_ai_valuation_model", "Valuation / Price Prediction")}</span>
              <span className="font-medium text-slate-900 dark:text-white">{status?.models?.valuation ?? "XGBoost-v4"}</span>
            </div>
            <div className="flex justify-between items-center text-sm p-3 bg-white/5 rounded-lg">
              <span className="text-slate-500 dark:text-slate-400">{t("admin_ai_location_model", "Location Intelligence")}</span>
              <span className="font-medium text-slate-900 dark:text-white">{"GeoGraph-v2"}</span>
            </div>
            <div className="flex justify-between items-center text-sm p-3 bg-white/5 rounded-lg">
              <span className="text-slate-500 dark:text-slate-400">{t("admin_ai_chatbot_model", "Chatbot")}</span>
              <span className="font-medium text-slate-900 dark:text-white">{status?.models?.chatbot ?? "—"}</span>
            </div>
            <div className="flex justify-between items-center text-sm p-3 bg-white/5 rounded-lg">
              <span className="text-slate-500 dark:text-slate-400">{t("admin_ai_recommendation_model", "Recommendation")}</span>
        ...
        <Card className="bg-white/5 border-slate-200 dark:border-white/10">
          ...
          <CardContent>
            <div className="space-y-4">
              {[
                { label: t("admin_ai_regression", "Regression"), count: models.filter(m => m.modelType === "Regression").length, color: "bg-slate-500" },
                { label: t("admin_ai_nlp", "NLP"), count: models.filter(m => m.modelType === "NLP").length, color: "bg-slate-500" },
                ...
                { label: t("admin_ai_other", "Other"), count: models.filter(m => !["Regression", "NLP", "Classification", "Diffusion"].includes(m.modelType)).length, color: "bg-slate-500" },
