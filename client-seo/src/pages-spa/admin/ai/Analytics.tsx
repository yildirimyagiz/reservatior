...
const insights = [{
  title: t("admin_ai_property_prices_expected_to"),
  confidence: 92,
  category: "Market",
  impact: "high"
}, {
  title: t("admin_ai_lead_conversion_rate_improved"),
  confidence: 88,
  category: "CRM",
  impact: "medium"
}, {
  title: t("admin_ai_market_saturation_detected_in"),
  confidence: 95,
  category: "Risk",
  impact: "high"
}, {
  title: t("admin_ai_new_rental_opportunities_identified"),
  confidence: 87,
  category: "Opportunity",
  impact: "medium"
}, {
  title: t("admin_ai_maintenance_costs_can_be"),
  confidence: 91,
  category: "Operations",
  impact: "high"
}];
const kpis = [{
  label: t("admin_ai_model_accuracy"),
  value: "96.8%",
  change: "+2.3%",
  icon: Target,
  ...
  return <div className="p-4 md:p-8 space-y-8 max-w-(--breakpoint-2xl) mx-auto min-h-screen">
      ...
      <div className="bg-white/5 p-6 rounded-2xl border border-slate-200 dark:border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" className="border-slate-200 dark:border-white/10 text-slate-500 dark:text-slate-400">
            <RefreshCw className="w-4 h-4 mr-2" />{t("admin_ai_refresh")}</Button>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-white/5 border border-slate-200 dark:border-white/10 p-1">
          <TabsTrigger value="overview" className="data-[state=active]:bg-primary data-[state=active]:text-white text-slate-500 dark:text-slate-400 rounded-lg">{t("admin_ai_overview")}</TabsTrigger>
          <TabsTrigger value="models" className="data-[state=active]:bg-primary data-[state=active]:text-white text-slate-500 dark:text-slate-400 rounded-lg">{t("admin_ai_models")}</TabsTrigger>
          <TabsTrigger value="insights" className="data-[state=active]:bg-primary data-[state=active]:text-white text-slate-500 dark:text-slate-400 rounded-lg">{t("admin_ai_insights")}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview" className="space-y-8 mt-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <Card className="lg:col-span-2 bg-white/5 border-slate-200 dark:border-white/10 rounded-2xl overflow-hidden">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-slate-900 dark:text-white">
                  <Activity className="w-5 h-5 text-primary" />{t("admin_ai_model_accuracy_trend")}</CardTitle>
                <CardDescription className="text-slate-500 dark:text-slate-400">{t("admin_ai_weekly_accuracy_performance_across")}</CardDescription>
              </CardHeader>
            ...
            <Card className="bg-white/5 border-slate-200 dark:border-white/10 rounded-2xl">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-slate-900 dark:text-white">
                  <Database className="w-5 h-5 text-primary" />{t("admin_ai_request_volume")}</CardTitle>
              </CardHeader>
        ...
        <TabsContent value="models" className="space-y-6 mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            ...
            delay: idx * 0.06
                ...
                <Card className="bg-white/5 border-slate-200 dark:border-white/10 rounded-2xl hover:border-primary/20 transition-all">
                  <CardContent className="p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-3 text-center">
                      <div className="p-2 rounded-lg bg-white/5">
                        <p className="text-xs text-slate-500 dark:text-slate-400">{t("admin_ai_requests")}</p>
                        <p className="text-sm font-bold text-slate-900 dark:text-white">{(model.requests / 1000).toFixed(1)}K</p>
                      </div>
                      <div className="p-2 rounded-lg bg-white/5">
                        <p className="text-xs text-slate-500 dark:text-slate-400">{t("admin_ai_latency")}</p>
                        <p className="text-sm font-bold text-slate-900 dark:text-white">{model.latency}</p>
                      </div>
                      <div className="p-2 rounded-lg bg-white/5">
                        <p className="text-xs text-slate-500 dark:text-slate-400">{t("admin_ai_last_train")}</p>
                        <p className="text-sm font-bold truncate text-slate-900 dark:text-white">{model.lastTrained}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
        ...
        <TabsContent value="insights" className="space-y-6 mt-6">
          <Card className="bg-white/5 border-slate-200 dark:border-white/10 rounded-2xl">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-slate-900 dark:text-white">
                <Sparkles className="w-5 h-5 text-primary" />{t("admin_ai_aigenerated_insights")}</CardTitle>
              <CardDescription className="text-slate-500 dark:text-slate-400">{t("admin_ai_automated_analysis_and_recommendations")}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              delay: idx * 0.06
            }} className={`p-4 rounded-xl border transition-all hover:border-primary/20 ${insight.impact === "high" ? "border-l-4 border-l-amber-500 bg-amber-500/5 border-slate-200 dark:border-white/10" : "border-l-4 border-l-slate-500 bg-slate-500/5 border-slate-200 dark:border-white/10"}`}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        {insight.impact === "high" ? <AlertTriangle className="w-4 h-4 text-amber-400" /> : <TrendingUp className="w-4 h-4 text-slate-500 dark:text-slate-400" />}
                        <Badge variant="outline" className="text-[10px] text-slate-500 dark:text-slate-400">{insight.category}</Badge>
                      </div>
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">{t("admin_ai_confidence")}</p>
                    </div>
                  </div>
                </motion.div>)}
            </CardContent>
