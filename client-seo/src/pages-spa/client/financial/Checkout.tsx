...
const PLAN_CATALOG: Record<string, { name: string; priceCents: number | null; maxProperties: number | null; maxUsers: number | null; maxListings: number | null; ai: boolean; integrations: boolean; support: boolean }> = {
  "starter-10":       { name: "STARTER (10 MÜLK)",   priceCents: 1900,  maxProperties: 10,   maxUsers: 1,  maxListings: 25,   ai: false, integrations: false, support: false },
  "growth-25":        { name: "GROWTH (25 MÜLK)",     priceCents: 4900,  maxProperties: 25,   maxUsers: 3,  maxListings: 100,  ai: false, integrations: false, support: false },
  "professional-50":  { name: "PRO (50 MÜLK)",        priceCents: 8000,  maxProperties: 50,   maxUsers: 5,  maxListings: 250,  ai: true,  integrations: false, support: true },
  "agency-100":       { name: "AGENCY (100 MÜLK)",    priceCents: 15000, maxProperties: 100,  maxUsers: 10, maxListings: 500,  ai: true,  integrations: false, support: true },
...
export default function Checkout() {
  ...
  const displayPrice = plan?.priceCents ? `$${(plan.priceCents / 100).toFixed(0)}` : t("checkout.custom", "ÖZEL");

  const steps = [
    { label: t("checkout.step_summary", "Paket Özeti"), icon: Receipt },
    { label: t("checkout.step_contract", "Sözleşme"), icon: FileText },
    { label: t("checkout.step_payment", "Ödeme"), icon: CreditCard },
  ];

  const handleCheckout = async () => {
    if (!contractAccepted) {
      toast({ title: t("checkout.contract_required", "Sözleşme Onayı Gerekli"), description: t("checkout.please_accept", "Devam etmek için hizmet sözleşmesini kabul edin."), variant: "destructive" });
      return;
    }
    try {
      setLoading(true);
      const payload = isPlanCheckout
        ? { planId, amount: plan!.priceCents, currency: "USD", paymentType: "SUBSCRIPTION", description: `Reservatior ${plan!.name} Abonelik` }
      ...
      if (response?.url) {
        ...
        toast({ title: t("checkout.success_demo", "Demo Modu"), description: t("checkout.demo_desc", "Ödeme altyapısı henüz entegre edilmemiştir. İşleminiz kaydedildi.") });
  ...
  const planFeatures = useMemo(() => {
    if (!plan) return [];
    const f: string[] = [];
    f.push(plan.maxProperties ? `${plan.maxProperties} Mülke Kadar Portföy` : "Sınırsız Mülk Portföyü");
    f.push(plan.maxUsers ? `${plan.maxUsers} Kullanıcı Erişimi` : "Sınırsız Kullanıcı");
    f.push(plan.maxListings ? `${plan.maxListings} Aktif İlan` : "Sınırsız İlan Yayını");
    if (plan.ai) f.push("Yapay Zeka Destekli Analiz");
    if (plan.integrations) f.push("Özel Entegrasyonlar & ERP");
    f.push(plan.support ? "7/24 Öncelikli Destek" : "E-posta Desteği");
    return f;
  }, [plan]);

  if (!isPlanCheckout && !bookingId && amount === 0) {
    return (
      <PageShell title={t("checkout.no_plan", "Paket Seçilmedi")} description="">
        <div className="flex flex-col items-center justify-center py-20 gap-6">
          <AlertCircle className="w-16 h-16 text-yellow-500" />
          <p className="text-slate-400 text-sm">{t("checkout.select_plan_first", "Lütfen önce bir paket seçin.")}</p>
          <Button onClick={() => navigate("/pricing")} className="bg-blue-600 hover:bg-blue-500">
            <ChevronLeft className="w-4 h-4 mr-2" /> {t("checkout.back_to_pricing", "Paketlere Dön")}
          </Button>
        </div>
      </PageShell>
    );
  ...
  return (
    <PageShell
      title={isPlanCheckout ? t("checkout.subscribe_title", "Abonelik Oluştur") : t("client.src.secure_neural_checkout")}
      description={isPlanCheckout ? t("checkout.subscribe_desc", "Seçtiğiniz paketi inceleyip sözleşmeyi onaylayarak aboneliğinizi başlatın.") : t("client.src.finalize_your_portfolio_transaction")}
    >
      {/* Step Progress Bar */}
        ...
        <AnimatePresence mode="wait">
                ...
                {isPlanCheckout && (
                  <>
                    <div className="flex items-baseline gap-2 mb-8">
                      ...
                      {plan!.priceCents && <span className="text-slate-500 text-xs font-bold tracking-wider">/ BİRİM BAŞINA AY</span>}
                    </div>
              ...
              <div className="flex flex-col justify-between">
                <div className="space-y-4 mb-8">
                  <div className="bg-[#1a1b1e]/60 border border-slate-800 p-6 rounded-2xl">
                    <h4 className="font-bold flex items-center gap-2 mb-2 text-white"><Zap className="w-4 h-4 text-yellow-400" /> Anında Aktivasyon</h4>
                    <p className="text-xs text-slate-400 leading-relaxed">Ödeme onaylandıktan sonra hesabınız anında aktif hale gelir ve tüm özelliklere erişim sağlarsınız.</p>
                  </div>
                  <div className="bg-[#1a1b1e]/60 border border-slate-800 p-6 rounded-2xl">
                    <h4 className="font-bold flex items-center gap-2 mb-2 text-white"><ShieldCheck className="w-4 h-4 text-emerald-400" /> Güvenli İşlem</h4>
                    <p className="text-xs text-slate-400 leading-relaxed">Tüm ödeme bilgileriniz AES-256 şifreleme ile korunur. Kart bilgileriniz sunucularımızda saklanmaz.</p>
                  </div>
                </div>
                <Button onClick={() => setStep(1)} className="w-full h-14 bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm tracking-wider rounded-2xl gap-2">
                  {t("checkout.continue_contract", "Sözleşmeye Devam Et")} <ChevronRight className="w-4 h-4" />
                </Button>
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="contract" initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 30 }}>
              <div className="bg-[#1a1b1e]/60 backdrop-blur-xl border border-slate-800 p-8 rounded-3xl mb-6">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-lg font-black text-white flex items-center gap-3"><FileText className="w-6 h-6 text-blue-400" /> {t("checkout.service_agreement", "Hizmet Sözleşmesi")}</h2>
              ...
              <div className="flex gap-4">
                <Button onClick={() => setStep(0)} variant="outline" className="flex-1 h-14 rounded-2xl border-slate-700 text-slate-300 hover:bg-slate-800 font-bold text-sm gap-2">
                  <ChevronLeft className="w-4 h-4" /> {t("checkout.back", "Geri")}
                </Button>
                <Button onClick={() => { if (contractAccepted) setStep(2); else toast({ title: "Sözleşme Onayı Gerekli", description: "Devam etmek için sözleşmeyi kabul edin.", variant: "destructive" }); }} className={`flex-1 h-14 rounded-2xl font-bold text-sm gap-2 transition-all ${contractAccepted ? "bg-blue-600 hover:bg-blue-500 text-white" : "bg-slate-800 text-slate-500 cursor-not-allowed"}`}>
                  {t("checkout.continue_payment", "Ödemeye Devam Et")} <ChevronRight className="w-4 h-4" />
                </Button>
              </div>
            </motion.div>
          )}
          ...
          {step === 2 && (
            <motion.div key="payment" initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 30 }} className="grid md:grid-cols-2 gap-8">
              <div className="bg-[#1a1b1e]/60 backdrop-blur-xl border border-slate-800 p-8 rounded-3xl">
                <h2 className="text-lg font-black text-white flex items-center gap-3 mb-8"><CreditCard className="w-6 h-6 text-blue-400" /> {t("checkout.payment_info", "Ödeme Bilgileri")}</h2>

                <div className="bg-blue-600/5 border border-blue-600/20 p-6 rounded-xl flex justify-between items-center mb-8">
                  <div>
                    <div className="text-slate-500 text-xs font-bold tracking-wider mb-1">TOPLAM TUTAR</div>
                ...
                <div className="space-y-3 mb-8">
                  ...
                  <div className="flex items-center gap-3 text-sm text-slate-400">
                    ...
                    <span>Anında hesap aktivasyonu</span>
                  </div>
                </div>
                ...
                <Button onClick={handleCheckout} disabled={loading} className="w-full h-16 bg-blue-600 hover:bg-blue-500 text-white font-black text-sm tracking-widest rounded-2xl gap-3 shadow-xl shadow-blue-600/20 transition-all">
                  {loading ? (
                    <span className="flex items-center gap-2"><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> İşleniyor...</span>
                  ) : (
                    <><CreditCard className="w-5 h-5" /> {t("checkout.pay_now", "ŞİMDİ ÖDE")}</>
                  )}
                </Button>
              </div>
              ...
              <div className="flex flex-col gap-4">
                <div className="bg-[#1a1b1e]/60 border border-slate-800 p-6 rounded-2xl">
                  <h4 className="font-bold flex items-center gap-2 mb-2 text-white"><Rocket className="w-4 h-4 text-blue-400" /> Sonraki Adımlar</h4>
                  <ul className="space-y-2 text-xs text-slate-400">
                    <li className="flex items-start gap-2"><span className="text-blue-400 font-bold">1.</span> Ödeme başarılı olduktan sonra hesabınız aktif edilir</li>
                    <li className="flex items-start gap-2"><span className="text-blue-400 font-bold">2.</span> Dashboard'a yönlendirilirsiniz</li>
                    <li className="flex items-start gap-2"><span className="text-blue-400 font-bold">3.</span> Mülk eklemeye hemen başlayabilirsiniz</li>
                  </ul>
                </div>
                ...
                <div className="mt-auto flex items-start gap-4 p-4 bg-yellow-600/5 border border-yellow-600/20 rounded-xl">
                  ...
                  <div>
                    <h5 className="font-bold text-sm text-yellow-500 mb-1">İptal Politikası</h5>
                    <p className="text-xs text-slate-400 leading-relaxed">Aboneliğinizi istediğiniz zaman iptal edebilirsiniz. İptal sonrası mevcut fatura dönemi sonuna kadar hizmetiniz devam eder.</p>
                  </div>
                </div>
                ...
                <Button onClick={() => setStep(1)} variant="outline" className="h-12 rounded-2xl border-slate-700 text-slate-400 hover:bg-slate-800 font-bold text-xs gap-2">
                  <ChevronLeft className="w-4 h-4" /> {t("checkout.back_contract", "Sözleşmeye Dön")}
                </Button>
              </div>
            </motion.div>
          )}
