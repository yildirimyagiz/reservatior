...
export function FinanceLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems: NavItem[] = [
    ...
    },
    {
      ...
      href: "/finance-os/dashboard",
      icon: LayoutDashboard
    },
    {
      title: "Ledgers & Payouts",
      href: "/finance-os/ledger",
      icon: Receipt
    },
    {
      title: "Escrow Status",
      href: "/finance-os/escrow",
      icon: ShieldCheck
    },
    {
        ...
        <div className="flex items-center gap-2">
          <Avatar className="h-8 w-8 shrink-0">
            <AvatarFallback className="text-xs bg-primary/20 text-primary border border-primary/30">
              {user?.firstName?.slice(0, 2).toUpperCase() || "FI"}
            </AvatarFallback>
          </Avatar>
          {sidebarOpen && (
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate text-foreground">{user?.firstName || "Finance"}</p>
