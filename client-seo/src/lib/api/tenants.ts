...
export const tenantsApi = {
  ...
  getAll: (params?: { orgId?: string; page?: number; limit?: number; active?: boolean }) => 
    apiClient.get<TenantsResponse>("/api/tenants", { params }),
  ...
  update: (id: string, data: TenantUpdate) => 
    apiClient.patch<Tenant>(`/api/tenants/${id}`, data),
  ...
  getIncreases: (params?: { orgId?: string; propertyId?: string; type?: string; page?: number; limit?: number }) =>
    apiClient.get<{ data: Increase[]; total: number }>("/api/hoa/increases", { params }),

  createIncrease: (data: Partial<Increase> & { orgId: string }) =>
    apiClient.post<{ data: Increase }>("/api/hoa/increases", data),

  updateIncrease: (id: string, data: Partial<Increase>) =>
    apiClient.patch<{ data: Increase }>(`/api/hoa/increases/${id}`, data),

  deleteIncrease: (id: string) =>
    apiClient.delete(`/api/hoa/increases/${id}`),

  getLeases: (id: string, params?: { page?: number; limit?: number; status?: string }) =>
    apiClient.get<any>(`/api/tenants/${id}/leases`, { params }),

  getPayments: (id: string, params?: { page?: number; limit?: number; status?: string; type?: string }) =>
    apiClient.get<any>(`/api/tenants/${id}/payments`, { params }),

  getMaintenance: (id: string, params?: { page?: number; limit?: number; status?: string; priority?: string }) =>
    apiClient.get<any>(`/api/tenants/${id}/maintenance`, { params }),

  // Scoring endpoints
  calculateScore: (id: string) =>
    apiClient.post<Tenant>(`/api/tenants/${id}/calculate-score`, {}),

  getScore: (id: string) =>
    apiClient.get<Tenant>(`/api/tenants/${id}/score`),
};
