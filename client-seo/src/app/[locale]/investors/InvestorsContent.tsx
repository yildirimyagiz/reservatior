...
import { TrendingUp, DollarSign, Percent, PieChart, ShieldCheck } from "lucide-react";
      ...
      <section className="relative h-[80svh] w-full overflow-hidden flex items-center justify-center">
        ...
        <div className="relative z-10 container mx-auto px-6 pt-24 flex flex-col items-start max-w-7xl">
          <m.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-3xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 border border-indigo-500/30 text-indigo-600 dark:text-indigo-400 mb-6 backdrop-blur-md">
            ...
            <div className="flex flex-wrap gap-4">
              <Button size="lg" className="rounded-full px-8 h-14 bg-primary text-primary-foreground hover:bg-primary/90 font-bold transition-all hover:scale-105">
                {t("client.investor.cta_primary", { defaultValue: "View Listings" })}
              </Button>
              <Button size="lg" variant="outline" className="rounded-full px-8 h-14 border-border hover:bg-accent font-bold backdrop-blur-md">
        ...
        <div className="container mx-auto px-6 max-w-7xl">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <m.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }}
              className="bg-card border border-border rounded-[2.5rem] p-8 backdrop-blur-xl relative overflow-hidden group flex flex-col justify-between">
              <div>
                <DollarSign className="w-10 h-10 text-indigo-500 dark:text-indigo-400 mb-6" />
              ...
              <h4 className="text-4xl font-black mt-8 text-indigo-500 dark:text-indigo-400">$0.00</h4>
            </m.div>

            <m.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}
              className="bg-card border border-border rounded-[2.5rem] p-8 backdrop-blur-xl relative overflow-hidden group flex flex-col justify-between">
              <div>
                <Percent className="w-10 h-10 text-emerald-500 dark:text-emerald-400 mb-6" />
              ...
              <h4 className="text-4xl font-black mt-8 text-emerald-500 dark:text-emerald-400">8.4% - 12.1%</h4>
            </m.div>

            <m.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }}
              className="bg-card border border-border rounded-[2.5rem] p-8 backdrop-blur-xl relative overflow-hidden group flex flex-col justify-between">
              <div>
                <TrendingUp className="w-10 h-10 text-purple-500 dark:text-purple-400 mb-6" />
              ...
              <h4 className="text-4xl font-black mt-8 text-purple-500 dark:text-purple-400">+12.3%</h4>
            </m.div>

            <m.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.4 }}
              className="md:col-span-3 bg-gradient-to-br from-indigo-500/10 to-purple-500/10 border border-indigo-500/20 rounded-[2.5rem] p-8 md:p-12 backdrop-blur-xl relative overflow-hidden group flex flex-col justify-between">
              <div className="absolute top-0 right-0 w-96 h-96 bg-indigo-500/10 rounded-full blur-[120px]" />
              <div className="relative z-10">
                <ShieldCheck className="w-12 h-12 text-indigo-500 dark:text-indigo-400 mb-6" />
