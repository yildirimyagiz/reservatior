...
export function Expenses() {
  ...
  const createMutation = useMutation({
    ...
    onError: () => {
      toast({ title: t("common.error"), description: t("client.src.failed_to_save_expense"), variant: "destructive" });
    }
  });
  ...
  return <>
      ...
      value: `$${expenses.reduce((s, e) => s + e.amount, 0).toLocaleString()}`
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
          <Table>
            <TableHeader><TableRow><TableHead>{t("common.category")}</TableHead><TableHead>{t("common.amount")}</TableHead><TableHead>{t("common.date")}</TableHead><TableHead>{t("common.description")}</TableHead><TableHead className="w-10" /></TableRow></TableHeader>
            <TableBody>
...
export function FinancialRecords() {
  ...
  return <PageShell title={t("client.src.financial_records")} description={t("client.src.income_expenses_and_profit")} stats={[{
    ...
    color: "text-brand"
          ...
          <Button variant="outline" size="sm" className="border-purple-200 text-brand bg-brand/10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div className="lg:col-span-2">
            <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
                <Table>
                    <TableHeader><TableRow><TableHead>{t("common.date")}</TableHead><TableHead>{t("common.description")}</TableHead><TableHead>{t("common.category")}</TableHead><TableHead>{t("common.type")}</TableHead><TableHead className="text-right">{t("common.amount")}</TableHead></TableRow></TableHeader>
                    <TableBody>
                        {loading ? <TableRow><TableCell colSpan={5} className="text-center py-12"><Loader2 className="w-6 h-6 animate-spin mx-auto" /></TableCell></TableRow> : records.length === 0 ? <TableRow><TableCell colSpan={5} className="text-center py-12 text-muted-foreground">{t("client.src.no_records_found")}</TableCell></TableRow> : records.map(r => <TableRow key={r.id} className="hover:bg-muted/40 transition-colors">
                            ...
                            <TableCell>
                                {r.type === "INCOME" ? <span className="text-xs flex items-center gap-1 text-blue-600 font-medium"><TrendingUp className="w-3 h-3" />{t("client.src.income")}</span> : <span className="text-xs flex items-center gap-1 text-red-600 font-medium"><TrendingDown className="w-3 h-3" />{t("client.src.expense")}</span>}
                            </TableCell>
        ...
        <div className="space-y-6">
            <div className="p-6 bg-gradient-to-br from-brand to-brand rounded-2xl text-white shadow-xl shadow-indigo-200 overflow-hidden relative">
                ...
                <h3 className="text-lg font-black italic mb-4 flex items-center gap-2">
                    <Wand2 className="w-5 h-5 text-brand" />{t("client.src.neural_roi_hub")}</h3>
            ...
            <div className="p-6 bg-card border border-border rounded-2xl shadow-sm">
                <h4 className="text-sm font-bold mb-4 flex items-center gap-2">
                    <Video className="w-4 h-4 text-brand" />{t("client.src.recent_marketing_assets")}</h4>
                <div className="space-y-3">
                    <div className="flex items-center justify-between p-2 hover:bg-muted/50 rounded-lg transition-colors cursor-pointer">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 bg-brand/15 rounded flex items-center justify-center">
                        ...
                        <Badge className="bg-success/10 text-success border-0 text-[9px] font-bold">{t("client.src.420_roi")}</Badge>
                    </div>
                    <div className="flex items-center justify-between p-2 hover:bg-muted/50 rounded-lg transition-colors cursor-pointer">
                        <div className="flex items-center gap-3">
                        ...
                        <Badge className="bg-success/10 text-success border-0 text-[9px] font-bold">{t("client.src.285_roi")}</Badge>
                    </div>
                </div>
                <Link to="/client/property">
                  <Button variant="ghost" className="w-full mt-4 text-[10px] font-black tracking-widest text-muted-foreground">{t("client.src.view_all_assets")}</Button>
                </Link>
            </div>
        </div>
      </div>
...
export function Payouts() {
  ...
  return <PageShell title={t("client.src.payouts")} description={t("client.src.commission_payments_vendor_payouts")} stats={[{
    ...
    value: payouts.length
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
        <Table>
          <TableHeader><TableRow><TableHead>{t("client.src.recipient")}</TableHead><TableHead>{t("client.src.method")}</TableHead><TableHead>{t("common.amount")}</TableHead><TableHead>{t("client.src.reference")}</TableHead><TableHead>{t("common.status")}</TableHead><TableHead className="w-10" /></TableRow></TableHeader>
          <TableBody>
