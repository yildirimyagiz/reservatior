    ...
    return (
      <PageShell title={t("checkout.no_plan", "No Plan Selected")} description="">
        <div className="flex flex-col items-center justify-center py-20 gap-6">
          ...
          <p className="text-muted-foreground text-sm">{t("checkout.select_plan_first", "Please select a plan first.")}</p>
          <Button onClick={() => navigate("/pricing")} className="bg-blue-600 hover:bg-brand/100">
          ...
          {steps.map((s, i) => (
            <div key={i} className="flex items-center flex-1">
              <div className="flex flex-col items-center gap-2">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center border-2 transition-all duration-500 ${i <= step ? "bg-blue-600 border-blue-500 text-white shadow-lg shadow-blue-600/30" : "bg-card border-border text-muted-foreground"}`}>
                  {i < step ? <Check className="w-5 h-5" /> : <s.icon className="w-5 h-5" />}
                </div>
                <span className={`text-[10px] font-bold tracking-widest uppercase ${i <= step ? "text-brand" : "text-muted-foreground"}`}>{s.label}</span>
              </div>
              {i < steps.length - 1 && (
                <div className="flex-1 mx-4 mb-6">
                  <div className={`h-0.5 rounded transition-all duration-500 ${i < step ? "bg-brand/100" : "bg-muted"}`} />
                </div>
              )}
            </div>
          ))}
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 0 && (
            <m.div key="summary" initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 30 }} className="grid md:grid-cols-2 gap-8">
              <div className="bg-card/60 backdrop-blur-xl border border-border p-8 rounded-3xl">
                ...
                {isPlanCheckout && (
                  <>
                    <div className="flex items-baseline gap-2 mb-8">
                      ...
                      {plan!.priceCents && <span className="text-muted-foreground text-xs font-bold tracking-wider">/ PER UNIT MONTHLY</span>}
                    </div>
                ...
                {!isPlanCheckout && (
                  <div className="bg-blue-600/5 border border-blue-600/20 p-6 rounded-xl">
                    <div className="text-muted-foreground text-sm mb-1">{t("client.src.total_synchronized_amount")}</div>
                    <div className="text-3xl font-black text-brand">{currency} {amount.toLocaleString()}</div>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col justify-between">
                <div className="space-y-4 mb-8">
                  <div className="bg-card/60 border border-border p-6 rounded-2xl">
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">Your account becomes active immediately after payment confirmation, giving you access to all features.</p>
                  </div>
                  <div className="bg-card/60 border border-border p-6 rounded-2xl">
                    <h4 className="font-bold flex items-center gap-2 mb-2 text-white"><ShieldCheck className="w-4 h-4 text-success" /> Secure Transaction</h4>
                    <p className="text-xs text-muted-foreground leading-relaxed">All payment information is protected with AES-256 encryption. Your card details are not stored on our servers.</p>
                  </div>
                </div>
                <Button onClick={() => setStep(1)} className="w-full h-14 bg-blue-600 hover:bg-brand/100 text-white font-bold text-sm tracking-wider rounded-2xl gap-2">
          ...
          {step === 1 && (
            <m.div key="contract" initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 30 }}>
              <div className="bg-card/60 backdrop-blur-xl border border-border p-8 rounded-3xl mb-6">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-lg font-black text-white flex items-center gap-3"><FileText className="w-6 h-6 text-brand" /> {t("checkout.service_agreement", "Service Agreement")}</h2>
                ...
                <div className="bg-[#0e0f11] border border-border rounded-2xl p-6 max-h-[400px] overflow-y-auto text-sm text-muted-foreground leading-relaxed space-y-4 scrollbar-thin scrollbar-thumb-slate-700">
              ...
              <div className="bg-card/60 border border-border p-6 rounded-2xl mb-6">
              ...
              <div className="flex gap-4">
                <Button onClick={() => setStep(0)} variant="outline" className="flex-1 h-14 rounded-2xl border-border text-muted-foreground hover:bg-muted font-bold text-sm gap-2">
                  <ChevronLeft className="w-4 h-4" /> {t("checkout.back", "Back")}
                </Button>
                <Button onClick={() => { if (contractAccepted) setStep(2); else toast({ title: "Contract Approval Required", description: "Please accept the contract to continue.", variant: "destructive" }); }} className={`flex-1 h-14 rounded-2xl font-bold text-sm gap-2 transition-all ${contractAccepted ? "bg-blue-600 hover:bg-brand/100 text-white" : "bg-muted text-muted-foreground cursor-not-allowed"}`}>
          ...
          {step === 2 && (
            <m.div key="payment" initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 30 }} className="grid md:grid-cols-2 gap-8">
              <div className="bg-card/60 backdrop-blur-xl border border-border p-8 rounded-3xl">
                <h2 className="text-lg font-black text-white flex items-center gap-3 mb-8"><CreditCard className="w-6 h-6 text-brand" /> {t("checkout.payment_info", "Payment Information")}</h2>

                <div className="bg-blue-600/5 border border-blue-600/20 p-6 rounded-xl flex justify-between items-center mb-8">
                  <div>
                    <div className="text-muted-foreground text-xs font-bold tracking-wider mb-1">TOTAL AMOUNT</div>
                    <div className="text-muted-foreground text-sm">{isPlanCheckout ? plan!.name : type.replace(/_/g, " ")}</div>
                  </div>
                  <div className="text-4xl font-black text-brand italic">{isPlanCheckout ? displayPrice : `${currency} ${amount}`}</div>
                </div>
                ...
                <Button onClick={handleCheckout} disabled={loading} className="w-full h-16 bg-blue-600 hover:bg-brand/100 text-white font-black text-sm tracking-widest rounded-2xl gap-3 shadow-xl shadow-blue-600/20 transition-all">
                  {loading ? (
              ...
              <div className="flex flex-col gap-4">
                <div className="bg-card/60 border border-border p-6 rounded-2xl">
                  <h4 className="font-bold flex items-center gap-2 mb-2 text-white"><Rocket className="w-4 h-4 text-brand" /> Next Steps</h4>
                  <ul className="space-y-2 text-xs text-muted-foreground">
                    <li className="flex items-start gap-2"><span className="text-brand font-bold">1.</span> Your account will be activated after successful payment</li>
                    <li className="flex items-start gap-2"><span className="text-brand font-bold">2.</span> You will be redirected to the dashboard</li>
                    <li className="flex items-start gap-2"><span className="text-brand font-bold">3.</span> You can start adding properties immediately</li>
                  </ul>
                </div>
                ...
                <div className="mt-auto flex items-start gap-4 p-4 bg-yellow-600/5 border border-yellow-600/20 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">You can cancel your subscription at any time. Your service will continue until the end of the current billing period after cancellation.</p>
                  </div>
                </div>

                <Button onClick={() => setStep(1)} variant="outline" className="h-12 rounded-2xl border-border text-muted-foreground hover:bg-muted font-bold text-xs gap-2">
