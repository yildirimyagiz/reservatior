...
export default function SolicitorManagement() {
  ...
  const getStatusBadge = (status: SolicitorRecord['assignments'][0]['status']) => {
    switch (status) {
      ...
      case 'completed':
        return <Badge className="bg-blue-100 text-blue-800 border-blue-200 font-bold"><CheckCircle2 className="w-3 h-3 mr-1" />{t("common.completed")}</Badge>;
      case 'engaged':
        return <Badge variant="outline" className="text-brand border-border bg-brand/10 font-bold"><Scale className="w-3 h-3 mr-1" />{t("client.src.engaged")}</Badge>;
      case 'instructed':
      ...
      case 'contract-exchange':
        return <Badge className="bg-brand/15 text-brand border-brand/30 font-bold">{t("client.src.exchange")}</Badge>;
      case 'on-hold':
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-muted/20 p-6 rounded-2xl border">
          ...
          <div className="flex gap-2 w-full md:w-auto">
             <Button className="bg-brand hover:bg-brand shadow-md shadow-indigo-100"><Plus className="w-4 h-4 mr-2" />{t("client.src.instruct_solicitor")}</Button>
          </div>
        </div>
                ...
                <p className="text-muted-foreground col-span-2">No solicitors found.</p>
                  ...
                  <CardHeader className="bg-muted/30 pb-4">
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-4">
                        <div className="w-14 h-14 bg-card rounded-xl shadow-sm border flex items-center justify-center">
                      ...
                      <Badge variant="secondary" className="bg-brand/10 text-brand hover:bg-brand/15 border-brand/30 font-bold">
                        {solicitor.type}
                      </Badge>
                    </div>
                  </CardHeader>
                  <CardContent className="pt-6 space-y-6 text-sm">
                    ...
                    <div className="flex flex-wrap gap-4 text-xs font-medium text-muted-foreground">
                       <span className="flex items-center"><Mail className="w-3.5 h-3.5 mr-1.5 text-brand" /> {solicitor.contact.email}</span>
                       <span className="flex items-center"><Phone className="w-3.5 h-3.5 mr-1.5 text-brand" /> {solicitor.contact.phone}</span>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        {solicitor.assignments.map(assign => <div key={assign.dealId} className="bg-white/50 border rounded-lg p-3 flex justify-between items-center group hover:bg-brand/50 transition-colors">
                            <div>
                              <p className="font-bold text-foreground mb-1 group-hover:text-brand transition-colors tracking-tighter">{assign.dealName}</p>
                            ...
                            <div className="text-right">
                              ...
                              <p className="text-xs font-bold mt-1 text-success flex items-center justify-end">
                    ...
                    <div className="flex gap-2 pt-2 border-t mt-4">
                      <Button variant="outline" size="sm" className="flex-1 font-bold">
                        <MessageSquare className="w-4 h-4 mr-2 text-brand" />{t("client.src.message")}</Button>
                      <Button variant="outline" size="sm" className="flex-1 font-bold">
                        <Plus className="w-4 h-4 mr-2 text-brand" />{t("client.src.new_task")}</Button>
                      <Button variant="ghost" size="sm" className="p-2" aria-label={t("common.open")}>
        ...
        <Card className="border-l-4 border-l-indigo-600 bg-card">
          <CardHeader className="pb-2">
             <CardTitle className="text-lg flex items-center font-bold">
               <Briefcase className="w-5 h-5 mr-3 text-brand" />{t("client.src.conveyancing_workflow_integration")}</CardTitle>
          </CardHeader>
