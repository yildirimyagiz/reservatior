...
export default function DealsManagement() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "PENDING":
        return <Badge className="bg-warning/10 text-orange-400 border-warning/20 text-[10px] font-black italic tracking-widest">{t("common.processing")}</Badge>;
      case "CLOSED":
      ...
      case "CANCELLED":
        return <Badge className="bg-red-500/10 text-red-400 border-red-500/20 text-[10px] font-black italic tracking-widest">{t("common.cancelled")}</Badge>;
      default:
        return <Badge className="bg-muted text-muted-foreground border-slate-500/20 text-[10px] font-black italic tracking-widest">{status}</Badge>;
    }
  };
  const stats = {
  ...
  return <PageShell title={t("client.src.deals_intelligence")} description={t("client.src.administrative_control_of_transaction")}>
      <div className="space-y-10 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <Card className="bg-background/40 border-white/5 rounded-3xl overflow-hidden shadow-2xl relative group">
            <CardContent className="p-8">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-black text-muted-foreground tracking-widest mb-1 italic">{t("client.src.pipeline_scale")}</p>
                  <h3 className="text-4xl font-black text-white italic tracking-tighter leading-none">{stats.totalDeals}</h3>
                </div>
                <Building className="w-10 h-10 text-muted-foreground opacity-20" />
              </div>
            </CardContent>
          </Card>

          <Card className="bg-background/40 border-white/5 rounded-3xl overflow-hidden shadow-2xl relative group border-l-blue-500/30 border-l-2">
            <CardContent className="p-8">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-black text-muted-foreground tracking-widest mb-1 italic">{t("client.src.gross_volume")}</p>
          ...
          <Card className="bg-background/40 border-white/5 rounded-3xl overflow-hidden shadow-2xl relative group border-l-violet-500/30 border-l-2">
            <CardContent className="p-8">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-black text-muted-foreground tracking-widest mb-1 italic">{t("client.src.active_nodes")}</p>
          ...
          <Card className="bg-background/40 border-white/5 rounded-3xl overflow-hidden shadow-2xl relative group border-l-orange-500/30 border-l-2">
            <CardContent className="p-8">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-black text-muted-foreground tracking-widest mb-1 italic">{t("client.src.conversions")}</p>
        ...
        {/* Search & Filter Bar */}
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 px-4 bg-background/40 p-6 rounded-3xl border border-white/5 shadow-2xl relative">
          ...
          <div className="flex gap-3">
            <Select value={filterType} onValueChange={setFilterType}>
              <SelectTrigger className="h-14 w-40 rounded-2xl bg-black/40 border-white/5 text-muted-foreground font-bold text-[10px] tracking-widest px-6">
                <SelectValue placeholder={t("common.type")} />
              </SelectTrigger>
              <SelectContent className="bg-background border-white/10 rounded-2xl text-white">
        ...
        {/* Data Table */}
        <Card className="bg-background/40 border-white/5 rounded-4xl overflow-hidden shadow-2xl relative border-l border-t">
              ...
              <Table>
                <TableHeader className="bg-white/5 border-b border-white/5">
                  <TableRow className="hover:bg-transparent border-none">
                    <TableHead className="text-[10px] font-black text-muted-foreground tracking-widest py-6 px-8">{t("client.src.transaction_profile")}</TableHead>
                    <TableHead className="text-[10px] font-black text-muted-foreground tracking-widest px-8">{t("common.location")}</TableHead>
                    <TableHead className="text-[10px] font-black text-muted-foreground tracking-widest px-8">{t("client.src.valuation")}</TableHead>
                    <TableHead className="text-[10px] font-black text-muted-foreground tracking-widest px-8">{t("client.src.sync_status")}</TableHead>
                    <TableHead className="text-[10px] font-black text-muted-foreground tracking-widest px-8 text-right">{t("client.src.intel")}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {loading ? <TableRow>
                      <TableCell colSpan={5} className="py-24 text-center">
                        ...
                        <p className="text-[10px] font-black text-muted-foreground tracking-widest italic animate-pulse">{t("client.src.synchronizing_neural_pipeline")}</p>
                      </TableCell>
                    </TableRow> : filteredDeals.length === 0 ? <TableRow>
                      <TableCell colSpan={5} className="py-24 text-center">
                        <p className="text-[10px] font-black text-muted-foreground tracking-widest italic">{t("client.src.zero_records_identified_in")}</p>
                      </TableCell>
                    ...
                    scale: 0.95
                          ...
                          <TableCell className="py-8 px-8">
                            <div className="flex items-center gap-6">
                              <div className="w-14 h-14 bg-black/40 border border-white/5 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-all font-black text-muted-foreground italic">
                          ...
                          <TableCell className="px-8">
                            <div className="space-y-1">
                              <p className="text-sm font-bold text-muted-foreground italic">{deal.address || 'Global Sector'}</p>
                              <div className="flex items-center gap-1 text-[9px] font-black text-muted-foreground italic">
                                <MapPin className="w-3 h-3 text-orange-500" /> {deal.city || 'Undeclared'}
                              </div>
                            </div>
                          </TableCell>
                          <TableCell className="px-8 font-black text-success italic text-lg">
                          ...
                          <TableCell className="px-8 text-right">
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <Button variant="ghost" className="h-10 w-10 rounded-xl hover:bg-white/10 text-muted-foreground" aria-label={t("common.more")}>
                                  <MoreHorizontal className="w-5 h-5" />
                                </Button>
                              </DropdownMenuTrigger>
                              <DropdownMenuContent align="end" className="bg-background border-white/10 rounded-2xl p-2 min-w-[180px] shadow-2xl">
      ...
      if (!open) {
        ...
        <DialogContent className="bg-background border-white/10 rounded-4xl text-white sm:max-w-xl p-0 overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)]">
          ...
          <div className="p-10 space-y-8">
            <div className="grid grid-cols-2 gap-8">
              <div className="col-span-2 space-y-3">
                <Label className="text-[10px] font-black text-muted-foreground tracking-widest italic ml-3">{t("client.src.project_designation")}</Label>
              ...
              <div className="space-y-3">
                <Label className="text-[10px] font-black text-muted-foreground tracking-widest italic ml-3">{t("client.src.sector_address")}</Label>
              ...
              <div className="space-y-3">
                <Label className="text-[10px] font-black text-muted-foreground tracking-widest italic ml-3">{t("client.src.neutral_city")}</Label>
              ...
              <div className="space-y-3">
                <Label className="text-[10px] font-black text-muted-foreground tracking-widest italic ml-3">{t("client.src.transaction_type")}</Label>
                ...
                dealType: val
                  ...
                  <SelectContent className="bg-background border-white/10 text-white rounded-2xl">
              ...
              <div className="space-y-3">
                <Label className="text-[10px] font-black text-muted-foreground tracking-widest italic ml-3">{t("client.src.pipeline_status")}</Label>
                ...
                status: val
                  ...
                  <SelectContent className="bg-background border-white/10 text-white rounded-2xl">
                    ...
                    <SelectItem value="ACTIVE" className="font-black italic">{t("common.active")}</SelectItem>
                    <SelectItem value="PENDING" className="font-black italic">{t("common.processing")}</SelectItem>
                    <SelectItem value="CLOSED" className="font-black italic">{t("client.src.closed")}</SelectItem>
                    <SelectItem value="CANCELLED" className="font-black italic text-red-500">{t("common.cancelled")}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-3">
                <Label className="text-[10px] font-black text-muted-foreground tracking-widest italic ml-3">{t("client.src.valuation_index")}</Label>
                <Input type="number" value={formData.listingPrice} onChange={e => setFormData({
                ...formData,
                listingPrice: Number(e.target.value)
              })} className="bg-black/40 border-white/5 rounded-2xl h-14 font-black px-6 text-success" />
              </div>
              <div className="space-y-3">
                <Label className="text-[10px] font-black text-muted-foreground tracking-widest italic ml-3">{t("client.src.conversion_price")}</Label>
