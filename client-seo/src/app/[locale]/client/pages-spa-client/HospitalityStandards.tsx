...
const TRUST_METRICS = [
  { value: "100%", label: "Guest Screening Rate", color: "text-success" },
  { value: "3+", label: "Minimum Night Stay", color: "text-amber-400" },
  { value: "24/7", label: "On-Site Concierge", color: "text-brand" },
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] dark:bg-[#0a0a0c] selection:bg-black selection:text-white dark:selection:bg-card dark:selection:text-black">
      ...
      <main className="max-w-[1400px] mx-auto px-8 lg:px-12 py-24 space-y-32">
        ...
        <section>
          ...
          >
            <div className="grid grid-cols-[1fr_120px_120px] md:grid-cols-[1fr_160px_160px] items-center px-8 py-5 bg-neutral-50 dark:bg-white/[0.02]">
              <span className="text-xs font-bold text-neutral-500 dark:text-muted-foreground tracking-widest uppercase">Feature</span>
              <span className="text-xs font-bold text-neutral-500 dark:text-muted-foreground tracking-widest uppercase text-center">Traditional</span>
              <span className="text-xs font-bold text-brand dark:text-brand tracking-widest uppercase text-center">Reservatior</span>
            </div>
            {COMPARISON.map((row, idx) => (
              <div key={idx} className={`grid grid-cols-[1fr_120px_120px] md:grid-cols-[1fr_160px_160px] items-center px-8 py-5 ${idx % 2 === 0 ? "bg-neutral-50/50 dark:bg-white/[0.01]" : ""} border-t border-neutral-200 dark:border-border hover:bg-neutral-50 dark:hover:bg-white/[0.03] transition-colors`}>
                <span className="text-sm font-semibold text-neutral-700 dark:text-muted-foreground tracking-wide">{row.feature}</span>
                ...
                <div className="flex justify-center">
                  <CheckCircle className="w-5 h-5 text-success shadow-[0_0_10px_#3b82f6]" />
                </div>
              </div>
            ))}
          </m.div>
        ...
        >
          <div className="bg-gradient-to-br from-brand/5 via-white/50 dark:via-[#14151a]/50 to-brand/5 rounded-[2rem] md:rounded-[3rem] p-10 md:p-16 border border-white/60 dark:border-border/60 relative">
            <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-brand/5 dark:bg-brand/5 blur-[120px] rounded-full pointer-events-none" />
            <div className="absolute bottom-0 left-0 w-[300px] h-[300px] bg-brand/5 dark:bg-brand/5 blur-[100px] rounded-full pointer-events-none" />
            ...
            <div className="relative z-10 grid lg:grid-cols-2 gap-12 items-center">
              <div className="space-y-8">
                ...
                <p className="text-neutral-500 dark:text-muted-foreground text-sm font-medium leading-relaxed max-w-lg">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="bg-white/50 dark:bg-background/50 p-6 rounded-2xl border border-white/60 dark:border-border/60 text-center group hover:bg-white/70 dark:hover:bg-background/70 transition-all">
                  <DollarSign className="w-8 h-8 text-brand mx-auto mb-4 group-hover:scale-110 transition-transform" />
                  <h4 className="text-2xl font-black text-neutral-900 dark:text-white tracking-tight mb-1">+40%</h4>
                  <p className="text-xs font-bold text-neutral-500 dark:text-muted-foreground tracking-widest uppercase">Revenue vs. Long-Term</p>
                </div>
                <div className="bg-white/50 dark:bg-background/50 p-6 rounded-2xl border border-white/60 dark:border-border/60 text-center group hover:bg-white/70 dark:hover:bg-background/70 transition-all">
                  <Star className="w-8 h-8 text-success mx-auto mb-4 group-hover:scale-110 transition-transform" />
                  <h4 className="text-2xl font-black text-neutral-900 dark:text-white tracking-tight mb-1">4.9★</h4>
                  <p className="text-xs font-bold text-neutral-500 dark:text-muted-foreground tracking-widest uppercase">Avg Guest Rating</p>
                </div>
                <div className="bg-white/50 dark:bg-background/50 p-6 rounded-2xl border border-white/60 dark:border-border/60 text-center group hover:bg-white/70 dark:hover:bg-background/70 transition-all">
                  <Shield className="w-8 h-8 text-brand mx-auto mb-4 group-hover:scale-110 transition-transform" />
                  <h4 className="text-2xl font-black text-neutral-900 dark:text-white tracking-tight mb-1">0</h4>
                  <p className="text-xs font-bold text-neutral-500 dark:text-muted-foreground tracking-widest uppercase">Security Incidents</p>
                </div>
                <div className="bg-white/50 dark:bg-background/50 p-6 rounded-2xl border border-white/60 dark:border-border/60 text-center group hover:bg-white/70 dark:hover:bg-background/70 transition-all">
                  <Users className="w-8 h-8 text-brand mx-auto mb-4 group-hover:scale-110 transition-transform" />
                  <h4 className="text-2xl font-black text-neutral-900 dark:text-white tracking-tight mb-1">98%</h4>
                  <p className="text-xs font-bold text-neutral-500 dark:text-muted-foreground tracking-widest uppercase">Resident Satisfaction</p>
                </div>
              </div>
            </div>
          </div>
