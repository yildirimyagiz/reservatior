    ...
    setCreateOpen(true);
      ...
      <div className="space-y-12">
        ...
        <div className="relative max-w-2xl mx-auto group">
           ...
           <input type="text" aria-label="Search events" placeholder={t("client.src.search_event_nodes")} className="w-full h-16 pl-16 pr-8 bg-card/60 border border-white/5 rounded-2xl text-[10px] font-black tracking-widest italic text-white placeholder:text-muted-foreground focus:outline-none focus:border-blue-500/50 transition-all shadow-xl" value={search} onChange={e => setSearch(e.target.value)} />
        </div>
            ...
            delay: idx * 0.1
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3">
                        <Badge className="bg-blue-600/10 text-brand border-none text-[8px] font-black italic tracking-widest px-3 py-1">
                          {row.type.replace("_", " ")}
                        </Badge>
                        <span className="text-[10px] font-black text-muted-foreground italic tracking-widest">{row.priority || "STANDARD_CLEARANCE"}</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-10 pt-8 border-t border-white/5 relative z-10">
                   <div className="space-y-1">
                     <p className="text-[8px] font-black text-muted-foreground italic flex items-center gap-1.5"><Clock className="w-2.5 h-2.5" />{t("client.src.startdate")}</p>
                     <p className="text-[11px] font-black text-white italic tracking-tight font-mono">{row.startDate}</p>
                   </div>
                   <div className="space-y-1">
                     <p className="text-[8px] font-black text-muted-foreground italic flex items-center gap-1.5"><MapPin className="w-2.5 h-2.5" />{t("client.src.geographicnode")}</p>
                     <p className="text-[11px] font-black text-white italic tracking-tight truncate">{row.location}</p>
                   </div>
                   <div className="space-y-1">
                     <p className="text-[8px] font-black text-muted-foreground italic flex items-center gap-1.5"><Users className="w-2.5 h-2.5" />{t("client.src.attendeecap")}</p>
                     <p className="text-[11px] font-black text-success italic tracking-tight">{row.attendeeCount}</p>
                   </div>
                   <div className="space-y-1">
                     <p className="text-[8px] font-black text-muted-foreground italic flex items-center gap-1.5"><Shield className="w-2.5 h-2.5" />{t("client.src.accesslvl")}</p>
                     <p className="text-[11px] font-black text-brand italic tracking-tight">{t("client.src.lvl3")}</p>
                   </div>
                </div>

                <div className="flex gap-4 relative z-10">
                   <Button className="flex-2 h-14 rounded-2xl bg-card text-black hover:bg-muted font-black italic tracking-widest text-[10px] transition-all" onClick={() => openEdit(row)}>{t("client.src.recalibrate_frequency")}</Button>
                   <Button variant="outline" className="h-14 w-14 rounded-2xl border-white/10 bg-white/5 text-muted-foreground hover:text-red-500 hover:bg-red-500/10 transition-all shrink-0" onClick={handleDelete} aria-label={t("common.delete")}>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-2xl bg-background border-white/10 text-white rounded-[32px] p-10 font-display">
           <DialogHeader>
             ...
             <DialogDescription className="text-[10px] font-black text-muted-foreground tracking-widest italic">{t("client.src.registering_bidirectional_event_node")}</DialogDescription>
           </DialogHeader>
           <form onSubmit={handleCreate} className="space-y-8 py-8">
             <div className="space-y-3">
                <Label className="text-[10px] font-black text-muted-foreground italic">{t("client.src.hubidentifier")}</Label>
                <Input value={form.title} onChange={e => setForm({
              ...form,
              title: e.target.value
            })} className="h-14 bg-black/40 border-white/5 rounded-2xl text-[10px] font-black italic text-white placeholder:text-foreground" placeholder={t("client.src.eg_downtownexposure01")} />
             </div>
             <div className="grid grid-cols-2 gap-8">
                <div className="space-y-3">
                   <Label className="text-[10px] font-black text-muted-foreground italic">{t("client.src.startuplink")}</Label>
                ...
                <div className="space-y-3">
                   <Label className="text-[10px] font-black text-muted-foreground italic">{t("client.src.endsyndication")}</Label>
             ...
             <div className="space-y-3">
                <Label className="text-[10px] font-black text-muted-foreground italic">{t("client.src.geographicvector")}</Label>
             ...
             <div className="space-y-3">
                <Label className="text-[10px] font-black text-muted-foreground italic">{t("client.src.directivesummary")}</Label>
             ...
             <DialogFooter className="gap-6 pt-6 border-t border-white/5">
                <Button type="button" variant="ghost" onClick={() => setCreateOpen(false)} className="text-[10px] font-black italic text-muted-foreground hover:text-white">{t("client.src.abort_operation")}</Button>
                <Button type="submit" className="h-16 px-12 rounded-2xl bg-blue-600 hover:bg-brand/100 text-white font-black text-[11px] italic tracking-[0.2em] shadow-2xl shadow-blue-600/30">{t("client.src.materialize_hud")}</Button>
             </DialogFooter>
           </form>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="max-w-2xl bg-background border-white/10 text-white rounded-[32px] p-10 font-display">
           ...
           {form && <form onSubmit={handleEdit} className="space-y-8 py-8">
              ...
              title: e.target.value
               ...
               <div className="grid grid-cols-2 gap-8">
                 <div className="space-y-3">
                    <Label className="text-[10px] font-black text-muted-foreground italic">{t("client.src.signalstate")}</Label>
                ...
                status: v
                      ...
                      <SelectContent className="bg-card border-white/10 font-display">
                         {Object.keys(STATUS_CONFIG).map(s => <SelectItem key={s} value={s} className="text-muted-foreground font-bold italic">{STATUS_CONFIG[s].label}</SelectItem>)}
                      </SelectContent>
                    </Select>
                 </div>
                 <div className="space-y-3">
                    <Label className="text-[10px] font-black text-muted-foreground italic">{t("client.src.hubtype")}</Label>
                ...
                type: v
                      ...
                      <SelectContent className="bg-card border-white/10 font-display">
                         {["OPEN_HOUSE", "LAUNCH", "TRAINING", "MEETING", "OTHER"].map(t => <SelectItem key={t} value={t} className="text-muted-foreground font-bold italic">{t.replace("_", " ")}</SelectItem>)}
                      </SelectContent>
                    </Select>
                 </div>
               </div>
