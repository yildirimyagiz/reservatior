...
export default function ReservationTracking({ propertyId }: { propertyId?: string }) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'confirmed':
        return 'bg-blue-600/10 text-brand border-blue-600/20';
      case 'cancelled':
        return 'bg-rose-600/10 text-rose-500 border-rose-600/20';
      case 'completed':
        return 'bg-blue-600/10 text-success border-blue-600/20';
      case 'no_show':
        return 'bg-muted text-muted-foreground border-slate-600/20';
      default:
        return 'bg-muted text-muted-foreground border-slate-600/20';
    }
  };
  ...
  return <div className="min-h-screen bg-background p-8 lg:p-12 relative overflow-hidden">
      ...
      <div className="max-w-[1600px] mx-auto space-y-12 relative z-10">
        ...
        y: 0
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-muted-foreground text-sm font-black tracking-widest italic">{t('client.property.reservationTracking.subtitle')}</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
             <Button variant="ghost" onClick={() => refetchReservations()} disabled={isLoadingReservations} className="h-16 w-16 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 text-muted-foreground hover:text-white transition-all shadow-xl" aria-label={t("common.refresh")}>
        ...
        {analytics && <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          delay: 0.1
              ...
              <Card className="bg-card/60 border-white/5 border-l border-t rounded-[32px] p-8 backdrop-blur-3xl shadow-3xl group relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-4">
                   <p className="text-[10px] font-black tracking-widest text-muted-foreground italic">{t('total')}</p>
                   <p className="text-4xl font-black text-white italic tracking-tighter leading-none">{analytics.total}</p>
                   <p className="text-[10px] font-black text-success italic tracking-widest">{t('thisMonth', {
          ...
          delay: 0.2
              ...
              <Card className="bg-card/60 border-white/5 border-l border-t rounded-[32px] p-8 backdrop-blur-3xl shadow-3xl group relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-4">
                   <p className="text-[10px] font-black tracking-widest text-muted-foreground italic">{t('client.property.reservationTracking.revenue')}</p>
                   ...
                   <p className="text-[10px] font-black text-brand italic tracking-widest">{t('client.property.reservationTracking.target', {
          ...
          delay: 0.3
              ...
              <Card className="bg-card/60 border-white/5 border-l border-t rounded-[32px] p-8 backdrop-blur-3xl shadow-3xl group relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-4">
                   <p className="text-[10px] font-black tracking-widest text-muted-foreground italic">{t('client.property.reservationTracking.occupancy')}</p>
                   <div className="flex items-baseline gap-4">
                      ...
                      {analytics.occupancy.trend === 'up' ? <TrendingUp className="w-4 h-4 text-success" /> : <TrendingDown className="w-4 h-4 text-rose-500" />}
                   </div>
                   <p className="text-[10px] font-black text-muted-foreground italic tracking-widest">{t('occupancyTarget', {
          ...
          delay: 0.4
              ...
              <Card className="bg-card/60 border-white/5 border-l border-t rounded-[32px] p-8 backdrop-blur-3xl shadow-3xl group relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-4">
                   <p className="text-[10px] font-black tracking-widest text-muted-foreground italic">{t('satisfaction')}</p>
                   ...
                   <div className="flex items-center gap-2">
                      ...
                      <p className="text-[10px] font-black text-muted-foreground italic tracking-widest">{t('avgRating', {
        ...
        }].map(s => <Card key={s.status} className="cursor-pointer bg-card/40 border-white/5 border-l border-t rounded-3xl p-6 hover:bg-white/5 transition-all shadow-xl group border-none" onClick={() => setFilter({
          ...filter,
          status: s.status
        })}>
               <div className="flex justify-between items-start">
                  <div className="space-y-2">
                     ...
                     <p className="text-[9px] font-black text-muted-foreground tracking-widest italic group-hover:text-white/40 transition-colors">{s.label}</p>
                  </div>
        ...
        scale: 1
          ...
          <Card className="bg-card/40 border-white/5 border-l border-t rounded-[40px] p-8 backdrop-blur-3xl shadow-3xl flex flex-col lg:flex-row lg:items-center justify-between gap-8">
            <div className="flex-1 max-w-2xl relative group">
               ...
               <input placeholder={t('client.property.reservationTracking.controls.search')} aria-label="Search reservations" className="w-full pl-16 h-16 bg-black/40 border border-white/5 rounded-[24px] text-white font-black italic text-xs tracking-widest focus:outline-none focus:ring-2 focus:ring-blue-600/50 transition-all placeholder:text-foreground" value={filter.search || ''} onChange={e => setFilter({
              ...filter,
            ...
            <div className="flex flex-wrap items-center gap-6">
                <select aria-label="Filter by status" className="h-16 px-8 rounded-2xl bg-black/40 border-white/5 text-muted-foreground font-black italic text-[10px] tracking-widest focus:ring-2 focus:ring-blue-600/50 outline-none" value={filter.status || ''} onChange={e => setFilter({
              ...filter,
              status: e.target.value || undefined
                ...
                <div className="h-16 bg-black/40 p-1.5 rounded-[20px] border border-white/5 flex gap-1 shadow-inner group">
                   <Button variant="ghost" className={cn("h-full px-6 rounded-xl font-black  italic text-[9px] tracking-widest transition-all", viewMode === 'list' ? "bg-card text-black shadow-xl" : "text-muted-foreground hover:text-white")} onClick={() => setViewMode('list')}><LayoutGrid className="w-4 h-4 mr-2" /> {t('client.property.reservationTracking.controls.list')}</Button>
                   <Button variant="ghost" className={cn("h-full px-6 rounded-xl font-black  italic text-[9px] tracking-widest transition-all", viewMode === 'grid' ? "bg-card text-black shadow-xl" : "text-muted-foreground hover:text-white")} onClick={() => setViewMode('grid')}><Activity className="w-4 h-4 mr-2" /> {t('client.property.reservationTracking.controls.grid')}</Button>
                   <Button variant="ghost" className={cn("h-full px-6 rounded-xl font-black  italic text-[9px] tracking-widest transition-all", viewMode === 'kanban' ? "bg-card text-black shadow-xl" : "text-muted-foreground hover:text-white")} onClick={() => setViewMode('kanban')}><Target className="w-4 h-4 mr-2" /> {t('kanban')}</Button>
                </div>
            </div>
          </Card>
        </m.div>
        ...
        <AnimatePresence mode="wait">
                  ...
                  count: filteredReservations.length
                   ...
                   <div className="divide-y divide-white/5">
                      {filteredReservations.map((res: any) => <div key={res.id} onClick={() => setSelectedReservation(res)} className="p-8 flex items-center justify-between hover:bg-white/2 transition-all group cursor-pointer relative">
                            <div className="flex items-center gap-8">
                               <div className="w-16 h-16 rounded-[24px] bg-white/5 border border-white/5 flex items-center justify-center group-hover:scale-110 transition-transform duration-500"><Building className="w-6 h-6 text-brand" /></div>
                               <div className="space-y-1.5">
                                  <p className="text-xl font-black text-white italic tracking-tighter leading-none group-hover:text-brand transition-colors">{res.property.name}</p>
                                  <div className="flex items-center gap-3 text-[10px] font-black text-muted-foreground tracking-widest italic">
                          ...
                          count: res.dates.nights
                            ...
                            <div className="flex items-center gap-12">
                               <div className="text-right hidden md:block">
                                  ...
                                  <p className="text-[9px] font-black text-muted-foreground tracking-widest italic mt-1.5">{t('checkIn')}</p>
                               </div>
                               ...
                               <Button variant="ghost" className="h-14 w-14 rounded-2xl bg-white/5 border border-white/5 hover:bg-brand hover:text-white transition-all" aria-label={t("common.next")}><ChevronRight className="w-5 h-5" /></Button>
                            </div>
                         </div>)}
                   </div>
                </Card>
          ...
          scale: 1.05
                ...
                {filteredReservations.map((res: any) => <Card key={res.id} onClick={() => setSelectedReservation(res)} className="bg-card/60 border-white/5 border-l border-t rounded-[40px] overflow-hidden hover:bg-white/5 transition-all shadow-3xl cursor-pointer group">
                     ...
                     <div className="p-10 space-y-6">
                        <div>
                           <h3 className="text-2xl font-black text-white italic tracking-tighter leading-none group-hover:text-brand transition-colors truncate">{res.property.name}</h3>
                           <p className="text-[10px] font-black text-muted-foreground tracking-widest italic mt-3 flex items-center gap-2"><MapPin className="w-3.5 h-3.5" />{t("client.src.hudoperationalunit")}</p>
                        </div>
                        <div className="h-px bg-white/5 w-full" />
                        <div className="grid grid-cols-2 gap-6">
                           <div>
                              <p className="text-[8px] font-black text-muted-foreground tracking-widest mb-1 italic">{t("client.src.checkin")}</p>
                              <p className="text-sm font-black text-white italic tracking-tighter leading-none">{res.dates.checkIn.toISOString().split('T')[0]}</p>
                           </div>
                           <div className="text-right">
                              <p className="text-[8px] font-black text-muted-foreground tracking-widest mb-1 italic">{t("client.src.totalunit")}</p>
                              <p className="text-sm font-black text-success italic tracking-tighter leading-none">{formatCurrency(res.pricing.totalAmount, res.pricing.currency)}</p>
                           </div>
                        </div>
                        <Button variant="ghost" className="w-full h-16 rounded-[24px] bg-white/5 border border-white/5 hover:bg-brand text-white font-black italic text-[10px] tracking-widest transition-all shadow-xl group/btn">
                          {t('viewDetails')}
          ...
          y: -20
                ...
                scale: 1.02
              }} className="bg-card border-white/5 border-l border-t rounded-[24px] p-6 shadow-xl cursor-grab active:cursor-grabbing group">
                             <p className="text-sm font-black text-white italic tracking-tighter group-hover:text-brand transition-colors truncate mb-3">{res.property.name}</p>
                             <div className="flex justify-between items-center text-[9px] font-black text-muted-foreground tracking-widest italic">
                                <span>{res.guest.name}</span>
                                <span className="text-success">{formatCurrency(res.pricing.totalAmount, res.pricing.currency)}</span>
                             </div>
                          </m.div>)}
                     </div>
                  </div>)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
           <Card className="bg-card/40 border-white/5 border-l border-t rounded-[40px] p-10 backdrop-blur-3xl shadow-3xl">
              <div className="flex items-center justify-between mb-10">
                 <h3 className="text-xl font-black text-white italic tracking-tighter flex items-center gap-4"><LineChart className="w-6 h-6 text-brand" /> {t('trends')}</h3>
              ...
              <div className="h-[250px] flex items-end gap-3 pb-8">
                ...
                duration: 1.5,
                delay: i * 0.05
              }} className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-brand to-blue-400 rounded-full group-hover:shadow-[0_0_20px_rgba(37,99,235,0.4)] transition-all" />
                    <div className="absolute -bottom-8 left-0 right-0 text-[8px] font-black text-muted-foreground text-center tracking-widest group-hover:text-brand transition-colors">{t("client.src.e")}{i + 1}</div>
                  </div>)}
              </div>
           </Card>
           ...
           <Card className="bg-card/40 border-white/5 border-l border-t rounded-[40px] p-10 backdrop-blur-3xl shadow-3xl">
              <div className="flex items-center justify-between mb-10">
                 <h3 className="text-xl font-black text-white italic tracking-tighter flex items-center gap-4"><PieChart className="w-6 h-6 text-success" /> {t('client.property.reservationTracking.charts.distribution')}</h3>
                 <div className="flex gap-2">
                    <div className="w-3 h-3 rounded-full bg-brand/100 shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
      ...
      <Dialog open={!!selectedReservation} onOpenChange={() => setSelectedReservation(null)}>
        <DialogContent className="max-w-[1000px] p-0 border-none bg-transparent overflow-hidden shadow-none">
          ...
          opacity: 1,
          scale: 1
        }} className="bg-card border border-white/10 rounded-[40px] overflow-hidden backdrop-blur-3xl shadow-4xl flex flex-col md:flex-row min-h-[700px]">
                ...
                <div className="w-full md:w-[400px] bg-black/40 border-r border-white/10 p-10 flex flex-col justify-between relative overflow-hidden">
                   ...
                   <div className="relative z-10 space-y-10">
                      ...
                      <div className="space-y-2">
                         <p className="text-[10px] font-black text-muted-foreground tracking-[0.2em] italic">{t("client.src.nodecoreidentifier")}</p>
                    ...
                    code: selectedReservation.bookingCode
                         ...
                         <p className="text-muted-foreground font-black italic text-[11px] tracking-widest pt-4 opacity-60">{t("client.src.hudmanifestoperational")}</p>
                      </div>
                   ...
                   <div className="relative z-10 grid grid-cols-1 gap-4">
                      ...
                      <Button className="h-16 rounded-2xl bg-blue-600 hover:bg-brand/100 text-white font-black italic text-[10px] tracking-widest transition-all shadow-2xl shadow-blue-600/30">{t('manage')}</Button>
                   </div>
                </div>
                ...
                <div className="flex-1 p-10 space-y-10 overflow-y-auto max-h-[80vh] custom-scrollbar">
                   ...
                   <div className="space-y-6">
                      <div className="flex items-center justify-between">
                         <h3 className="text-xs font-black text-white italic tracking-[0.2em] flex items-center gap-3"><Building className="w-4 h-4 text-brand" /> {t('propertyDetails')}</h3>
                         <span className="text-[9px] font-black text-muted-foreground">{t("client.src.0xcf42")}</span>
                      </div>
                      <Card className="bg-black/20 border-white/5 rounded-3xl p-6 hover:bg-white/5 transition-all group">
                         <div className="flex gap-8">
                            ...
                            <div className="flex-1 space-y-4 pt-2">
                               <h4 className="text-2xl font-black text-white italic tracking-tighter leading-none group-hover:text-brand transition-colors">{selectedReservation.property.name}</h4>
                               <p className="text-[10px] font-black text-muted-foreground tracking-widest italic flex items-center gap-2"><MapPin className="w-3.5 h-3.5" /> {selectedReservation.property.address}</p>
                               <div className="flex gap-6 pt-4 border-t border-white/5">
                                  <div className="flex items-center gap-2 text-[10px] font-black text-white italic tracking-widest"><Bed className="w-3.5 h-3.5 text-brand" /> {selectedReservation.property.bedrooms}</div>
                                  <div className="flex items-center gap-2 text-[10px] font-black text-white italic tracking-widest"><Bath className="w-3.5 h-3.5 text-success" /> {selectedReservation.property.bathrooms}</div>
                   ...
                   {/* Pricing HUD */}
                   <div className="space-y-6">
                      <h3 className="text-xs font-black text-white italic tracking-[0.2em] flex items-center gap-3"><DollarSign className="w-4 h-4 text-success" /> {t('pricingAnalysis')}</h3>
                      <Card className="bg-black/20 border-white/5 rounded-3xl p-8 space-y-4 shadow-inner relative overflow-hidden group">
                         ...
                         <div className="flex justify-between items-center text-[11px] font-black text-muted-foreground tracking-widest italic">
                            <span>{t('nightlyRate', {
                      count: selectedReservation.dates.nights
                         ...
                         <div className="flex justify-between items-center text-[11px] font-black text-muted-foreground tracking-widest italic pt-2">
                         ...
                         <div className="flex justify-between items-center text-[11px] font-black text-muted-foreground tracking-widest italic pt-2 pb-6">
                            <span>{t('taxes')}</span>
                         ...
                         <div className="flex justify-between items-center pt-4 relative z-10">
                            <span className="text-[10px] font-black text-success tracking-[0.2em] italic">{t('client.property.reservationTracking.dialog.totalPaid')}</span>
                            <span className="text-4xl font-black text-success italic tracking-tighter leading-none shadow-[0_0_20px_rgba(52,211,153,0.2)]">{formatCurrency(selectedReservation.pricing.totalAmount, selectedReservation.pricing.currency)}</span>
                         </div>
                      </Card>
                   </div>
                </div>
