...
export default function PropertyManagement() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "AVAILABLE":
        return <Badge className="bg-success/10 text-success border-success/20 px-3 text-[8px] font-black tracking-widest h-6 rounded-full">{t('client.property.management.status.forSale')}</Badge>;
      case "RESERVED":
      ...
      default:
        return <Badge className="bg-muted text-muted-foreground border-slate-500/20 px-3 text-[8px] font-black tracking-widest h-6 rounded-full">{status}</Badge>;
    }
  };
      ...
      <div className="max-w-[1600px] mx-auto space-y-12 relative z-10">
        ...
        y: 0
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-muted-foreground text-sm font-black tracking-widest italic">{t('client.property.management.subtitle')}</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
             <Button variant="ghost" className="h-16 px-8 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 text-muted-foreground font-black italic text-xs tracking-widest transition-all">
               <Cpu className="w-5 h-5 mr-3 text-brand" /> {t('getAIReport')}
             </Button>
             <Button onClick={() => setShowCreateDialog(true)} className="h-16 px-10 rounded-2xl bg-blue-600 hover:bg-brand/100 text-white font-black italic text-xs tracking-widest shadow-xl shadow-blue-600/20 transition-all hover:scale-105 active:scale-95">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          delay: i * 0.1
               ...
               <Card className="bg-card/40 border border-white/5 rounded-[32px] overflow-hidden p-8 hover:bg-card/80 transition-all duration-500 shadow-2xl relative backdrop-blur-xl group hover:shadow-[0_0_40px_rgba(59,130,246,0.15)] hover:-translate-y-2 hover:border-blue-500/30">
                 <div className="absolute top-0 right-0 p-6 opacity-[0.03] group-hover:opacity-10 group-hover:scale-125 group-hover:rotate-12 transition-all duration-700 group-hover:text-brand">
                    <s.icon className="w-32 h-32 -mt-8 -mr-8" />
                 </div>
                 <div className="flex justify-between items-start relative z-10">
                   <div>
                      <p className="text-[10px] font-black tracking-widest text-muted-foreground mb-2 italic group-hover:text-muted-foreground transition-colors">{s.label}</p>
          ...
          <Card className="bg-card/40 border-white/5 border-l border-t rounded-[40px] p-8 backdrop-blur-3xl shadow-3xl">
            <div className="flex flex-col md:flex-row items-center gap-8">
               <div className="flex-1 w-full relative group">
                  ...
                  <input placeholder={t('client.property.management.search')} aria-label="Search properties" className="w-full pl-16 h-16 bg-black/40 border border-white/5 rounded-[24px] text-white font-black italic text-xs tracking-widest focus:outline-none focus:ring-2 focus:ring-blue-600/50 transition-all placeholder:text-foreground" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
               </div>
               <div className="flex items-center gap-4">
                  <Select value={filterStatus} onValueChange={setFilterStatus}>
                     <SelectTrigger className="w-64 h-16 bg-black/40 border-white/5 rounded-[24px] text-white font-black italic text-[10px] tracking-widest px-6"><SelectValue placeholder={t("common.status")} /></SelectTrigger>
                  ...
                  <div className="flex bg-black/40 rounded-[24px] p-1.5 border border-white/5 h-16 shadow-inner">
                    <Button variant="ghost" onClick={() => setViewMode("table")} className={cn("h-full px-6 rounded-xl font-black  italic text-[10px] tracking-widest transition-all", viewMode === "table" ? "bg-card text-black shadow-xl" : "text-muted-foreground hover:text-white")}><List className="w-4 h-4 mr-2" />{t("client.src.list")}</Button>
                    <Button variant="ghost" onClick={() => setViewMode("grid")} className={cn("h-full px-6 rounded-xl font-black  italic text-[10px] tracking-widest transition-all", viewMode === "grid" ? "bg-card text-black shadow-xl" : "text-muted-foreground hover:text-white")}><Grid3X3 className="w-4 h-4 mr-2" />{t("client.src.grid")}</Button>
                  </div>
               </div>
            </div>
          </Card>
        ...
        <AnimatePresence mode="wait">
          {loading ? <div className="py-32 flex flex-col items-center justify-center gap-6">
              ...
              <p className="text-[10px] font-black tracking-[0.3em] italic text-muted-foreground">{t('client.property.management.syncing')}</p>
                     ...
                     <Card className="group relative bg-card/40 border border-white/5 rounded-[40px] overflow-hidden backdrop-blur-xl transition-all duration-700 hover:shadow-[0_0_50px_rgba(59,130,246,0.15)] hover:border-blue-500/40 h-full flex flex-col hover:-translate-y-3">
                        <div className="aspect-[4/3] relative overflow-hidden">
                          ...
                          <div className="absolute bottom-6 left-6 right-6 flex items-end justify-between z-20">
                              <div className="space-y-1 transform group-hover:-translate-y-1 transition-transform duration-500">
                                 <p className="text-white font-black text-3xl italic tracking-tighter leading-none group-hover:text-brand transition-colors drop-shadow-lg">{p.listingPrice ? "$" + parseInt(p.listingPrice.toString()).toLocaleString('en-US') : '--'}</p>
                                 <span className="text-[10px] font-black text-blue-300 tracking-widest italic drop-shadow-md">{p.type.replace('_', ' ')}</span>
                              </div>
                              <Button size="icon" aria-label={t("common.video_call")} className="h-14 w-14 rounded-[24px] bg-white/10 backdrop-blur-2xl border border-white/20 hover:bg-brand hover:border-blue-500 transition-all duration-500 text-white hover:scale-110 active:scale-90 shadow-2xl group-hover:shadow-blue-500/20">
                        ...
                        <CardContent className="p-8 space-y-6 flex-1 flex flex-col">
                          <div className="space-y-1.5">
                            ...
                            <p className="text-muted-foreground text-[10px] font-black italic tracking-widest flex items-center gap-2"><MapPin className="w-3.5 h-3.5 text-brand" /> {p.city}, {p.country}</p>
                          </div>

                          <div className="grid grid-cols-3 gap-4 py-4 border-y border-white/5">
                             <div className="text-center group/metric">
                                <Bed className="w-4 h-4 mx-auto mb-1.5 text-muted-foreground group-hover/metric:text-brand transition-colors" />
                                <span className="text-sm font-black text-muted-foreground group-hover/metric:text-white transition-colors italic">{p.bedrooms || 0}</span>
                             </div>
                             <div className="text-center group/metric">
                                <Bath className="w-4 h-4 mx-auto mb-1.5 text-muted-foreground group-hover/metric:text-brand transition-colors" />
                                <span className="text-sm font-black text-muted-foreground group-hover/metric:text-white transition-colors italic">{p.bathrooms || 0}</span>
                             </div>
                             <div className="text-center group/metric">
                                <Maximize2 className="w-4 h-4 mx-auto mb-1.5 text-muted-foreground group-hover/metric:text-brand transition-colors" />
                                <span className="text-[11px] font-black text-muted-foreground group-hover/metric:text-white transition-colors italic">{p.areaSqm || 0}{t("client.src.m")}</span>
                             </div>
                          </div>

                          <div className="grid grid-cols-2 gap-4 pt-4 mt-auto relative z-20">
                             <Button onClick={() => handleEditProperty(p)} variant="ghost" className="bg-white/5 border border-white/5 text-[10px] font-black italic tracking-widest rounded-[20px] h-12 hover:bg-white/10 text-muted-foreground hover:text-white transition-all duration-300">
                                {t('edit')}
                             </Button>
                             <Button className="bg-blue-600/10 text-brand hover:bg-brand hover:text-white border border-blue-600/20 text-[10px] font-black italic tracking-widest rounded-[20px] h-12 transition-all duration-300 group/btn">
          ...
          opacity: 1
        }} className="bg-card/40 border border-white/5 border-l border-t rounded-[48px] overflow-hidden backdrop-blur-3xl shadow-3xl">
               <Table>
                  <TableHeader className="bg-white/2 border-b border-white/5">
                    <TableRow className="hover:bg-transparent border-none">
                      <TableHead className="text-muted-foreground text-[10px] font-black tracking-[0.2em] px-10 py-8 italic">{t('client.property.management.detail')}</TableHead>
                      <TableHead className="text-muted-foreground text-[10px] font-black tracking-[0.2em] px-10 italic">{t('client.property.management.category')}</TableHead>
                      <TableHead className="text-muted-foreground text-[10px] font-black tracking-[0.2em] px-10 italic">{t('client.property.management.status')}</TableHead>
                      <TableHead className="text-muted-foreground text-[10px] font-black tracking-[0.2em] px-10 italic">{t('price')}</TableHead>
                      <TableHead className="text-muted-foreground text-[10px] font-black tracking-[0.2em] px-10 italic">{t('client.property.management.metrics')}</TableHead>
                      <TableHead className="text-muted-foreground text-[10px] font-black tracking-[0.2em] px-10 text-right italic"></TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                     {filteredProperties.map(p => <TableRow key={p.id} className="border-b border-white/5 hover:bg-white/2 transition-all group/row">
                          <TableCell className="px-10 py-8">
                             <div className="flex items-center gap-8">
                               ...
                               <div className="space-y-1.5">
                                 <p className="font-black text-white text-xl italic tracking-tighter leading-none group-hover:text-brand transition-colors">{p.name}</p>
                                 <p className="text-muted-foreground text-[10px] font-black italic tracking-widest flex items-center gap-2"><MapPin className="w-3.5 h-3.5 text-brand" /> {p.city}, {p.country}</p>
                               </div>
                             </div>
                          </TableCell>
                          <TableCell className="px-10">
                            <Badge className="bg-blue-600/10 text-brand border border-blue-600/20 text-[8px] font-black tracking-widest px-4 h-7 rounded-full italic">{p.propertyCategory}</Badge>
                          </TableCell>
                          <TableCell className="px-10">{getStatusBadge(p.listingStatus)}</TableCell>
                          <TableCell className="px-10">
                             <p className="text-[9px] font-black text-muted-foreground tracking-widest italic leading-none mb-1">{t("client.src.totalvalue")}</p>
                          ...
                          <TableCell className="px-10">
                             <div className="flex items-center gap-6">
                               <div className="space-y-1 text-center group/metric">
                                 <Bed className="w-3.5 h-3.5 text-muted-foreground group-hover/row:text-brand transition-colors mx-auto" />
                                 <p className="text-xs font-black text-muted-foreground italic leading-none">{p.bedrooms || 0}</p>
                               </div>
                               <div className="space-y-1 text-center group/metric">
                                 <Bath className="w-3.5 h-3.5 text-muted-foreground group-hover/row:text-brand transition-colors mx-auto" />
                                 <p className="text-xs font-black text-muted-foreground italic leading-none">{p.bathrooms || 0}</p>
                               </div>
                               <div className="space-y-1 text-center group/metric">
                                 <Maximize2 className="w-3.5 h-3.5 text-muted-foreground group-hover/row:text-brand transition-colors mx-auto" />
                                 <p className="text-[10px] font-black text-muted-foreground italic leading-none">{p.areaSqm || 0}{t("client.src.m")}</p>
                               </div>
                             </div>
                          </TableCell>
                          <TableCell className="px-10 text-right">
                             <DropdownMenu>
                               ...
                               <DropdownMenuContent align="end" className="bg-card border-white/10 text-white w-64 p-3 rounded-2xl shadow-3xl backdrop-blur-3xl">
                                 <DropdownMenuLabel className="text-[10px] font-black text-muted-foreground tracking-widest p-4 pb-2 italic">{t("client.src.assetcontrol")}</DropdownMenuLabel>
                                 <DropdownMenuSeparator className="bg-white/5 mx-2" />
                                 <DropdownMenuItem onClick={() => handleEditProperty(p)} className="rounded-xl h-12 gap-4 font-black italic text-[10px] tracking-widest focus:bg-white/5 focus:text-brand cursor-pointer"><Edit className="w-4 h-4" /> {t('edit')}</DropdownMenuItem>
                                 <DropdownMenuItem className="rounded-xl h-12 gap-4 font-black italic text-[10px] tracking-widest focus:bg-white/5 focus:text-success cursor-pointer"><Sparkles className="w-4 h-4" /> {t('aiEnhance')}</DropdownMenuItem>
                                 <DropdownMenuItem className="rounded-xl h-12 gap-4 font-black italic text-[10px] tracking-widest focus:bg-white/5 focus:text-brand cursor-pointer"><Video className="w-4 h-4" /> {t('client.property.management.studio')}</DropdownMenuItem>
                                 <DropdownMenuSeparator className="bg-white/5 mx-2" />
        ...
        if (!open) {
          ...
          <DialogContent className="max-w-4xl bg-card border-white/10 text-white rounded-[48px] p-0 overflow-hidden shadow-3xl backdrop-blur-3xl">
            <div className="bg-gradient-to-r from-brand/10 to-brand/10 p-12 border-b border-white/5 flex items-center justify-between">
              <div>
                <DialogTitle className="text-4xl font-black italic tracking-tighter text-brand">{showEditDialog ? t('client.property.management.dialog.update') : t('client.property.management.dialog.create')}</DialogTitle>
                <DialogDescription className="text-muted-foreground font-black italic tracking-widest text-[10px] pt-4 leading-relaxed">{t('client.property.management.dialog.desc')}</DialogDescription>
              </div>
            ...
            <div className="p-12 grid grid-cols-1 md:grid-cols-2 gap-12 max-h-[60vh] overflow-y-auto custom-scrollbar">
               <div className="space-y-8">
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black tracking-widest text-muted-foreground italic ml-2">{t('client.property.management.dialog.name')}</Label>
                    <Input value={formData.name} onChange={e => setFormData({
                  ...formData,
                  name: e.target.value
                })} className="h-16 bg-black/40 border-white/5 rounded-[24px] px-6 text-white font-black italic text-xs tracking-widest focus:ring-blue-600/50 transition-all placeholder:text-foreground" />
                  </div>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                       <Label className="text-[10px] font-black tracking-widest text-muted-foreground italic ml-2">{t('client.property.management.dialog.type')}</Label>
                    ...
                    <div className="space-y-2">
                       <Label className="text-[10px] font-black tracking-widest text-muted-foreground italic ml-2">{t('client.property.management.dialog.status')}</Label>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                     <div className="space-y-2">
                        <Label className="text-[10px] font-black tracking-widest text-muted-foreground italic ml-2">{t('bedrooms')}</Label>
                    ...
                    bedrooms: e.target.value
                     ...
                     <div className="space-y-2">
                        <Label className="text-[10px] font-black tracking-widest text-muted-foreground italic ml-2">{t('bathrooms')}</Label>
               ...
               <div className="space-y-8">
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black tracking-widest text-muted-foreground italic ml-2">{t('address')}</Label>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                     <div className="space-y-2">
                        <Label className="text-[10px] font-black tracking-widest text-muted-foreground italic ml-2">{t('city')}</Label>
                    ...
                    city: e.target.value
                     ...
                     <div className="space-y-2">
                        <Label className="text-[10px] font-black tracking-widest text-muted-foreground italic ml-2">{t('country')}</Label>
                  ...
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black tracking-widest text-muted-foreground italic ml-2">{t('aiNotes')}</Label>
                    <Textarea value={formData.notes} onChange={e => setFormData({
                  ...formData,
                  notes: e.target.value
                })} rows={5} className="bg-black/40 border-white/5 rounded-[32px] text-white placeholder:text-foreground font-black italic text-[10px] tracking-widest p-6 min-h-[140px] focus:ring-blue-600/50 resize-none transition-all" placeholder={t('aiNotesPlaceholder')} />
                  </div>
               </div>
            </div>
            ...
            <DialogFooter className="p-12 bg-white/2 border-t border-white/5 flex gap-4">
              ...
              setShowCreateDialog(false);
              setShowEditDialog(false);
            }} className="h-16 px-10 text-[10px] font-black italic text-muted-foreground">{t('client.property.management.dialog.cancel')}</Button>
              <Button onClick={handleSaveProperty} className="flex-1 bg-blue-600 hover:bg-brand/100 text-white font-black italic text-xs tracking-widest h-16 px-12 rounded-[24px] shadow-xl shadow-blue-600/20 active:scale-95 transition-all">
        ...
        <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
          <DialogContent className="bg-card border-white/10 text-white rounded-[40px] p-12 backdrop-blur-3xl shadow-3xl max-w-lg">
            <div className="flex flex-col items-center text-center space-y-6">
              ...
              <div className="space-y-2">
                 ...
                 <DialogDescription className="text-muted-foreground font-black italic tracking-widest text-[10px] leading-relaxed">
                    {t('deleteDesc', {
            ...
            <DialogFooter className="mt-12 flex flex-col sm:flex-row gap-4">
               <Button variant="ghost" onClick={() => setShowDeleteDialog(false)} className="flex-1 h-16 font-black italic text-[10px] tracking-widest text-muted-foreground">{t('client.property.management.dialog.cancel')}</Button>
