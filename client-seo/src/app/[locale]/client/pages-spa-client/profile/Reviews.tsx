    ...
    return Array.from({ length: 5 }).map((_, i) => (
      <Star 
        ...
        className={cn(
          ...
          i < rating ? 'fill-blue-500 text-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.3)]' : 'text-slate-800'
        )} 
      />
    ));
  };
        ...
        >
          <div className="flex items-center gap-8">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[10px] font-black text-slate-500 tracking-[0.3em] italic">{t('reviewsSubtitle')}</p>
            </div>
          </div>
          ...
          >
            <div className="space-y-1 text-center">
               <p className="text-[10px] font-black text-slate-500 italic tracking-widest leading-none">{t('ratingTitle')}</p>
            ...
            <AnimatePresence mode="popLayout">
              {reviews
                ...
                .map((review, index) => (
                  ...
                  >
                    <Card className="bg-[#1a1b1e]/40 border-white/5 border-l border-t rounded-[40px] overflow-hidden backdrop-blur-3xl group hover:bg-white/5 transition-all shadow-2xl relative">
                        ...
                        <div className="flex flex-col md:flex-row gap-12">
                          <div className="flex md:flex-col items-center md:items-start gap-6">
                            <div className="relative group/avatar">
                               <div className="absolute inset-0 bg-blue-500/20 blur-xl opacity-0 group-hover/avatar:opacity-100 transition-opacity" />
                               <Avatar className="w-20 h-20 border border-white/10 shadow-2xl ring-4 ring-blue-500/5 hov">
                                <AvatarFallback className="bg-[#14151a] text-blue-400 font-black italic text-xl">{review.user.avatar}</AvatarFallback>
                              </Avatar>
                            </div>
                            <div className="space-y-1">
                              <p className="font-black text-white italic tracking-tighter text-xl leading-none">{review.user.name}</p>
                              <p className="text-[10px] font-black text-slate-500 italic tracking-widest leading-none">{review.date}</p>
                            </div>
                          </div>
                          ...
                          <div className="flex-1 space-y-6">
                            <div className="flex flex-wrap items-center justify-between gap-6">
                              ...
                              <Badge className="bg-blue-600/10 text-blue-400 border border-blue-500/20 text-[8px] font-black italic tracking-widest px-4 py-2 rounded-full gap-2 shadow-xl">
                            ...
                            <div className="flex flex-wrap items-center gap-8 pt-4">
                              <Button variant="ghost" className="h-10 px-0 hover:bg-transparent text-slate-500 hover:text-blue-400 text-[10px] font-black italic tracking-widest flex items-center gap-3 transition-colors">
                                <MessageCircle className="w-5 h-5 opacity-40 group-hover:opacity-100" />
                                {t('reply')}
                              </Button>
                              <Button variant="ghost" className="h-10 px-0 hover:bg-transparent text-slate-500 hover:text-red-500 text-[10px] font-black italic tracking-widest flex items-center gap-3 transition-colors">
                                <AlertCircle className="w-5 h-5 opacity-40 group-hover:opacity-100" />
                                {t('report')}
                              </Button>
                              
                              {activeTab === 'received' && (
                                <div className="ml-auto flex items-center gap-3 px-6 py-2.5 bg-emerald-500/10 border border-emerald-500/20 rounded-full shadow-2xl">
                                  <div className="h-4 w-4 rounded-full bg-emerald-500/20 flex items-center justify-center">
                                     <CheckCircle2 className="w-3 h-3 text-emerald-500" />
                                  </div>
                                  <span className="text-[10px] font-black text-emerald-400 italic tracking-widest">{t('reviewsVerified')}</span>
                                </div>
                              )}
                            </div>
                          </div>
