...
export default function CompareList() {
  ...
  return <div className="min-h-screen bg-background p-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto space-y-12 relative z-10">
        ...
        y: 0
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[10px] font-black text-muted-foreground tracking-[0.3em] italic">{t('compareSubtitle')}</p>
            </div>
          </div>
        ...
        opacity: 1,
        scale: 1
      }} className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-0 rounded-[48px] overflow-hidden border border-white/5 shadow-3xl bg-card/40 backdrop-blur-3xl border-l border-t">
          ...
          <div className="hidden lg:flex flex-col border-r border-white/5 bg-black/20">
            <div className="h-[340px] p-10 flex flex-col justify-end gap-2">
               <div className="flex items-center gap-3 text-brand/60 mb-2">
            ...
            <div className="p-6 h-20 flex items-center border-b border-white/5 font-black text-[10px] text-muted-foreground italic tracking-widest">{t('compareLocation')}</div>
            <div className="p-6 h-20 flex items-center border-b border-white/5 font-black text-[10px] text-muted-foreground italic tracking-widest">{t('compareType')}</div>
            <div className="p-6 h-20 flex items-center border-b border-white/5 font-black text-[10px] text-muted-foreground italic tracking-widest">{t('livingSpace')}</div>
            <div className="p-6 h-20 flex items-center border-b border-white/5 font-black text-[10px] text-muted-foreground italic tracking-widest">{t('accommodation')}</div>
            
            {featureList.map((feature, i) => <div key={feature} className={cn("p-6 h-18 flex items-center border-b border-white/5 font-black text-[10px] text-muted-foreground italic  tracking-widest transition-colors hover:text-white group", i % 2 === 0 ? 'bg-black/10' : 'bg-transparent')}>
                {feature}
              </div>)}
          </div>
          ...
          <div className="flex overflow-x-auto scrollbar-hide">
            <AnimatePresence mode="popLayout">
              ...
              x: -20
                  ...
                  <Button variant="ghost" size="icon" aria-label={t("common.close")} className="absolute top-4 right-4 z-20 rounded-2xl bg-black/40 backdrop-blur-xl border border-white/10 text-muted-foreground hover:text-red-500 opacity-0 group-hover/col:opacity-100 transition-all shadow-xl" onClick={() => removeProperty(property.id)}>
                    <X className="w-5 h-5" />
                  </Button>
                  ...
                  <div className="h-[340px] p-8 flex flex-col gap-6 relative overflow-hidden">
                    ...
                    <div className="space-y-4">
                      <h3 className="font-black text-xl text-white italic tracking-tighter leading-tight line-clamp-2 group-hover/col:text-brand transition-colors">{property.title}</h3>
                      <Badge className="bg-success/10 text-success border-success/20 text-[8px] font-black italic tracking-widest px-3 py-1 rounded-full">{property.status}</Badge>
                    </div>
                  </div>
                  ...
                  <div className="p-8 h-20 flex items-center border-b border-white/5 text-[10px] font-black text-muted-foreground italic tracking-widest">
                  ...
                  <div className="p-8 h-20 flex items-center border-b border-white/5 text-[10px] font-black text-muted-foreground italic tracking-widest">
                    <div className="flex items-center gap-3">
                      {property.type === 'VILLA' ? <Home className="w-4 h-4 text-success/60" /> : <Building2 className="w-4 h-4 text-brand/60" />}
                  ...
                  <div className="p-8 h-20 flex items-center border-b border-white/5 text-[10px] font-black text-muted-foreground italic tracking-widest">
                    <div className="flex items-center gap-3">
                      <Square className="w-4 h-4 text-brand/60" />
                  ...
                  <div className="p-8 h-20 flex items-center border-b border-white/5 text-[10px] font-black text-muted-foreground italic tracking-widest">
                    <div className="flex items-center gap-6">
                      <div className="flex items-center gap-2">
                        <Bed className="w-4 h-4 text-brand/40" />
                        <span>{property.beds}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Bath className="w-4 h-4 text-brand/40" />
                ...
                return <div key={feature} className={cn("p-8 h-18 border-b border-white/5 flex items-center justify-center lg:justify-start group/row", i % 2 === 0 ? 'bg-black/10' : 'bg-transparent')}>
                        <div className="flex items-center gap-4">
                          {hasFeature ? <div className="flex items-center gap-4">
                              <div className="h-6 w-6 rounded-full bg-success/10 border border-success/20 flex items-center justify-center">
                                <Check className="w-3.5 h-3.5 text-success shadow-[0_0_10px_rgba(16,185,129,0.3)]" />
                              </div>
                              <span className="lg:hidden text-[10px] font-black italic tracking-widest text-success">{feature}</span>
                            </div> : <div className="flex items-center gap-4">
                              <div className="h-6 w-6 rounded-full bg-white/2 border border-white/5 flex items-center justify-center">
                                <X className="w-3.5 h-3.5 text-foreground" />
                              </div>
                              <span className="lg:hidden text-[10px] font-black italic tracking-widest text-muted-foreground">{feature}</span>
                            </div>}
                        </div>
                      </div>;
              })}
                  ...
                  <div className="p-10 bg-black/20">
                    <Button className="w-full h-14 bg-card text-black hover:bg-muted font-black italic tracking-widest text-[10px] rounded-2xl shadow-xl transition-all">
                      {t('viewFull')}
                    </Button>
                  </div>
                </m.div>)}
              ...
              {properties.length < 3 && <div className="min-w-[340px] flex-1 flex flex-col items-center justify-center bg-white/2 border-dashed border-r border-white/5 relative overflow-hidden group/add">
                  ...
                  <p className="text-[10px] font-black text-muted-foreground italic tracking-[0.3em] group-hover/add:text-brand transition-colors">{t('addPlaceholder')}</p>
                </div>}
            </AnimatePresence>
          </div>
        </m.div>
