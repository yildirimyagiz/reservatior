...
export const HomeChatClient = () => {
  ...
  return (
    <div className="flex flex-col h-screen font-sans relative overflow-hidden bg-[#fafafa] dark:bg-[#0a0a0c]">
        ...
        <div className="flex items-center gap-3">
          {creditsRemaining !== null && (
            <div className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-200/60 dark:border-amber-700/30 shadow-sm">
              ...
              <span className="text-xs font-medium text-amber-500 dark:text-amber-500/80">{t("client.src.credit", { defaultValue: "Credits" })}</span>
            </div>
          )}
          <Link href="/" className="px-5 py-2.5 rounded-full bg-white/50 dark:bg-slate-800/50 hover:bg-white/80 dark:hover:bg-slate-800/80 border border-white/60 dark:border-slate-700/60 text-sm font-semibold text-neutral-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 shadow-sm backdrop-blur-md transition-all">
            {t("client.src.return_to_home", { defaultValue: "Return to Home" })}
          </Link>
        </div>
      </header>
                    ...
                    {msg.properties.map((prop, idx) => (
                      ...
                      >
                        <div className="h-48 overflow-hidden relative m-2 rounded-3xl">
                          {/* Using img tag to avoid domain configuration requirements for external property images */}
      ...
      <div className="flex-none p-4 md:p-8 z-20 bg-gradient-to-t from-[#fafafa] via-[#fafafa]/80 dark:from-[#0a0a0c] dark:via-[#0a0a0c]/80 to-transparent">
        <div className="max-w-4xl mx-auto relative group">
          ...
          <div className="relative flex items-center bg-white/70 dark:bg-[#14151a]/70 backdrop-blur-2xl border border-white dark:border-slate-800 shadow-2xl shadow-indigo-900/5 dark:shadow-none rounded-[2.5rem] p-2 focus-within:bg-white/90 dark:focus-within:bg-[#14151a]/90 transition-all">
            ...
            <textarea
              ...
              placeholder={t("client.src.tell_ai_what_you_are_looking_for", { defaultValue: "Tell AI what you are looking for..." })}
              className="flex-1 max-h-32 min-h-[60px] bg-transparent border-none focus:ring-0 resize-none py-4 px-2 text-neutral-900 dark:text-white placeholder:text-neutral-400 dark:placeholder:text-slate-500 font-medium text-[17px] focus-visible:outline-none"
              rows={1}
            />
        ...
        <p className="text-center text-[11px] font-medium text-neutral-400 dark:text-slate-500 mt-4 tracking-wide uppercase">
          {t("client.src.reservatior_ai_can_make_mistakes_verify_information", { defaultValue: "Reservatior AI can make mistakes. Verify important information." })}
        </p>
      </div>
          ...
          >
            ...
            >
              ...
              <div className="bg-gradient-to-br from-indigo-600 to-purple-600 p-6 text-white text-center relative overflow-hidden">
                ...
                <h2 className="text-2xl font-bold mb-1 relative z-10">{t("client.src.accommodation_pre_application", { defaultValue: "Accommodation Pre-Application" })}</h2>
                <p className="text-indigo-100 text-sm font-medium relative z-10">
                  {selectedPropertyForBooking.title} {t("client.src.let_us_prepare_your_request_for", { defaultValue: "let us prepare your request for" })}
                </p>
              </div>
              ...
              <div className="p-6 space-y-6 bg-neutral-50/50 dark:bg-transparent">
                ...
                {/* Guest Count */}
                <div>
                  <label className="block text-sm font-bold text-neutral-700 dark:text-slate-300 mb-2">{t("client.src.number_of_guests", { defaultValue: "Number of Guests" })}</label>
                  <div className="flex items-center gap-4 bg-white dark:bg-[#0a0a0c] p-2 rounded-2xl border border-neutral-200 dark:border-slate-800 shadow-sm">
                    ...
                    >-</button>
                    <div className="flex-1 text-center font-black text-xl text-indigo-950 dark:text-indigo-300">{bookingForm.guestCount} {t("client.src.person", { defaultValue: "Person" })}</div>
                    <button 
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <span className="font-bold text-sm">{t("client.src.smoking", { defaultValue: "Smoking" })}</span>
                    <span className="text-[11px] font-medium opacity-70">{t("client.src.indoors_on_balcony", { defaultValue: "Indoors or Balcony" })}</span>
                  </button>
                  ...
                  >
                    ...
                    <span className="font-bold text-sm">{t("client.src.bbq_grill", { defaultValue: "BBQ Grill" })}</span>
                    <span className="text-[11px] font-medium opacity-70">{t("client.src.in_garden_or_terrace", { defaultValue: "In Garden or Terrace" })}</span>
                  </button>
                </div>

                {/* Details */}
                <div>
                  <label className="block text-sm font-bold text-neutral-700 dark:text-slate-300 mb-2">{t("client.src.guest_profile_visit_purpose", { defaultValue: "Guest Profile & Visit Purpose" })}</label>
                  <textarea 
                    value={bookingForm.guestDetails}
                    onChange={(e) => setBookingForm(p => ({ ...p, guestDetails: e.target.value }))}
                    placeholder={t("client.src.guest_profile_placeholder", { defaultValue: "Tell us a bit about yourself..." })}
              ...
              <div className="p-6 bg-white dark:bg-[#14151a] border-t border-neutral-100 dark:border-slate-800 flex gap-3">
                ...
                <button 
                  onClick={() => {
                    setInput(`${selectedPropertyForBooking.title} ${t("client.src.complete_rental_application_for_person", { defaultValue: "complete rental application for person" })} ${bookingForm.guestCount}, ${t("client.src.smoking_label", { defaultValue: "Smoking:" })} ${bookingForm.smoking?t("client.src.yes", { defaultValue: "Yes"}):t("client.src.no", { defaultValue: "No" })}, ${t("client.src.bbq_label", { defaultValue: "BBQ:" })} ${bookingForm.bbq?t("client.src.i_want", { defaultValue: "I want" }):t("client.src.i_dont_want", { defaultValue: "I don't want" })}, ${t("client.src.details_label", { defaultValue: "Details:" })} ${bookingForm.guestDetails || t("client.src.not_specified", { defaultValue: "Not specified" })}`);
                    setSelectedPropertyForBooking(null);
                    setTimeout(handleSend, 100);
                  }}
                  className="flex-2 py-3.5 rounded-xl font-bold text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 shadow-lg shadow-indigo-500/25 dark:shadow-none active:scale-95 transition-all"
                >
                  {t("client.src.submit_request", { defaultValue: "Submit Request" })}
                </button>
              </div>
            </motion.div>
          </motion.div>
