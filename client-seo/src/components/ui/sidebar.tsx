...
function Sidebar({
  ...
  if (collapsible === "none") {
    return <div data-slot="sidebar" className={cn("bg-sidebar text-sidebar-foreground flex h-full w-(--sidebar-width)-col", className)} {...props}>
        {children}
      </div>;
  }
  if (isMobile) {
    return <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        "--sidebar-width": SIDEBAR_WIDTH_MOBILE
          ...
          <SheetHeader className="sr-only">
            <SheetTitle>{t("client.src.sidebar")}</SheetTitle>
            <SheetDescription>{t("client.src.displays_the_mobile_sidebar")}</SheetDescription>
          </SheetHeader>
  ...
  return <div className="group peer text-sidebar-foreground hidden md:block" data-state={state} data-collapsible={state === "collapsed" ? collapsible : ""} data-variant={variant} data-side={side} data-slot="sidebar">
    ...
    variant === "floating" || variant === "inset" ? "p-2 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+var(--spacing-4)+2px)]" : "group-data-[collapsible=icon]:w-(--sidebar-width-icon) group-data-[side=left]:border-r group-data-[side=right]:border-l", className)} {...props}>
...
function SidebarTrigger({
  ...
  return <Button data-sidebar="trigger" data-slot="sidebar-trigger" variant="ghost" size="icon" className={cn("min-h-12 min-w-12", className)} onClick={event => {
    ...
    toggleSidebar();
      ...
      <PanelLeftIcon />
      <span className="sr-only">{t("client.src.toggle_sidebar")}</span>
    </Button>;
}
