      ...
      <CardContent className="space-y-4">
        {locks.map((lock, i) => (
          <div key={i} className="flex items-center gap-3 p-3 bg-secondary/20 rounded-xl border border-white/5 group hover:bg-secondary/30 transition-all cursor-pointer">
            <div className={`p-2 rounded-lg ${lock.status === "LOCKED" ? "bg-slate-800 text-white" : "bg-blue-500/10 text-blue-500"}`}>
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 text-[10px] text-slate-500">
                <span className="flex items-center gap-1 font-bold">
                  <BatteryMedium className={`w-3 h-3 ${lock.battery === "12%" ? "text-red-500 animate-pulse" : "text-blue-500"}`} /> {lock.battery}
                </span>
                <span className="flex items-center gap-1 font-bold">
            ...
            <Badge variant="outline" className={`text-[10px] uppercase font-bold ${lock.status === "LOCKED" ? "text-slate-400 border-slate-700" : "text-blue-400 border-blue-800"}`}>
