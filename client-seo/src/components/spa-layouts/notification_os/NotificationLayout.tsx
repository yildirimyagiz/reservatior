...
import { LayoutDashboard, Bell, MessageSquare, ScrollText, Radio, ChevronRight } from "lucide-react";
...
export function NotificationLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems: NavItem[] = [
    {
      title: "Dashboard",
      href: "/notification-os/dashboard",
      icon: LayoutDashboard
    },
    {
      title: "Notifications",
      href: "/notification-os/notifications",
      icon: Bell
    },
    {
      title: "Messages",
      href: "/notification-os/messages",
      icon: MessageSquare
    },
    {
      title: "Logs",
      href: "/notification-os/logs",
      icon: ScrollText
    },
    {
      title: "Channels",
      href: "/notification-os/channels",
      icon: Radio
    }
  ];
        ...
        <div className="flex items-center gap-2">
          <Avatar className="h-8 w-8 shrink-0">
            <AvatarFallback className="text-xs bg-primary/20 text-primary border border-primary/30">
              {user?.firstName?.slice(0, 2).toUpperCase() || "NO"}
            </AvatarFallback>
          </Avatar>
          {sidebarOpen && (
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate text-foreground">{user?.firstName || "Notification"}</p>
