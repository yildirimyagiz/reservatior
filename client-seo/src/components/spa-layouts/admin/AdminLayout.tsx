...
export function AdminLayout({
  ...
  return <div className="flex flex-col h-screen bg-[#0A0A0B] text-white overflow-hidden transition-colors duration-300">
      {/* Unified Application Header - Full Width */}
      <AppHeader />

      <div className="flex flex-1 overflow-hidden">
        {/* Desktop Sidebar */}
        <aside className={cn("hidden md:flex flex-col border-r border-white/10 bg-white/5 transition-all duration-300 shrink-0", sidebarOpen ? "w-64" : "w-16")}>
          <SidebarContent />
        </aside>
        ...
        {mobileSidebarOpen && <div className="fixed inset-0 z-50 md:hidden">
            ...
            <aside className="absolute left-0 top-0 bottom-0 w-64 bg-white/5 flex flex-col pt-16">
              <SidebarContent />
            </aside>
          </div>}

        <main className="flex-1 overflow-auto bg-[#0A0A0B] flex flex-col transition-colors duration-300">
