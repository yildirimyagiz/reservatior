...
import { LayoutDashboard, Key, Plug, Webhook, ScrollText, ChevronRight } from "lucide-react";
...
export function DeveloperLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems: NavItem[] = [
    {
      title: "Dashboard",
      href: "/developer-os/dashboard",
      icon: LayoutDashboard
    },
    {
      title: "API Keys",
      href: "/developer-os/api-keys",
      icon: Key
    },
    {
      title: "Integrations",
      href: "/developer-os/integrations",
      icon: Plug
    },
    {
      title: "Webhooks",
      href: "/developer-os/webhooks",
      icon: Webhook
    },
    {
      title: "Logs",
      href: "/developer-os/logs",
      icon: ScrollText
    }
  ];
        ...
        <div className="flex items-center gap-2">
          <Avatar className="h-8 w-8 shrink-0">
            <AvatarFallback className="text-xs bg-primary/20 text-primary border border-primary/30">
              {user?.firstName?.slice(0, 2).toUpperCase() || "DV"}
            </AvatarFallback>
          </Avatar>
          {sidebarOpen && (
