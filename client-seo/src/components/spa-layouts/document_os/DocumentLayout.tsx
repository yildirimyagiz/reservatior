...
import { LayoutDashboard, FileStack, FileSignature, PenTool, Copy, ChevronRight } from "lucide-react";
...
export function DocumentLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems: NavItem[] = [
    {
      title: "Dashboard",
      href: "/document-os/dashboard",
      icon: LayoutDashboard
    },
    {
      title: "Documents",
      href: "/document-os/documents",
      icon: FileStack
    },
    {
      title: "Contracts",
      href: "/document-os/contracts",
      icon: FileSignature
    },
    {
      title: "Signatures",
      href: "/document-os/signatures",
      icon: PenTool
    },
    {
      title: "Templates",
      href: "/document-os/templates",
      icon: Copy
    }
  ];
        ...
        <div className="flex items-center gap-2">
          <Avatar className="h-8 w-8 shrink-0">
            <AvatarFallback className="text-xs bg-primary/20 text-primary border border-primary/30">
              {user?.firstName?.slice(0, 2).toUpperCase() || "DC"}
            </AvatarFallback>
          </Avatar>
          {sidebarOpen && (
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate text-foreground">{user?.firstName || "Document"}</p>
