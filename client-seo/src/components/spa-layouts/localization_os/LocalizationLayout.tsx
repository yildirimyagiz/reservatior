...
import { LayoutDashboard, Globe, Coins, Languages, Scale, Receipt, ChevronRight } from "lucide-react";
...
export function LocalizationLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems: NavItem[] = [
    {
      title: "Dashboard",
      href: "/localization-os/dashboard",
      icon: LayoutDashboard
    },
    {
      title: "Countries",
      href: "/localization-os/countries",
      icon: Globe
    },
    {
      title: "Currencies",
      href: "/localization-os/currencies",
      icon: Coins
    },
    {
      title: "Languages",
      href: "/localization-os/languages",
      icon: Languages
    },
    {
      title: "Compliance",
      href: "/localization-os/compliance",
      icon: Scale
    },
    {
      title: "Taxes",
      href: "/localization-os/taxes",
      icon: Receipt
    }
  ];
        ...
        <div className="flex items-center gap-2">
          <Avatar className="h-8 w-8 shrink-0">
            <AvatarFallback className="text-xs bg-primary/20 text-primary border border-primary/30">
              {user?.firstName?.slice(0, 2).toUpperCase() || "LO"}
            </AvatarFallback>
          </Avatar>
          {sidebarOpen && (
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate text-foreground">{user?.firstName || "Localization"}</p>
