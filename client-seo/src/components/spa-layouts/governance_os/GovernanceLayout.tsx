...
import { LayoutDashboard, ShieldCheck, FileCheck, Scale, ScrollText, ChevronRight } from "lucide-react";
...
export function GovernanceLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems: NavItem[] = [
    {
      title: "Dashboard",
      href: "/governance-os/dashboard",
      icon: LayoutDashboard
    },
    {
      title: "Rules",
      href: "/governance-os/rules",
      icon: ShieldCheck
    },
    {
      title: "Compliance",
      href: "/governance-os/compliance",
      icon: FileCheck
    },
    {
      title: "Legal",
      href: "/governance-os/legal",
      icon: Scale
    },
    {
      title: "Audit Trail",
      href: "/governance-os/audit-trail",
      icon: ScrollText
    }
  ];
        ...
        <div className="flex items-center gap-2">
          <Avatar className="h-8 w-8 shrink-0">
            <AvatarFallback className="text-xs bg-primary/20 text-primary border border-primary/30">
              {user?.firstName?.slice(0, 2).toUpperCase() || "GO"}
            </AvatarFallback>
          </Avatar>
          {sidebarOpen && (
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate text-foreground">{user?.firstName || "Governance"}</p>
