...
import { LayoutDashboard, Users, FileSignature, Truck, Star, ChevronRight } from "lucide-react";
...
export function PartnerLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems: NavItem[] = [
    {
      title: "Dashboard",
      href: "/partner-os/dashboard",
      icon: LayoutDashboard
    },
    {
      title: "Partners",
      href: "/partner-os/partners",
      icon: Users
    },
    {
      title: "Agreements",
      href: "/partner-os/agreements",
      icon: FileSignature
    },
    {
      title: "Suppliers",
        ...
        <div className="flex items-center gap-2">
          <Avatar className="h-8 w-8 shrink-0">
            <AvatarFallback className="text-xs bg-primary/20 text-primary border border-primary/30">
              {user?.firstName?.slice(0, 2).toUpperCase() || "PT"}
            </AvatarFallback>
          </Avatar>
          {sidebarOpen && (
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate text-foreground">{user?.firstName || "Partner"}</p>
