...
import { LayoutDashboard, BarChart3, FileText, Activity, HeartPulse, ChevronRight } from "lucide-react";
...
export function AnalyticsLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems: NavItem[] = [
    {
      title: "Dashboard",
      href: "/analytics-os/dashboard",
      icon: LayoutDashboard
    },
    {
      title: "Analytics",
      href: "/analytics-os/analytics",
      icon: BarChart3
    },
    {
      title: "Reports",
      href: "/analytics-os/reports",
      icon: FileText
    },
    {
      title: "Metrics",
      href: "/analytics-os/metrics",
      icon: Activity
    },
    {
      title: "Health",
      href: "/analytics-os/health",
      icon: HeartPulse
    }
  ];
        ...
        <div className="flex items-center gap-2">
          <Avatar className="h-8 w-8 shrink-0">
            <AvatarFallback className="text-xs bg-primary/20 text-primary border border-primary/30">
              {user?.firstName?.slice(0, 2).toUpperCase() || "AN"}
            </AvatarFallback>
          </Avatar>
          {sidebarOpen && (
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate text-foreground">{user?.firstName || "Analytics"}</p>
