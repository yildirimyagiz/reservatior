...
export function PriceShieldWidget({
  ...
  return <Card className="border-none shadow-2xl bg-card overflow-hidden ring-1 ring-slate-100 flex flex-col">
      <CardHeader className="bg-gradient-to-br from-brand to-info text-white pb-8 relative overflow-hidden">
        ...
        <div className="flex items-center justify-between relative z-10">
          ...
          <Badge variant="secondary" className="bg-success text-white hover:bg-blue-600 border-none font-black text-[10px] animate-pulse">{t("client.src.save")}{type === 'SALE' ? '$' + (maxPrice - ourPrice).toLocaleString() : avgSavings + '%'}
          </Badge>
        </div>
        <div className="mt-4 relative z-10">
          ...
          <CardDescription className="text-brand text-xs font-medium flex items-center gap-1 mt-1">{t("client.src.aiverified_price_comparisons_for")}{type.toLowerCase()}
          </CardDescription>
        </div>
      </CardHeader>
      ...
      <CardContent className="p-6 -mt-4 bg-card rounded-t-3xl relative z-20 space-y-6">
        <div className="space-y-4">
          {comps.map(comp => <div key={comp.source} className={`relative p-4 rounded-2xl border transition-all ${comp.isLower ? 'bg-brand/10 border-brand/30 ring-2 ring-indigo-500/10' : 'bg-muted border-border opacity-70 group hover:opacity-100'}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-[10px] ${comp.isLower ? 'bg-brand text-white' : 'bg-muted text-muted-foreground'}`}>
                    {comp.source.charAt(0)}
                  </div>
                  <div>
                    <span className={`text-xs font-black ${comp.isLower ? 'text-brand' : 'text-muted-foreground'}`}>{comp.source}</span>
                    {comp.isLower && <p className="text-[9px] font-bold text-brand flex items-center gap-0.5">
                        <Key className="w-2.5 h-2.5" /> {type === 'SALE' ? 'Direct Asset Sale' : 'Direct Booking'}
                      </p>}
                  </div>
                </div>
                <div className="text-right">
                  <span className={`text-lg font-black ${comp.isLower ? 'text-brand' : 'text-muted-foreground line-through'}`}>
                    {comp.currency}{comp.price.toLocaleString()}
                  </span>
                  {!comp.isLower && <p className="text-[10px] font-bold text-muted-foreground">{getSourcePrefix(comp.source)}</p>}
                </div>
              </div>
              {!comp.isLower && <div className="mt-2 flex items-center gap-4">
                   <Progress value={ourPrice / comp.price * 100} className="h-1 bg-muted" />
        ...
        <div className="p-5 rounded-3xl bg-card text-white space-y-4 shadow-xl ring-1 ring-slate-800">
