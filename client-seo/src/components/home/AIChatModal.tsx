      ...
      {isOpen && (
        ...
        >
          <m.div
            ...
            className="w-full max-w-4xl h-[85vh] bg-[#fafafa] dark:bg-[#0a0a0c] rounded-3xl shadow-2xl overflow-hidden relative flex flex-col border border-white/60 dark:border-slate-800"
          >
            {/* Header */}
            <div className="flex-none p-6 flex justify-between items-center border-b border-white/40 dark:border-slate-800/40 bg-white/30 dark:bg-[#14151a]/30 backdrop-blur-xl">
              ...
              <button onClick={onClose} aria-label="Close chat" className="p-2 rounded-full hover:bg-neutral-200 dark:hover:bg-slate-800 transition-colors">
                ...
                {aiMessages.length === 0 && (
                  <div className="flex flex-col items-center justify-center h-[40vh] text-center space-y-8">
                    ...
                    <div className="space-y-3 mt-8 max-w-3xl">
                      <p className="text-sm font-bold text-neutral-400 dark:text-slate-500 uppercase tracking-widest">Örnek sorular:</p>
                  ...
                  <div key={msg.id} className={`flex flex-col ${msg.role === "user" ? "items-end" : "items-start"} group`}>
                    <div className={`relative ${msg.role === "user" ? "flex flex-col items-end" : "flex flex-col items-start"}`}>
                      ...
                      <button
                        ...
                        className="absolute -top-2 right-0 p-1.5 rounded-full bg-white/80 dark:bg-slate-700/80 backdrop-blur-sm border border-white/60 dark:border-slate-600/60 opacity-0 group-hover:opacity-100 transition-opacity z-10 hover:bg-white dark:hover:bg-slate-600"
                      >
                        <MoreVertical className="w-4 h-4 text-neutral-600 dark:text-slate-300" />
                      </button>
                      ...
                      <AnimatePresence>
                        {messageMenuOpen === msg.id && (
                          <m.div
                            ...
                            className={`absolute ${msg.role === "user" ? "right-0" : "left-0"} top-0 mt-8 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-white/60 dark:border-slate-700/60 py-2 z-20 min-w-[180px]`}
                          >
                            <button onClick={() => copyMessage(msg.text)} className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors text-left">
                              <Copy className="w-4 h-4 text-neutral-500 dark:text-slate-400" />
                              <span className="text-sm font-medium text-neutral-700 dark:text-slate-300">Kopyala</span>
                            </button>
                            <button onClick={() => shareMessage(msg.text)} className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors text-left">
                              <Share2 className="w-4 h-4 text-neutral-500 dark:text-slate-400" />
                              <span className="text-sm font-medium text-neutral-700 dark:text-slate-300">Paylaş</span>
                            </button>
                            <button onClick={() => archiveMessage(msg.id)} className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors text-left">
                              <Archive className="w-4 h-4 text-neutral-500 dark:text-slate-400" />
                              <span className="text-sm font-medium text-neutral-700 dark:text-slate-300">Arşivle</span>
                            </button>
                            {msg.role === "user" && (
                              <button onClick={() => startEditMessage(msg.id, msg.text)} className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors text-left">
                                <Edit2 className="w-4 h-4 text-neutral-500 dark:text-slate-400" />
                                <span className="text-sm font-medium text-neutral-700 dark:text-slate-300">Düzenle</span>
                              </button>
                            )}
