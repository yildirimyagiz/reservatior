...
echo "Building and restarting containers on the server..."
sshpass -e ssh -o StrictHostKeyChecking=no root@72.61.71.6 "cd /root/reservatior && docker compose build server web && docker compose restart server web"
