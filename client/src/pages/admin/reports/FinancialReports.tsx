...
export default function FinancialReports() {
  ...
  const stats = [{
    title: t("admin.reports.total_revenue_dna"),
    value: "$452,000",
    change: "+12.5%",
    trend: "up",
    icon: DollarSign,
    ...
    title: t("admin.reports.entity_subsystems"),
    value: "2,450",
    change: "+4.3%",
    trend: "up",
    icon: Zap,
    ...
    title: t("admin.reports.payout_queue"),
    value: "$12,400",
    change: "-2.1%",
    trend: "down",
    icon: Activity,
    ...
    title: t("admin.reports.system_net_margin"),
    value: "$84,200",
    change: "+8.7%",
    trend: "up",
    icon: PieChart,
  ...
  return <PageShell title={t("admin.reports.financial_neural_matrix")} description={t("admin.reports.systemwide_revenue_stream_analysis")}>
      <div className="space-y-10 pb-20">
        
        {/* KPI NEURAL GRID */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, i) => <Card key={i} className="bg-card border-border rounded-3xl overflow-hidden shadow-2xl relative group border-l-2 border-l-transparent hover:border-l-blue-500/50 transition-all">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <Card className="lg:col-span-8 bg-card backdrop-blur-xl border-border rounded-4xl overflow-hidden shadow-2xl border-l border-t relative group">
            <CardHeader className="p-8 border-b border-border flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription className="text-[10px] font-bold text-muted-foreground mt-1">{t("admin.reports.crosscluster_revenue_performance_metrics")}</CardDescription>
              </div>
              <Button variant="ghost" className="h-10 rounded-xl hover:bg-muted/50 text-muted-foreground font-bold text-[9px] gap-2">{t("admin.reports.synchronize_matrix")}<Download className="w-3.5 h-3.5" />
              </Button>
            </CardHeader>
          ...
          <Card className="lg:col-span-4 bg-card backdrop-blur-xl border-border rounded-4xl overflow-hidden shadow-2xl border-l border-t">
            ...
            <CardContent className="p-8 space-y-8">
              {[{
              name: t("admin.reports.entity_agencies", "Varlık Acenteleri"),
              value: "$280k",
              pct: 62,
              color: "bg-blue-500 shadow-blue-500/40"
            }, {
              name: t("admin.reports.premium_nodes", "Premium Düğümler"),
              value: "$94k",
              pct: 21,
              color: "bg-purple-500 shadow-purple-500/40"
            }, {
              name: t("admin.reports.api_stream_access", "API Akış Erişimi"),
              value: "$52k",
              pct: 12,
              color: "bg-emerald-500 shadow-emerald-500/40"
            }, {
              name: t("admin.reports.protocol_fees", "Protokol Ücretleri"),
              value: "$26k",
              pct: 5,
              color: "bg-orange-500 shadow-orange-500/40"
                  ...
                  <div className="h-1.5 w-full bg-muted/50 rounded-full overflow-hidden">
                    <div className={cn("h-full rounded-full transition-all duration-1000 shadow-[0_0_10px]", item.color)} style={{
                  width: `${item.pct}%`
                }} />
                  </div>
                </div>)}
              ...
              <div className="pt-8 border-t border-border mt-auto">
                 <div className="p-6 bg-muted/50 rounded-3xl border border-border group hover:bg-muted/50 transition-all cursor-pointer">
                    <p className="text-[9px] font-bold text-muted-foreground mb-1">{t("admin.reports.projection_buffer")}</p>
                    <p className="text-xl font-bold text-foreground whitespace-pre leading-none">{t("admin.reports.positive_cycle_forecast")}</p>
        ...
        <Card className="bg-card border-border rounded-4xl overflow-hidden shadow-2xl border-l border-t relative">
           <CardHeader className="p-8 border-b border-border flex flex-row items-center justify-between">
              ...
              <Button variant="ghost" className="h-10 px-4 rounded-xl hover:bg-muted/50 text-blue-400 font-bold text-[9px] gap-2">{t("admin.reports.view_global_ledger")}<ChevronRight className="w-4 h-4" />
              </Button>
           </CardHeader>
               ...
               <table className="w-full text-left">
                 <thead className="bg-muted/50 border-b border-border">
                   <tr className="border-none hover:bg-transparent">
                     <th className="py-6 px-8 text-[10px] font-bold text-muted-foreground">{t("admin.reports.temporal_hash")}</th>
                     <th className="py-6 px-8 text-[10px] font-bold text-muted-foreground">{t("admin.reports.entity_origin")}</th>
                     <th className="py-6 px-8 text-[10px] font-bold text-muted-foreground">{t("admin.reports.sync_date")}</th>
                     <th className="py-6 px-8 text-[10px] font-bold text-muted-foreground">{t("admin.reports.type_class")}</th>
                     <th className="py-6 px-8 text-[10px] font-bold text-muted-foreground text-right">{t("admin.reports.creditdebit")}</th>
                     <th className="py-6 px-8 text-[10px] font-bold text-muted-foreground text-right">{t("admin.reports.node_state")}</th>
                   </tr>
                 </thead>
                 <tbody className="divide-y divide-white/5">
                   {[1, 2, 3, 4, 5, 6].map((_, i) => <tr key={i} className="group hover:bg-muted/50 transition-all">
                       <td className="py-8 px-8 font-mono text-[10px] font-bold text-blue-400">{t("admin.reports.trx9482")}{i}{t("admin.reports.xls")}</td>
                       <td className="py-8 px-8">
                          <p className="text-sm font-bold text-foreground leading-none">{t("admin.reports.neural_realty_systems")}{i + 1}</p>
                          <p className="text-[9px] font-bold text-slate-600 mt-1">{t("admin.reports.orgclusterdelta")}</p>
                       </td>
                       <td className="py-8 px-8 text-[10px] font-bold text-muted-foreground">{t("admin.reports.mar")}{24 - i}, 2026</td>
                       <td className="py-8 px-8">
                         <Badge variant="outline" className="bg-muted/50 border-border text-[9px] font-bold px-2 py-0.5 text-muted-foreground">{t("admin.reports.systemsync")}</Badge>
                       </td>
