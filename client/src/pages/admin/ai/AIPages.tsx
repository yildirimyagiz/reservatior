...
export function AIStudio() {
  ...
  const MODES = [{
    ...
    label: t("admin.ai.property_description"),
    icon: "🏡"
  }, {
    value: "email-template",
    label: t("admin.ai.email_template"),
    icon: "✉️"
  }, {
    value: "market-analysis",
    label: t("admin.ai.market_analysis"),
    icon: "📊"
  }, {
    value: "negotiation-script",
    label: t("admin.ai.negotiation_script"),
    icon: "🤝"
  }, {
    value: "listing-title",
    label: t("admin.ai.listing_title"),
    icon: "✏️"
  }];
  ...
  return <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          <h2 className="font-semibold flex items-center gap-2"><Sparkles className="w-4 h-4 text-primary" />{t("admin.ai.generate_content")}</h2>
          <div className="space-y-1.5">
            <Label>{t("admin.ai.mode")}</Label>
          ...
          <div className="space-y-1.5">
            <Label>{t("admin.ai.your_input")}</Label>
          ...
          <Button onClick={handleGenerate} disabled={loading || !prompt.trim()} className="w-full">
            {loading ? <><RefreshCw className="w-4 h-4 mr-2 animate-spin" />{t("admin.ai.generating")}</> : <><Sparkles className="w-4 h-4 mr-2" />{t("admin.ai.generate")}</>}
          </Button>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold flex items-center gap-2"><Bot className="w-4 h-4 text-primary" />{t("admin.ai.output")}</h2>
            {result && <Button variant="outline" size="sm" onClick={() => {
            navigator.clipboard?.writeText(result);
            toast({
              title: t("admin.ai.copied")
            });
          }}>{t("admin.ai.copy")}</Button>}
          </div>
          {result ? <div className="bg-muted/30 rounded-lg p-4 text-sm leading-relaxed whitespace-pre-wrap min-h-[200px]">{result}</div> : <div className="min-h-[200px] flex flex-col items-center justify-center text-muted-foreground gap-3">
              ...
              <p className="text-sm">{t("admin.ai.generated_content_will_appear")}</p>
            </div>}
        </div>
      </div>
    </div>;
...
export function AIValuation() {
  ...
  const handleRunAll = async () => {
    ...
    setRunning(false);
    toast({
      title: t("admin.ai.valuations_updated"),
      description: t("admin.ai.all_properties_revalued")
    });
  };
  const TREND_STYLES = {
    UP: "text-green-600",
  ...
  return <>
      <PageShell title={t("admin.ai.ai_property_valuation")} description={t("admin.ai.automated_property_value_estimates")} actions={<Button onClick={handleRunAll} disabled={running} size="sm">
            <RefreshCw className={`w-4 h-4 mr-1.5 ${running ? "animate-spin" : ""}`} />
            {running ? "Running..." : "Run All Valuations"}
          </Button>} stats={[{
      label: t("admin.ai.valued_properties"),
      value: valuations.length
    }, {
      label: t("admin.ai.avg_confidence"),
      value: `${Math.round(valuations.reduce((s, v) => s + v.confidence, 0) / (valuations.length || 1) * 100)}%`
    }, {
      label: t("admin.ai.trending_up"),
      value: valuations.filter(v => v.trend === "UP").length
    }, {
      label: t("admin.ai.total_portfolio_value"),
          ...
          {valuations.map(v => <div key={v.id} className="bg-card border border-border rounded-xl p-5 space-y-4">
              ...
              <div className="flex items-end justify-between">
                ...
                <div className="text-right text-sm text-muted-foreground">
                  <p>{v.comparables}{t("admin.ai.comparables")}</p>
                  <p className="text-xs">{t("admin.ai.updated")}{v.lastRun.split(" ")[0]}</p>
                </div>
              </div>
              <div>
                <div className="flex justify-between text-xs mb-1"><span className="text-muted-foreground">{t("admin.ai.confidence")}</span><span>{Math.round(v.confidence * 100)}%</span></div>
            ...
            setDetailOpen(true);
                ...
                <Eye className="w-3.5 h-3.5 mr-1.5" />{t("admin.ai.view_comparables")}</Button>
            </div>)}
        </div>
      </PageShell>
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>{t("admin.ai.valuation_detail")}{selected?.propertyName}</DialogTitle></DialogHeader>
          {selected && <div className="space-y-4 py-2">
              <div className="text-center py-4 bg-muted/30 rounded-xl">
                ...
                <p className="text-sm text-muted-foreground mt-1">{t("admin.ai.estimated_market_value")}</p>
              </div>
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div><p className="text-xs text-muted-foreground">{t("admin.ai.confidence")}</p><p className="font-semibold">{Math.round(selected.confidence * 100)}%</p></div>
                <div><p className="text-xs text-muted-foreground">{t("admin.ai.comparables_used")}</p><p className="font-semibold">{selected.comparables}</p></div>
                <div><p className="text-xs text-muted-foreground">{t("admin.ai.price_trend")}</p><p className={`font-semibold ${TREND_STYLES[selected.trend]}`}>{TREND_ICONS[selected.trend]} {selected.trend}</p></div>
                <div><p className="text-xs text-muted-foreground">{t("admin.ai.last_updated")}</p><p className="font-semibold text-xs">{selected.lastRun}</p></div>
              </div>
              <div className="text-xs text-muted-foreground bg-muted/30 p-3 rounded-lg">{t("admin.ai.value_estimate_based_on")}{selected.comparables}{t("admin.ai.comparable_properties_sold_within")}</div>
            </div>}
        </DialogContent>
      </Dialog>
    </>;
...
export function Automation() {
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    setCreateOpen(false);
    toast({
      title: t("admin.ai.rule_created")
    });
  };
  return <>
      <PageShell title={t("admin.ai.automation_rules")} description={t("admin.ai.triggerbased_workflows_that_run")} createLabel={t("admin.ai.new_rule", "Yeni Kural")} onCreateClick={() => setCreateOpen(true)} stats={[{
          ...
          {MOCK_RULES.map(rule => <div key={rule.id} className={`bg-card border rounded-xl p-4 flex items-center gap-4 ${rule.isActive ? "border-border" : "border-border opacity-60"}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                  ...
                  <span>→</span>
                  <span>{t("admin.ai.action")}<strong className="text-foreground">{rule.action}</strong></span>
                </div>
                <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                  <span>{t("admin.ai.ran")}{rule.runCount}{t("admin.ai.times")}</span>
                  {rule.lastRun && <span>{t("admin.ai.last")}{rule.lastRun}</span>}
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem><Edit className="w-4 h-4 mr-2" />{t("admin.ai.edit")}</DropdownMenuItem>
                    <DropdownMenuItem className="text-destructive"><Trash2 className="w-4 h-4 mr-2" />{t("admin.ai.delete")}</DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>
            </div>)}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>{t("admin.ai.new_automation_rule")}</DialogTitle><DialogDescription>{t("admin.ai.define_a_trigger_and")}</DialogDescription></DialogHeader>
          <form onSubmit={handleCreate} className="space-y-4 py-2">
            <div className="space-y-1.5"><Label>{t("admin.ai.rule_name")}</Label><Input value={form.name} onChange={e => setForm({
              ...form,
              name: e.target.value
            })} required /></div>
            <div className="space-y-1.5"><Label>{t("admin.ai.trigger")}</Label>
              <Select value={form.trigger} onValueChange={v => setForm({
              ...form,
              trigger: v
            })}>
                <SelectTrigger><SelectValue placeholder={t("admin.ai.when_this_happens")} /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="lead-created">{t("admin.ai.lead_created")}</SelectItem>
                  <SelectItem value="lease-expiring">{t("admin.ai.lease_expiring")}</SelectItem>
                  <SelectItem value="payment-overdue">{t("admin.ai.payment_overdue")}</SelectItem>
                  <SelectItem value="lead-score-high">{t("admin.ai.lead_score")}{'>'} 80</SelectItem>
                  <SelectItem value="booking-confirmed">{t("admin.ai.booking_confirmed")}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5"><Label>{t("admin.ai.action")}</Label>
              <Select value={form.action} onValueChange={v => setForm({
              ...form,
              action: v
            })}>
                <SelectTrigger><SelectValue placeholder={t("admin.ai.then_do_this")} /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="send-email">{t("admin.ai.send_email")}</SelectItem>
                  <SelectItem value="create-task">{t("admin.ai.create_task")}</SelectItem>
                  <SelectItem value="notify-agent">{t("admin.ai.notify_agent")}</SelectItem>
                  <SelectItem value="assign-lead">{t("admin.ai.assign_lead")}</SelectItem>
                  <SelectItem value="send-sms">{t("admin.ai.send_sms")}</SelectItem>
                </SelectContent>
              </Select>
            </div>
