...
export default function SecurityEvents() {
  ...
  return <PageShell title={t("admin.security.neural_event_matrix")} description={t("admin.security.realtime_surveillance_of_systemwide")}>
      <div className="space-y-10 pb-20">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-4">
           <div className="relative flex-1 max-w-md group">
              ...
              <Input placeholder={t("admin.security.search_audit_trail")} className="bg-card border-border rounded-2xl pl-12 h-14 text-foreground focus:ring-blue-500/20 transition-all" value={search} onChange={e => setSearch(e.target.value)} />
           </div>
              ...
              <Table>
                 <TableHeader className="bg-muted/50 border-b border-border">
                    <TableRow className="hover:bg-transparent border-none">
                       <TableHead className="text-[10px] font-bold text-muted-foreground py-6 px-8">{t("admin.security.audit_entry")}</TableHead>
                       <TableHead className="text-[10px] font-bold text-muted-foreground px-8">{t("admin.security.severity")}</TableHead>
                       <TableHead className="text-[10px] font-bold text-muted-foreground px-8">{t("admin.security.entity_signature")}</TableHead>
                       <TableHead className="text-[10px] font-bold text-muted-foreground px-8">{t("admin.security.node_state")}</TableHead>
                 ...
                 <TableBody>
                    {loading ? <TableRow>
                          <TableCell colSpan={5} className="py-20 text-center">
                             ...
                             <p className="text-xs font-bold text-muted-foreground animate-pulse">{t("admin.security.syncing_event_matrix")}</p>
                          </TableCell>
                       </TableRow> : filteredEvents.map(event => <TableRow key={event.id} className="border-b border-border hover:bg-muted/50 transition-all group">
                           ...
                           <TableCell className="px-8">
                              <div className="space-y-1">
                                 ...
                                 <p className="text-[9px] font-bold text-slate-600 mt-1 flex items-center gap-1"><Globe className="w-3 h-3" /> {event.ipAddress}</p>
                              </div>
                           </TableCell>
                           <TableCell className="px-8">
                           ...
                           <TableCell className="px-8 text-right">
                              <Button variant="ghost" className="h-12 w-12 rounded-2xl hover:bg-muted/50 text-muted-foreground hover:text-foreground" onClick={() => {
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
         <DialogContent className="bg-[#14151a] border-border text-foreground rounded-3xl max-w-xl">
            <DialogHeader>
            ...
            {selectedEvent && <div className="space-y-6 pt-6">
                  <div className="grid grid-cols-2 gap-6">
                     <div className="p-4 bg-muted/50 rounded-2xl border border-border">
                        <p className="text-[9px] font-bold text-muted-foreground mb-1">{t("admin.security.temporal_stamp")}</p>
                        <p className="text-sm font-bold text-foreground">{new Date(selectedEvent.createdAt).toLocaleString()}</p>
                     </div>
                     <div className="p-4 bg-muted/50 rounded-2xl border border-border">
                        <p className="text-[9px] font-bold text-muted-foreground mb-1">{t("admin.security.node_signature")}</p>
                        <p className="text-sm font-bold text-foreground font-mono">{selectedEvent.ipAddress}</p>
                     </div>
                  </div>
                  <div className="p-6 bg-muted/50 rounded-2xl border border-border">
                     <p className="text-[9px] font-bold text-muted-foreground mb-2">{t("admin.security.surveillance_descriptor")}</p>
                     <p className="text-sm text-muted-foreground leading-relaxed">{selectedEvent.description}</p>
                  </div>
               </div>}
            <DialogFooter className="pt-6">
               <Button onClick={() => setDetailOpen(false)} className="bg-muted/50 hover:bg-muted/50 text-foreground border border-border rounded-xl font-bold text-[10px] transition-all px-8">{t("admin.security.close_audit")}</Button>
            </DialogFooter>
         </DialogContent>
      </Dialog>
    </PageShell>;
