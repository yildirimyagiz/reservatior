...
export default function AdminProperties() {
  ...
  const handleGenerateBrochure = async (propertyId: string) => {
    try {
      toast({ title: t('admin.brochure.title', "AI Brochure"), description: t('admin.brochure.triggering', "Triggering brochure engine...") });
      await apiClient.post('/crm/generate-brochure', { propertyId });
      toast({
        title: t('success'),
        description: t('admin.brochure.success', "AI brochure generation triggered. You will be notified via email."),
      });
    } catch (error) {
                          ...
                          <TableCell className="px-8 text-right">
                             <DropdownMenu>
                               ...
                               <DropdownMenuContent align="end" className="bg-[#14151a] border-border rounded-2xl shadow-2xl p-2 min-w-[180px]">
                                 ...
                                 <DropdownMenuItem onClick={() => handleGenerateBrochure(property.id)} className="rounded-xl px-4 py-3 text-[10px] font-bold text-muted-foreground hover:text-foreground transition-all cursor-pointer">
