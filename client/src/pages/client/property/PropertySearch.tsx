    ...
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }} className="group cursor-pointer flex flex-col md:flex-row gap-6 bg-card border border-border/50 rounded-3xl p-4 shadow-sm hover:shadow-lg transition-all">
      <div className="relative w-full md:w-96 h-64 md:h-full shrink-0 rounded-2xl overflow-hidden bg-muted border border-border/40">
        {displayImages.length > 1 ? (
              ...
              {displayImages.map((img: string, i: number) => (
                <CarouselItem key={i} className="relative w-full h-full pl-0">
                  <Link to={`/properties/${p.id}`} className="block w-full h-full">
                    <img src={img} alt={`${p.name} - ${i + 1}`} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
          ...
          <Link to={`/properties/${p.id}`} className="block w-full h-full">
            <img src={displayImages[0]} alt={p.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
            <div className="absolute inset-0 bg-linear-to-t from-black/60 via-black/0 to-transparent opacity-80" />
          </Link>
        )}
                          ...
                          return (
                            <Badge
                              ...
                              className={cn(
                                ...
                                isSelected 
                                  ? "bg-linear-to-r from-primary to-primary/80 text-primary-foreground border-transparent shadow-md scale-105" 
      ...
      {activeTab === "projects" ? (
        ...
        <div className={cn("flex-1 max-w-[1600px] mx-auto w-full", viewMode === "map" ? "p-0" : "px-6 md:px-12 py-8")}>
          ...
          <div className="flex flex-col items-center justify-center h-96 space-y-4">
            ...
            <h3 className="text-2xl font-black bg-clip-text text-transparent bg-linear-to-r from-foreground to-foreground/60">{t("client.src.no_properties_found_1", "No properties found")}</h3>
          ...
          <div className="flex flex-col gap-12">
            ...
            {groupedSections.map((section) => (
              <div key={section.id} className="flex flex-col gap-6">
                <h2 className="text-3xl font-black bg-clip-text text-transparent bg-linear-to-r from-foreground to-foreground/70">{section.title}</h2>
                   ...
                   <Popup className="rounded-2xl overflow-hidden shadow-lg p-0 border-0" closeButton={false}>
                     <div className="flex flex-col w-[240px] font-sans">
                        <div className="relative h-[140px] w-full">
                          <img src={p.photos?.[0]?.url || p.image || "https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?auto=format&fit=crop&w=800&q=80"} className="w-full h-full object-cover" />
