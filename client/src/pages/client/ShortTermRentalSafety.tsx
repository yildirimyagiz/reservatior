    ...
    <div className="min-h-screen bg-[#14151a]">
      ...
      <main className="max-w-[1400px] mx-auto px-8 lg:px-12 py-24 space-y-32">
            ...
            {OPERATOR_REQUIREMENTS.map((req, idx) => (
              ...
              >
                <div className="bg-[#1a1b1e]/60 backdrop-blur-xl border border-white/5 rounded-3xl p-8 h-full hover:border-white/10 transition-all group border-l border-t">
                  ...
                  <h3 className="text-lg font-black text-white italic tracking-tight mb-3">{req.requirement}</h3>
                  <p className="text-xs font-bold text-slate-400 italic leading-relaxed tracking-wide">{req.description}</p>
                </div>
              </motion.div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {ENFORCEMENT_TIERS.map((tier, idx) => (
              <motion.div
                ...
                variants={fadeUp}
              >
                <div className={`bg-gradient-to-br ${tier.color} backdrop-blur-xl border ${tier.borderColor} rounded-3xl p-8 h-full hover:border-white/10 transition-all border-l border-t`}>
                  <div className="text-center mb-6">
                    <Badge className={`mb-4 ${tier.borderColor} text-white/60 text-[8px] font-black tracking-widest italic`}>
                      STRIKE {idx + 1}
                    </Badge>
                    <h3 className="text-2xl font-black text-white italic tracking-tighter mb-2">{tier.tier}</h3>
                    <p className="text-amber-400 text-lg font-black italic">{tier.penalty}</p>
                    <p className="text-white/40 text-sm font-bold">{tier.amount}</p>
                  </div>
                  <p className="text-xs font-bold text-slate-400 italic leading-relaxed tracking-wide text-center">{tier.description}</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <section>
          ...
          >
            <div className="grid grid-cols-5 items-center px-8 py-5 bg-white/[0.02]">
              <span className="text-[10px] font-black text-slate-500 tracking-widest italic uppercase">Country</span>
              <span className="text-[10px] font-black text-slate-500 tracking-widest italic uppercase text-center">Min Nights</span>
              <span className="text-[10px] font-black text-slate-500 tracking-widest italic uppercase text-center">Peak Nights</span>
              <span className="text-[10px] font-black text-slate-500 tracking-widest italic uppercase text-center">Insurance</span>
              <span className="text-[10px] font-black text-slate-500 tracking-widest italic uppercase text-center">Noise Limit</span>
            </div>
            {REGIONAL_CONFIG.map((config, idx) => (
              <div key={idx} className={`grid grid-cols-5 items-center px-8 py-5 ${idx % 2 === 0 ? "bg-white/[0.01]" : ""} border-t border-white/5 hover:bg-white/[0.03] transition-colors`}>
                <span className="text-xs font-black text-white/80 italic tracking-wide">{config.country}</span>
                <span className="text-xs font-black text-white/60 italic tracking-wide text-center">{config.minNights}</span>
                <span className="text-xs font-black text-white/60 italic tracking-wide text-center">{config.peakNights}</span>
                <span className="text-xs font-black text-white/60 italic tracking-wide text-center">{config.insurance}</span>
                <span className="text-xs font-black text-white/60 italic tracking-wide text-center">{config.noiseLimit}</span>
              </div>
            ))}
          </motion.div>
        </section>
        ...
        >
          <Crown className="w-12 h-12 text-white/20 mx-auto" />
