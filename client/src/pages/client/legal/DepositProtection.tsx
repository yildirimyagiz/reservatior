...
export default function DepositProtection() {
  ...
  const getStatusBadge = (status: DepositRecord['status']) => {
    switch (status) {
      case 'protected':
        return <Badge className="bg-green-100 text-green-800 border-green-200"><ShieldCheck className="w-3 h-3 mr-1" />{t("client.src.protected")}</Badge>;
      case 'pending':
        return <Badge variant="outline" className="text-yellow-600 border-yellow-200 bg-yellow-50"><Calendar className="w-3 h-3 mr-1" />{t("client.src.pending")}</Badge>;
      case 'claimed':
        return <Badge className="bg-orange-100 text-orange-800 border-orange-200">{t("client.src.claimed")}</Badge>;
      case 'returned':
        return <Badge className="bg-blue-100 text-blue-800 border-blue-200">{t("client.src.returned")}</Badge>;
      case 'disputed':
        return <Badge variant="destructive"><ShieldAlert className="w-3 h-3 mr-1" />{t("client.src.disputed")}</Badge>;
    }
  };
  return <PageShell title={t("client.src.deposit_protection")} description={t("client.src.manage_tenancy_deposits_and")}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card className="bg-linear-to-r from-emerald-600 to-indigo-500 text-white shadow-xl border-none">
            <CardContent className="pt-4">
          ...
          <Card className="bg-linear-to-br from-green-50 to-white">
            <CardContent className="pt-4">
          ...
          <Card className="bg-linear-to-br from-red-50 to-white">
            <CardContent className="pt-4">
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
          ...
          <div className="flex gap-2 w-full md:w-auto">
            ...
            <Button className="bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-100"><Plus className="w-4 h-4 mr-2" />{t("client.src.new_protection")}</Button>
          </div>
        </div>
        ...
        <Tabs defaultValue="all" className="w-full">
          <TabsList>
            ...
            <TabsTrigger value="active">{t("client.src.active")}</TabsTrigger>
            <TabsTrigger value="pending">{t("client.src.pending")}</TabsTrigger>
                  ...
                  <CardContent className="p-0">
                    <div className="flex flex-col md:flex-row">
                      <div className="p-6 flex-1">
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground flex items-center">{t("client.src.tenant")}<span className="font-semibold text-foreground ml-1">{deposit.tenant}</span>
                            </p>
                          </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                          <div>
                            <p className="text-muted-foreground flex items-center"><DollarSign className="w-3 h-3 mr-1" />{t("client.src.amount")}</p>
                          ...
                          <div>
                            ...
                            <p className="font-medium text-indigo-600">{deposit.provider} ({deposit.scheme})</p>
                          </div>
                          <div>
                      ...
                      <div className="bg-muted/30 p-4 flex md:flex-col justify-center gap-2 border-t md:border-t-0 md:border-l w-full md:w-48">
                        <Button variant="ghost" size="sm" className="w-full justify-start hover:bg-white">
                          <FileText className="w-4 h-4 mr-2 text-indigo-600" />{t("client.src.certificate")}</Button>
                        <Button variant="ghost" size="sm" className="w-full justify-start hover:bg-white text-orange-600">
        ...
        <div className="bg-indigo-600 rounded-2xl p-6 text-white flex items-center justify-between">
          <div>
            ...
            <p className="text-indigo-100 max-w-xl">{t("client.src.legally_tenancy_deposits_must")}</p>
          </div>
          <Button variant="secondary" className="hidden lg:flex text-indigo-600 font-bold">{t("client.src.learn_more")}</Button>
        </div>
      </div>
    </PageShell>;
}