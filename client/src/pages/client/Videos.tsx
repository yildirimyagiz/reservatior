...
import {
  Play, Pause, Heart, MessageCircle, Share2, Sparkles, Eye,
...
const SAMPLE_VIDEO = "/data/sample.mp4";
...
export default function Videos() {
  ...
  useEffect(() => {
    const el = playingVideoId ? videoRefs.current[playingVideoId] : null;
    if (el) {
      el.currentTime = 0;
      el.play().catch(() => {});
    }
  ...
  return (
    <>
      <Helmet>
        <title>{t('videos.title', 'Real Estate Video Tours')} — Reservatior</title>
        <meta name="description" content="AI-powered cinematic real estate video tours." />
      </Helmet>
      ...
      <div className="h-[calc(100vh-64px)] bg-[#0A0B0F] text-white flex overflow-hidden">
            ...
            return (
              <div key={video.id} className="relative w-full h-full shrink-0 snap-start snap-always flex flex-col">
                ...
                <div className="flex-1 relative overflow-hidden bg-black flex items-center justify-center">
                  {/* Dark background fallback - always visible */}
                  <div className="absolute inset-0 bg-black" />
                  
                   {/* Background Image / Video with dark fallback */}
                   <div className="absolute inset-0 bg-black">
                      {video.videoUrl && playingVideoId === video.id ? (
                        <video ref={(el) => { if (el) videoRefs.current[video.id] = el; }}
                          className="absolute inset-0 w-full h-full object-cover"
                          poster={video.image} src={video.videoUrl}
                          playsInline onClick={() => togglePlay(video.id)}
                        />
                      ) : (
                      ...
                      <div className="absolute inset-0 bg-linear-to-t from-[#0E0F15] via-black/60 to-black/30" />
                  ...
                  <div className="absolute top-5 left-5 z-20 flex items-center gap-3">
                    {playingVideoId === video.id ? (
                      <Badge className="bg-emerald-600 text-white border-0 px-3 py-1.5 text-xs font-bold tracking-wider gap-1.5 rounded-lg shadow-lg shadow-emerald-600/30 animate-pulse">
                        <Play className="w-3 h-3" /> {t('videos.now_playing_badge', 'NOW PLAYING')}
                      </Badge>
                    ) : (
                    ...
                    <Badge className="bg-black/60 text-white border border-white/10 px-3 py-1.5 text-xs font-medium rounded-lg gap-1.5">
                  ...
                  <div className="absolute top-5 right-5 z-20">
                    <div className="bg-black/80 rounded-2xl px-5 py-3 border border-white/10">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none">
                    <button onClick={() => togglePlay(video.id)}
                      className="w-20 h-20 rounded-full bg-black/60 flex items-center justify-center border border-white/20 hover:scale-110 hover:bg-black/80 transition-all shadow-2xl pointer-events-auto">
                      {playingVideoId === video.id ? (
                  ...
                  <div className="absolute bottom-24 left-6 z-20">
                    ...
                    <h2 className="text-3xl lg:text-4xl font-black text-white tracking-tight drop-shadow-2xl">{t(video.rooms[isActive ? activeRoom : 0] || video.rooms[0], (video.rooms[isActive ? activeRoom : 0] || video.rooms[0]).split('.').pop() || "") as string}</h2>
                  </div>
                    ...
                    {video.rooms.map((room: string, i: number) => (
                      <button key={i} onClick={() => setActiveRoom(i)}
                        className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition-all
                          ${(isActive ? activeRoom : 0) === i ? "bg-blue-600 text-white shadow-lg shadow-blue-600/30" : "bg-black/50 text-white/60 hover:bg-black/70"}`}>
                  ...
                  <div className="absolute right-5 top-1/2 -translate-y-1/2 z-20 flex flex-col items-center gap-5">
                    <button onClick={() => setLiked(!liked)} className="flex flex-col items-center gap-1 group">
                      <div className={`w-11 h-11 rounded-full flex items-center justify-center transition-all ${liked ? "bg-red-500 text-white" : "bg-black/50 text-white hover:bg-black/70"}`}>
                        <Heart className={`w-5 h-5 ${liked ? "fill-white" : ""}`} />
                      </div>
                      <span className="text-[10px] text-white/80 font-semibold">3.2K</span>
                    </button>
                    <button className="flex flex-col items-center gap-1 group">
                      <div className="w-11 h-11 rounded-full bg-black/50 text-white flex items-center justify-center hover:bg-black/70 transition-all">
                    ...
                    <button className="flex flex-col items-center gap-1 group mt-4">
                      <div className="w-11 h-11 rounded-full bg-linear-to-br from-violet-600 to-blue-600 text-white flex items-center justify-center hover:scale-110 transition-all shadow-lg shadow-violet-600/30">
        ...
        <div className="w-full lg:w-[420px] bg-[#0E0F15] border-l border-white/10 flex flex-col shrink-0">
              ...
              return (
                <button key={p.id} onClick={() => { 
                  ...
                  <div className="relative w-[140px] h-[90px] rounded-lg overflow-hidden shrink-0 bg-black">
                    <img src={p.image} alt={p.title} className="absolute inset-0 w-full h-full object-cover opacity-90 group-hover:scale-105 transition-transform duration-700" />
                    <div className="absolute inset-0 bg-black/20" />
                    {/* Price overlay */}
                  ...
                  <div className="flex-1 min-w-0 py-0.5 flex flex-col justify-between">
                    ...
                    <div className="space-y-1.5 mt-1">
                      <div className="flex items-center gap-1">
                        <div className="w-4 h-4 rounded-full bg-linear-to-br from-blue-500 to-violet-500 flex items-center justify-center shrink-0">
          ...
          <div className="p-4 border-t border-white/10 shrink-0">
            <div className="flex items-center gap-3 p-3 rounded-xl bg-linear-to-r from-violet-600/10 to-blue-600/10 border border-violet-500/10 cursor-pointer hover:bg-white/10 transition-colors">
              <div className="w-10 h-10 rounded-full bg-linear-to-br from-violet-600 to-blue-600 flex items-center justify-center shrink-0 shadow-lg shadow-violet-600/20">
