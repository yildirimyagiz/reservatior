...
import { ArrowLeft, FileText, Cpu, Shield, Globe, ShieldCheck, Building2, ScrollText } from "lucide-react";
        ...
        delay: 0.2
          ...
          <div className="p-16 rounded-[64px] bg-[#1a1b1e]/40 border border-white/5 backdrop-blur-3xl shadow-3xl space-y-20 border-l border-t relative overflow-hidden">
            ...
            <section className="space-y-10 relative">
              <h2 className="text-sm font-black text-purple-400 tracking-[0.4em] italic flex items-center gap-4">
                <div className="h-px w-8 bg-purple-500/30" />
                <ScrollText className="w-5 h-5" /> {t('authorization')}
              </h2>
              <div className="space-y-6">
                <p className="text-[11px] font-bold text-slate-400 italic leading-loose">
                  {t('authorizationDesc')}
                </p>
                <div className="grid gap-4 p-8 rounded-3xl bg-black/40 border border-white/5">
                  <h4 className="text-[10px] font-black text-purple-400 italic tracking-widest flex items-center gap-3">
                    <Building2 className="w-4 h-4" /> {t('authorizedCountries')}
                  </h4>
                  <p className="text-[10px] font-bold text-slate-500 italic leading-relaxed">
                    {t('authorizedCountriesList')}
                  </p>
                </div>
                <div className="grid md:grid-cols-2 gap-4">
                  {(t('authorizationDetails', { returnObjects: true }) as string[]).map((item, i) => (
                    <div key={i} className="flex items-start gap-4 p-5 rounded-2xl bg-black/30 border border-white/5 hover:border-purple-500/20 transition-all group">
                      <ShieldCheck className="w-4 h-4 text-purple-500/50 mt-0.5 shrink-0" />
                      <span className="text-[10px] font-bold text-slate-400 group-hover:text-slate-300 italic leading-relaxed">{item}</span>
                    </div>
                  ))}
                </div>
              </div>
