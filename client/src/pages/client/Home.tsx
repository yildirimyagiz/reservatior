...
export default function Home() {
  ...
  const [searchMode, setSearchMode] = useState<"STAYS" | "RENT" | "BUY">("STAYS");
  ...
  const slides = useMemo(() => {
    if (!response?.data || !Array.isArray(response.data) || response.data.length === 0) return FALLBACK_SLIDES;
    ...
    <>
      ...
      <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
        ...
        <section className="relative min-h-[92vh] flex items-center overflow-hidden">
          ...
          <div className="w-full max-w-4xl flex flex-col items-center text-center space-y-6 md:space-y-8 mt-[-10vh] pb-32 md:pb-0">
              
              <p className="text-base sm:text-lg md:text-xl text-white/95 font-medium max-w-2xl leading-relaxed drop-shadow-lg">
              ...
              {/* SEARCH TABS */}
              <div className="flex justify-center items-center gap-1 md:gap-4 mb-4 md:mb-6 mt-4 md:mt-6 z-20 relative bg-white/90 dark:bg-black/40 backdrop-blur-md p-1 rounded-full border border-black/5 dark:border-white/10 mx-auto w-fit shadow-sm">
                <button 
                  type="button"
                  onClick={() => setSearchMode("STAYS")}
                  className={`px-3 md:px-6 py-1.5 md:py-2 rounded-full text-xs md:text-base font-bold transition-all whitespace-nowrap ${searchMode === "STAYS" ? "bg-blue-600 text-white shadow-lg" : "text-slate-600 hover:text-slate-900 hover:bg-black/5 dark:text-white/80 dark:hover:text-white dark:hover:bg-white/10"}`}
                >{t('home.search.mode_stays', 'Stays')}</button>
                <button 
                  type="button"
                  onClick={() => setSearchMode("RENT")}
                  className={`px-3 md:px-6 py-1.5 md:py-2 rounded-full text-xs md:text-base font-bold transition-all whitespace-nowrap ${searchMode === "RENT" ? "bg-blue-600 text-white shadow-lg" : "text-slate-600 hover:text-slate-900 hover:bg-black/5 dark:text-white/80 dark:hover:text-white dark:hover:bg-white/10"}`}
                >{t('home.search.mode_rent', 'Rent')}</button>
                <button 
                  type="button"
                  onClick={() => setSearchMode("BUY")}
                  className={`px-3 md:px-6 py-1.5 md:py-2 rounded-full text-xs md:text-base font-bold transition-all whitespace-nowrap ${searchMode === "BUY" ? "bg-blue-600 text-white shadow-lg" : "text-slate-600 hover:text-slate-900 hover:bg-black/5 dark:text-white/80 dark:hover:text-white dark:hover:bg-white/10"}`}
              ...
              <form onSubmit={handleSearch} className="w-full max-w-5xl bg-background/95 dark:bg-black/60 backdrop-blur-3xl border border-border/50 p-2 rounded-3xl md:rounded-full flex flex-col md:flex-row items-center gap-2 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] relative z-20">
                {/* Where Segment */}
                <div className="flex-[1.5] px-4 md:px-6 py-2 w-full md:w-auto hover:bg-blue-50/50 dark:hover:bg-blue-900/20 rounded-full transition-colors text-left flex flex-col justify-center">
              ...
              <div className="flex flex-wrap justify-center gap-2 md:gap-3 pt-4 md:pt-6">
                {[
                  { key: 'home.vibes.beachfront', default: '🏖️ Beachfront', type: 'BEACHFRONT' },
                  { key: 'home.vibes.mansions', default: '🏛️ Mansions', type: 'VILLA' },
                  { key: 'home.vibes.penthouses', default: '🏙️ Penthouses', type: 'PENTHOUSE' },
                  { key: 'home.vibes.mountains', default: '⛰️ Mountains', type: 'MOUNTAIN' },
                  { key: 'home.vibes.modern', default: '📐 Modern', type: 'MODERN' }
                ].map(vibe => (
                  <Badge key={vibe.key} variant="outline" className="px-3 md:px-5 py-1.5 md:py-2.5 rounded-full border-border/30 bg-background/80 dark:bg-black/40 backdrop-blur-xl text-foreground hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all cursor-pointer shadow-lg text-xs md:text-sm font-bold"
                    onClick={() => navigate(`/properties?propertyType=${vibe.type}`)}>
            ...
            <div className="absolute bottom-8 left-4 md:left-6 right-4 md:right-6 flex flex-col md:flex-row justify-between items-end md:items-center gap-4 md:gap-6 z-30 pointer-events-none">
              
              <div className="flex items-center gap-3 md:gap-4 bg-background/80 dark:bg-black/60 backdrop-blur-2xl border border-border/20 rounded-full p-1.5 md:p-2 pr-4 md:pr-6 shadow-2xl pointer-events-auto w-full md:w-auto">
                ...
                <div className="min-w-0">
                  <div className="text-foreground font-bold text-xs md:text-base leading-tight truncate">{slide.title}</div>
                  <div className="text-muted-foreground text-[10px] md:text-sm font-medium truncate">{slide.location} • {slide.price}</div>
                </div>
              </div>
