  ...
  return <section className="py-20 bg-gradient-to-b from-white to-gray-50">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl font-bold mb-4">{t("client.src.trusted_by")}<span className="text-purple-600">{t("client.src.industry_leaders")}</span>
          </h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">{t("client.src.see_how_real_estate")}</p>
        </div>

        {/* Category Filter */}
        <div className="flex justify-center mb-12">
          <div className="inline-flex bg-white rounded-lg shadow-sm border p-1">
            {categories.map(category => <button key={category} onClick={() => setSelectedCategory(category)} className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${selectedCategory === category ? "bg-purple-600 text-white" : "text-gray-600 hover:text-gray-900"}`}>
                {category}
              </button>)}
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-16">
          ...
          <div className="space-y-4">
            {filteredTestimonials.map((testimonial, index) => <Card key={testimonial.id} className={`cursor-pointer transition-all ${activeTestimonial === index ? "ring-2 ring-purple-600 bg-purple-50" : "hover:shadow-md"}`} onClick={() => setActiveTestimonial(index)}>
