...
export function VideoFeed({
  ...
  useEffect(() => {
    const fetchFeed = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:3000"}/api/v1/feed?limit=10`);
...
function VideoSlide({
  ...
  useEffect(() => {
    if (active && videoRef.current) {
      ...
      const timer = setTimeout(() => {
        fetch(`${import.meta.env.VITE_API_URL || "http://localhost:3000"}/api/v1/feed/view/${listing.id}`, {
          method: 'POST'
        });
      }, 2000);
  ...
  const handleLike = async () => {
    ...
    try {
      await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:3000"}/api/v1/feed/like/${listing.id}`, {
        method: 'POST'
      });
    } catch (e) {
