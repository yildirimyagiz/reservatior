  ...
  return <nav className="fixed top-0 inset-x-0 z-50 p-4 flex justify-center">
      ...
      opacity: 1
        ...
        <div className="flex items-center gap-8">
          <Link to="/" className="text-xl font-display font-bold bg-linear-to-r from-blue-400 to-purple-600 bg-clip-text text-transparent">{t("client.src.stageai")}</Link>
          ...
          <div className="flex items-center gap-6">
            ...
            <Link to="/showcase" className={`text-sm font-medium transition-colors ${isActive("/showcase") ? "text-primary" : "text-muted-foreground hover:text-primary"}`}>
              {t("nav.showcase")}
            </Link>
            <Link to="/pricing" className={`text-sm font-medium transition-colors ${isActive("/pricing") ? "text-primary" : "text-muted-foreground hover:text-primary"}`}>
              {t("nav.pricing")}
            </Link>
            <Link to="/studio" className={`text-sm font-medium flex items-center gap-1 transition-colors ${isActive("/studio") ? "text-purple-400" : "text-purple-400/80 hover:text-purple-400"}`}>
      ...
      y: 0
          ...
          {isAuthenticated ? <div className="space-y-2">
              ...
              <Link to="/api-keys" onClick={() => setIsOpen(false)}>
                <span className="text-lg font-medium cursor-pointer flex items-center gap-2">
                  <span className="w-4 h-4 text-center">🔑</span>{t("client.src.api_keys")}</span>
              </Link>
              <Link to="/mobile-devices" onClick={() => setIsOpen(false)}>
                <span className="text-lg font-medium cursor-pointer flex items-center gap-2">
