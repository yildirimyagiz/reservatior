          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="text-xs font-black tracking-wide text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/10 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 uppercase italic">
                <Plus className="w-3.5 h-3.5" /> {t("client.src.create_new")}
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="center" className="w-48 bg-background border-border">
              <DropdownMenuItem asChild className="cursor-pointer">
                <Link to="/host/new-listing?type=SALE" className="flex items-center gap-2">🏡 {t("client.src.for_sale")}</Link>
              </DropdownMenuItem>
              <DropdownMenuItem asChild className="cursor-pointer">
                <Link to="/host/new-listing?type=RENT" className="flex items-center gap-2">🔑 {t("client.src.for_rent")}</Link>
              </DropdownMenuItem>
              <DropdownMenuItem asChild className="cursor-pointer">
                <Link to="/host/new-listing?type=BOOKING" className="flex items-center gap-2">🏨 {t("client.src.short_term_rental", "Short Term Rental")}</Link>
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </nav>
            ...
            <nav className="p-6 space-y-6">
              <ul className="grid gap-1">
                ...
                <div className="pt-4 mt-2 border-t border-border">
                  <h4 className="px-4 text-[10px] font-black tracking-widest text-emerald-500 uppercase italic mb-2">{t("client.src.create_new")}</h4>
                  <MobileNavItem to="/host/new-listing?type=SALE" icon={<Building className="w-5 h-5 text-emerald-500" />} label={t("client.src.for_sale")} onClick={() => setIsMobileMenuOpen(false)} />
                  <MobileNavItem to="/host/new-listing?type=RENT" icon={<Key className="w-5 h-5 text-emerald-500" />} label={t("client.src.for_rent")} onClick={() => setIsMobileMenuOpen(false)} />
                  <MobileNavItem to="/host/new-listing?type=BOOKING" icon={<Building2 className="w-5 h-5 text-emerald-500" />} label={t("client.src.short_term_rental", "Short Term Rental")} onClick={() => setIsMobileMenuOpen(false)} />
                </div>
              </ul>
