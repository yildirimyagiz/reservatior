...
/// Repository for EscrowStatusHistory operations
/// Provides CRUD operations with proper error handling and type safety
class EscrowStatusHistoryRepository {
  final DioClient _dioClient;

  EscrowStatusHistoryRepository(this._dioClient);

  /// Get EscrowStatusHistory by ID
  /// Returns [EscrowStatusHistory] if found, throws [RepositoryException] otherwise
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw RepositoryException(
          message: 'Failed to fetch escrow_status_history',
  ...
  /// Get all escrow_status_histories with pagination and filtering
  /// Returns list of [EscrowStatusHistory] objects
  Future<List<EscrowStatusHistory>> getescrow_status_histories({
    int page = 1,
    int limit = 20,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return data.map((item) => EscrowStatusHistory.fromJson(item)).toList();
      } else {
        throw RepositoryException(
          message: 'Failed to fetch escrow_status_histories',
          code: response.statusCode.toString(),
  ...
  /// Create new EscrowStatusHistory
  /// Returns created [EscrowStatusHistory] object
