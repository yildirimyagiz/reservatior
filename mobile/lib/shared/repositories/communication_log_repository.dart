...
/// Repository for CommunicationLog operations
/// Provides CRUD operations with proper error handling and type safety
class CommunicationLogRepository {
  final DioClient _dioClient;

  CommunicationLogRepository(this._dioClient);

  /// Get CommunicationLog by ID
  /// Returns [CommunicationLog] if found, throws [RepositoryException] otherwise
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw RepositoryException(
          message: 'Failed to fetch communication_log',
  ...
  /// Get all communication_logs with pagination and filtering
  /// Returns list of [CommunicationLog] objects
  Future<List<CommunicationLog>> getcommunication_logs({
    int page = 1,
    int limit = 20,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return data.map((item) => CommunicationLog.fromJson(item)).toList();
      } else {
        throw RepositoryException(
          message: 'Failed to fetch communication_logs',
          code: response.statusCode.toString(),
  ...
  /// Create new CommunicationLog
  /// Returns created [CommunicationLog] object
