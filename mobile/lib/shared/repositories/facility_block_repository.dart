...
/// Repository for FacilityBlock operations
/// Provides CRUD operations with proper error handling and type safety
class FacilityBlockRepository {
  final DioClient _dioClient;

  FacilityBlockRepository(this._dioClient);

  /// Get FacilityBlock by ID
  /// Returns [FacilityBlock] if found, throws [RepositoryException] otherwise
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw RepositoryException(
          message: 'Failed to fetch facility_block',
  ...
  /// Get all facility_blocks with pagination and filtering
  /// Returns list of [FacilityBlock] objects
  Future<List<FacilityBlock>> getfacility_blocks({
    int page = 1,
    int limit = 20,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return data.map((item) => FacilityBlock.fromJson(item)).toList();
      } else {
        throw RepositoryException(
          message: 'Failed to fetch facility_blocks',
          code: response.statusCode.toString(),
  ...
  /// Create new FacilityBlock
  /// Returns created [FacilityBlock] object
