...
/// Repository for IntegrationLog operations
/// Provides CRUD operations with proper error handling and type safety
class IntegrationLogRepository {
  final DioClient _dioClient;

  IntegrationLogRepository(this._dioClient);

  /// Get IntegrationLog by ID
  /// Returns [IntegrationLog] if found, throws [RepositoryException] otherwise
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw RepositoryException(
          message: 'Failed to fetch integration_log',
  ...
  /// Get all integration_logs with pagination and filtering
  /// Returns list of [IntegrationLog] objects
  Future<List<IntegrationLog>> getintegration_logs({
    int page = 1,
    int limit = 20,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return data.map((item) => IntegrationLog.fromJson(item)).toList();
      } else {
        throw RepositoryException(
          message: 'Failed to fetch integration_logs',
          code: response.statusCode.toString(),
  ...
  /// Create new IntegrationLog
  /// Returns created [IntegrationLog] object
