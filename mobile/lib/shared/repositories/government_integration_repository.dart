...
/// Repository for GovernmentIntegration operations
/// Provides CRUD operations with proper error handling and type safety
class GovernmentIntegrationRepository {
  final DioClient _dioClient;

  GovernmentIntegrationRepository(this._dioClient);

  /// Get GovernmentIntegration by ID
  /// Returns [GovernmentIntegration] if found, throws [RepositoryException] otherwise
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw RepositoryException(
          message: 'Failed to fetch government_integration',
  ...
  /// Get all government_integrations with pagination and filtering
  /// Returns list of [GovernmentIntegration] objects
  Future<List<GovernmentIntegration>> getgovernment_integrations({
    int page = 1,
    int limit = 20,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return data.map((item) => GovernmentIntegration.fromJson(item)).toList();
      } else {
        throw RepositoryException(
          message: 'Failed to fetch government_integrations',
          code: response.statusCode.toString(),
  ...
  /// Create new GovernmentIntegration
  /// Returns created [GovernmentIntegration] object
