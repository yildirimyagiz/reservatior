...
// ================================================================
// CommissionRule Admin Page  |  11 fields
// Auto-generated — edit with care
// ================================================================

class CommissionRuleAdminPage extends ConsumerWidget {
  const CommissionRuleAdminPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(commissionRuleLoadingProvider);
    return Scaffold(
      appBar: AppBar(
        title: const Text('Commission Rule Management'),
        ...
        actions: [
          ...
          IconButton(icon: const Icon(Icons.refresh), tooltip: 'Refresh',
              onPressed: () => ref.invalidate(commissionRuleListProvider)),
        ],
      ),
      body: const _CommissionRuleBody(),
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'CommissionRuleFAB',
...
class _CommissionRuleBody extends ConsumerStatefulWidget {
  const _CommissionRuleBody({super.key});
  @override ConsumerState<_CommissionRuleBody> createState() => __CommissionRuleBodyState();
}

class __CommissionRuleBodyState extends ConsumerState<_CommissionRuleBody> {
        ...
        data: (items) {
          ...
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Commission Rules yet',
...
void _showDetail(BuildContext context, CommissionRule item) {
  showDialog(context: context, builder: (ctx) => Dialog(
    child: ConstrainedBox(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Commission Rule Details'),
...
void _showForm(BuildContext context, WidgetRef ref, {CommissionRule? item}) {
  showDialog(context: context, builder: (ctx) => _CommissionRuleForm(item: item, ref: ref));
}

class _CommissionRuleForm extends ConsumerStatefulWidget {
  final CommissionRule? item;
  final WidgetRef ref;
  const _CommissionRuleForm({super.key, this.item, required this.ref});
  @override ConsumerState<_CommissionRuleForm> createState() => __CommissionRuleFormState();
}

class __CommissionRuleFormState extends ConsumerState<_CommissionRuleForm> {
  ...
  void _submit() {
    ...
    if (widget.item == null) {
      widget.ref.read(commissionRuleCreateStateProvider.notifier).state = CommissionRule.fromJson(data);
    } else {
      widget.ref.read(commissionRuleUpdateStateProvider.notifier).state = {
        'id': widget.item!.id,
        'commissionRule': CommissionRule.fromJson({...widget.item!.toJson(), ...data}),
      };
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      child: ConstrainedBox(
        ...
        child: Scaffold(
          appBar: AppBar(
            title: Text(isEdit ? 'Edit Commission Rule' : 'New Commission Rule'),
            automaticallyImplyLeading: false,
            ...
            child: ListView(
              ...
              children: [
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item?.commission?.toString() ?? '',
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item?.minVolume?.toString() ?? '',
                  ...
                  TextFormField(
                    ...
                    keyboardType: TextInputType.number,
                    initialValue: widget.item?.maxVolume?.toString() ?? '',
                ...
                FilledButton.icon(
                  ...
                  label: Text(isEdit ? 'Save Changes' : 'Create Commission Rule'),
...
void _confirmDel(BuildContext context, WidgetRef ref, CommissionRule item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    ...
    actions: [
      ...
      FilledButton.icon(
        onPressed: () {
          ref.read(commissionRuleDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
