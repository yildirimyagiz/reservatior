...
import '../widgets/escrow_status_history_form_widget.dart';

// ── EscrowStatusHistory Client Page

class EscrowStatusHistoryClientPage extends ConsumerStatefulWidget {
  const EscrowStatusHistoryClientPage({super.key});
  @override ConsumerState<EscrowStatusHistoryClientPage> createState() => _EscrowStatusHistoryClientPageState();
}

class _EscrowStatusHistoryClientPageState extends ConsumerState<EscrowStatusHistoryClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Escrow Status Historys'),
        backgroundColor: Colors.teal[700], foregroundColor: Colors.white,
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(escrowStatusHistoryListProvider)),
        ],
      ),
          ...
          data: (items) {
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Escrow Status Historys', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
                ...
                itemBuilder: (ctx, i) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      leading: CircleAvatar(backgroundColor: _stColor(item.fromStatus), foregroundColor: Colors.white, child: Text(item.changedBy != null && item.changedBy!.toString().isNotEmpty ? item.changedBy!.toString()[0].toUpperCase() : '?'),),
      ...
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'EscrowStatusHistoryClientFAB',
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              const Text('Escrow Status History Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
          ...
          Expanded(child: ListView(controller: sc, padding: const EdgeInsets.all(16), children: [
                  ...
                  _row('Changed At', _fmt(item.changedAt), Icons.calendar_today),
          ])),
        ]),
      ),
    );
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Escrow Status History', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            constraints: BoxConstraints(maxHeight: MediaQuery.of(ctx).size.height * 0.7),
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16),
              child: EscrowStatusHistoryFormWidget(
                onSubmit: (newItem) {
                  ref.read(escrowStatusHistoryCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
