...
class MessageDetailWidget extends StatelessWidget {
  final Message item;
  const MessageDetailWidget({super.key, required this.item});
      ...
      if (item.readStatus != null)
        Padding(
          ...
          child: Row(children: [
            ...
            Chip(
              label: Text(item.readStatus!.toString(), style: const TextStyle(fontWeight: FontWeight.w600)),
              backgroundColor: _stColor(item.readStatus).withOpacity(0.15),
            ),
          ]),
        ),
