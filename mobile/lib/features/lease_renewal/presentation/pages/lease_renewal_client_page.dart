...
import '../widgets/lease_renewal_form_widget.dart';

// ── LeaseRenewal Client Page

class LeaseRenewalClientPage extends ConsumerStatefulWidget {
  const LeaseRenewalClientPage({super.key});
  @override ConsumerState<LeaseRenewalClientPage> createState() => _LeaseRenewalClientPageState();
}

class _LeaseRenewalClientPageState extends ConsumerState<LeaseRenewalClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Lease Renewals'),
        backgroundColor: Colors.teal[700], foregroundColor: Colors.white,
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(leaseRenewalListProvider)),
        ],
      ),
          ...
          data: (items) {
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Lease Renewals', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
                ...
                itemBuilder: (ctx, i) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      leading: CircleAvatar(backgroundColor: _stColor(item.status), foregroundColor: Colors.white, child: Text(item.leaseId != null && item.leaseId!.toString().isNotEmpty ? item.leaseId!.toString()[0].toUpperCase() : '?'),),
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              const Text('Lease Renewal Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
          ...
          Expanded(child: ListView(controller: sc, padding: const EdgeInsets.all(16), children: [
                  ...
                  _row('Renewal Date', _fmt(item.renewalDate), Icons.calendar_today),
                  _row('Response Deadline', _fmt(item.responseDeadline), Icons.calendar_today),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Lease Renewal', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            constraints: BoxConstraints(maxHeight: MediaQuery.of(ctx).size.height * 0.7),
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16),
              child: LeaseRenewalFormWidget(
                onSubmit: (newItem) {
                  ref.read(leaseRenewalCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
