...
import '../widgets/immigration_status_check_form_widget.dart';

// ── ImmigrationStatusCheck Client Page

class ImmigrationStatusCheckClientPage extends ConsumerStatefulWidget {
  const ImmigrationStatusCheckClientPage({super.key});
  @override ConsumerState<ImmigrationStatusCheckClientPage> createState() => _ImmigrationStatusCheckClientPageState();
}

class _ImmigrationStatusCheckClientPageState extends ConsumerState<ImmigrationStatusCheckClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Immigration Status Checks'),
        backgroundColor: Colors.teal[700], foregroundColor: Colors.white,
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(immigrationStatusCheckListProvider)),
        ],
      ),
          ...
          data: (items) {
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Immigration Status Checks', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
                ...
                itemBuilder: (ctx, i) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      leading: CircleAvatar(backgroundColor: _stColor(item.checkStatus), foregroundColor: Colors.white, child: Text(item.checkStatus != null && item.checkStatus!.toString().isNotEmpty ? item.checkStatus!.toString()[0].toUpperCase() : '?'),),
      ...
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'ImmigrationStatusCheckClientFAB',
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              const Text('Immigration Status Check Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Immigration Status Check', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            constraints: BoxConstraints(maxHeight: MediaQuery.of(ctx).size.height * 0.7),
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16),
              child: ImmigrationStatusCheckFormWidget(
                onSubmit: (newItem) {
                  ref.read(immigrationStatusCheckCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
