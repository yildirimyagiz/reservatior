...
// ================================================================
// EscrowAccount Admin Page  |  11 fields
// Auto-generated — edit with care
// ================================================================

class EscrowAccountAdminPage extends ConsumerWidget {
  const EscrowAccountAdminPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(escrowAccountLoadingProvider);
    return Scaffold(
      appBar: AppBar(
        title: const Text('Escrow Account Management'),
        ...
        actions: [
          ...
          IconButton(icon: const Icon(Icons.refresh), tooltip: 'Refresh',
              onPressed: () => ref.invalidate(escrowAccountListProvider)),
        ],
      ),
      body: const _EscrowAccountBody(),
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'EscrowAccountFAB',
        onPressed: () => _showForm(context, ref),
        icon: const Icon(Icons.add), label: const Text('New Escrow Account'),
      ),
    );
  }
}
...
class _EscrowAccountBody extends ConsumerStatefulWidget {
  const _EscrowAccountBody();
  @override ConsumerState<_EscrowAccountBody> createState() => __EscrowAccountBodyState();
}

class __EscrowAccountBodyState extends ConsumerState<_EscrowAccountBody> {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(escrowAccountListProvider);
      ...
      Padding(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            hintText: 'Search Escrow Accounts…',
      ...
      Expanded(child: async.when(
        data: (items) {
          final list = _q.isEmpty
              ? items
              : items.where((item) => '${item.orgId?.toString() ?? ''} ${item.reservationId?.toString() ?? ''} ${item.currency?.toString() ?? ''}'.toLowerCase().contains(_q)).toList();
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.inbox_outlined, size: 56, color: Colors.grey[350]),
              const SizedBox(height: 12),
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Escrow Accounts yet',
                  style: TextStyle(color: Colors.grey[500])),
            ]));
          }
          return RefreshIndicator(
            onRefresh: () async => ref.invalidate(escrowAccountListProvider),
        ...
        error: (e, _) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 12),
          ElevatedButton.icon(onPressed: () => ref.invalidate(escrowAccountListProvider),
              icon: const Icon(Icons.refresh), label: const Text('Retry')),
        ])),
      )),
    ]);
  }
  Color _stColor(EscrowAccount item) {
...
void _showDetail(BuildContext context, EscrowAccount item) {
  showDialog(context: context, builder: (ctx) => Dialog(
    child: ConstrainedBox(
      constraints: const BoxConstraints(maxWidth: 500, maxHeight: 640),
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Escrow Account Details'),
        ...
        body: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _row('Held At', _formatDate(item.heldAt), Icons.calendar_today),
...
void _showForm(BuildContext context, WidgetRef ref, {EscrowAccount? item}) {
  showDialog(context: context, builder: (ctx) => _EscrowAccountForm(item: item, ref: ref));
}

class _EscrowAccountForm extends ConsumerStatefulWidget {
  final EscrowAccount? item;
  final WidgetRef ref;
  const _EscrowAccountForm({this.item, required this.ref});
  @override ConsumerState<_EscrowAccountForm> createState() => __EscrowAccountFormState();
}

class __EscrowAccountFormState extends ConsumerState<_EscrowAccountForm> {
  ...
  void _submit() {
    ...
    if (widget.item == null) {
      widget.ref.read(escrowAccountCreateStateProvider.notifier).state = EscrowAccount.fromJson(data);
    } else {
      widget.ref.read(escrowAccountUpdateStateProvider.notifier).state = {
        'id': widget.item!.id,
        'escrowAccount': EscrowAccount.fromJson({...widget.item!.toJson(), ...data}),
      };
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      child: ConstrainedBox(
        ...
        child: Scaffold(
          appBar: AppBar(
            title: Text(isEdit ? 'Edit Escrow Account' : 'New Escrow Account'),
            automaticallyImplyLeading: false,
              ...
              children: [
                  ...
                  InkWell(
                    ...
                    child: InputDecorator(
                      ...
                      child: Text(_heldAt != null ? _formatDate(_heldAt) : 'Tap to select date'),
                    ),
                  ),
                  InkWell(
                    onTap: () async {
                ...
                FilledButton.icon(
                  ...
                  label: Text(isEdit ? 'Save Changes' : 'Create Escrow Account'),
...
void _confirmDel(BuildContext context, WidgetRef ref, EscrowAccount item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    icon: const Icon(Icons.warning_amber_rounded, color: Colors.orange, size: 40),
    title: const Text('Delete Escrow Account?'),
    content: const Text('This action cannot be undone.'),
    actions: [
      TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
      FilledButton.icon(
        onPressed: () {
          ref.read(escrowAccountDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
