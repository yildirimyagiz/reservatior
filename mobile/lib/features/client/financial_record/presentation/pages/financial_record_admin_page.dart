...
import 'package:reservatior/features/client/financial_record/presentation/widgets/financial_record_list_widget.dart';
import 'package:reservatior/features/client/financial_record/presentation/widgets/financial_record_form_widget.dart';
...
class FinancialRecordAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(financialRecordListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_financialrecord_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(financialRecordListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: FinancialRecordListWidget(
                items: filteredData as List<FinancialRecord>,
              ),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(financialRecordListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: FinancialRecordFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(financialRecordCreateProvider.notifier).state = val;
            else
              ref.read(financialRecordUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
