...
class GuestListWidget extends StatelessWidget {
  final List<Guest> items;
  const GuestListWidget({super.key, required this.items});
  ...
  Widget build(BuildContext context) {
    if (items.isEmpty)
      return Center(
        child: Padding(
          ...
          child: Text('mobile.auto.no_guest_items_found'.tr(),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        final item = items[i];
        final heroTag = 'hero_guest_${item.id}';
        String title = 'Item';
        try {
