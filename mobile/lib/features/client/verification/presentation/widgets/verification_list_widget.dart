...
class VerificationListWidget extends StatelessWidget {
  ...
  const VerificationListWidget({super.key, required this.items});
  ...
  Widget build(BuildContext context) {
    if (items.isEmpty)
      return Center(
        child: Padding(
          ...
          child: Text('mobile.auto.no_verification_items_found'.tr(),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        final item = items[i];
        final heroTag = 'hero_verification_${item.id}';
        String title = 'Item';
        try {
