...
import 'package:reservatior/features/client/project_report/presentation/widgets/project_report_list_widget.dart';
import 'package:reservatior/features/client/project_report/presentation/widgets/project_report_form_widget.dart';
...
class ProjectReportAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(projectReportListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_projectreport_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(projectReportListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: ProjectReportListWidget(
                items: filteredData as List<ProjectReport>,
              ),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(projectReportListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: ProjectReportFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(projectReportCreateProvider.notifier).state = val;
            else
              ref.read(projectReportUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
