...
import 'package:reservatior/features/client/document_analysis/presentation/widgets/document_analysis_list_widget.dart';
import 'package:reservatior/features/client/document_analysis/presentation/widgets/document_analysis_form_widget.dart';
...
class DocumentAnalysisAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(documentAnalysisListProvider(orgId));

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_documentanalysis_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async =>
                ref.refresh(documentAnalysisListProvider(orgId)),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: DocumentAnalysisListWidget(
                items: filteredData as List<DocumentAnalysis>,
              ),
            ),
          );
        },
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: DocumentAnalysisFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(documentAnalysisCreateProvider.notifier).state = val;
            else
              ref.read(documentAnalysisUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
