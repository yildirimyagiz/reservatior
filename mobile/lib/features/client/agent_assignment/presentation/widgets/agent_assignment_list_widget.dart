...
class AgentAssignmentListWidget extends StatelessWidget {
  ...
  const AgentAssignmentListWidget({super.key, required this.items});
  ...
  Widget build(BuildContext context) {
    if (items.isEmpty) {
      return Center(
        child: Padding(
          ...
          child: Text('mobile.auto.no_agent_assignment_items_found'.tr(),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        final item = items[i];
        final heroTag = 'hero_agent_assignment_${item.id}';
        String title = 'Item';
        try {
