...
import 'package:reservatior/features/client/guest_profile/presentation/widgets/guest_profile_list_widget.dart';
import 'package:reservatior/features/client/guest_profile/presentation/widgets/guest_profile_form_widget.dart';
...
class GuestProfileAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(guestProfileListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_guestprofile_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(guestProfileListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: GuestProfileListWidget(
                items: filteredData as List<GuestProfile>,
              ),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(guestProfileListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: GuestProfileFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(guestProfileCreateProvider.notifier).state = val;
            else
              ref.read(guestProfileUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
