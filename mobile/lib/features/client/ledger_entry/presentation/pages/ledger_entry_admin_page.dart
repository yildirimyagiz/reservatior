...
import 'package:reservatior/features/client/ledger_entry/presentation/widgets/ledger_entry_list_widget.dart';
import 'package:reservatior/features/client/ledger_entry/presentation/widgets/ledger_entry_form_widget.dart';
...
class LedgerEntryAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(ledgerEntryListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_ledgerentry_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(ledgerEntryListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: LedgerEntryListWidget(
                items: filteredData as List<LedgerEntry>,
              ),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(ledgerEntryListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: LedgerEntryFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(ledgerEntryCreateProvider.notifier).state = val;
            else
              ref.read(ledgerEntryUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
