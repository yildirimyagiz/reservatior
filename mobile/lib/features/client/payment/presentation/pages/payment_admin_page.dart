...
import 'package:reservatior/features/client/payment/presentation/widgets/payment_list_widget.dart';
import 'package:reservatior/features/client/payment/presentation/widgets/payment_form_widget.dart';
...
class PaymentAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(paymentListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_payment_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(paymentListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: PaymentListWidget(items: filteredData as List<Payment>),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(paymentListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: PaymentFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(paymentCreateProvider.notifier).state = val;
            else
              ref.read(paymentUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
