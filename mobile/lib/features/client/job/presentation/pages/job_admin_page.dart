...
import 'package:reservatior/shared/providers/job_provider.dart';
import 'package:reservatior/shared/providers/auth_provider.dart';
import 'package:reservatior/shared/models/models.dart';
import 'package:reservatior/core/theme/app_theme.dart';
import 'package:reservatior/features/client/job/presentation/widgets/job_list_widget.dart';
import 'package:reservatior/features/client/job/presentation/widgets/job_form_widget.dart';
...
class JobAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(jobListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_job_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(jobListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: JobListWidget(items: filteredData as List<Job>),
            ),
          );
        },
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: JobFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(jobCreateProvider.notifier).state = val;
            else
              ref.read(jobUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
