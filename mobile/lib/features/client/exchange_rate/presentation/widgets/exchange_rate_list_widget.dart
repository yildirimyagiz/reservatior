...
class ExchangeRateListWidget extends StatelessWidget {
  ...
  const ExchangeRateListWidget({super.key, required this.items});
  ...
  Widget build(BuildContext context) {
    if (items.isEmpty)
      return Center(
        child: Padding(
          ...
          child: Text('mobile.auto.no_exchange_rate_items_found'.tr(),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        final item = items[i];
        final heroTag = 'hero_exchange_rate_${item.id}';
        String title = 'Item';
        try {
