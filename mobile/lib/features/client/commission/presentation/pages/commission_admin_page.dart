...
import 'package:reservatior/features/client/commission/presentation/widgets/commission_list_widget.dart';
import 'package:reservatior/features/client/commission/presentation/widgets/commission_form_widget.dart';
...
class CommissionAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('mobile.finance.commissionTitle'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(commissionListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: CommissionListWidget(
                items: filteredData as List<Commission>,
              ),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 16),
              Text(
                '${'mobile.finance.commissionError'.tr()}$e',
                style: const TextStyle(color: AppColors.textSecondaryDark),
              ),
              TextButton(
                onPressed: () => ref.refresh(commissionListProvider),
                child: Text('mobile.finance.commissionRetry'.tr()),
              ),
            ],
          ),
        ),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: CommissionFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(commissionCreateProvider.notifier).state = val;
            else
              ref.read(commissionUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
