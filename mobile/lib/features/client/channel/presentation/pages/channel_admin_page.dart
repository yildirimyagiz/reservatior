...
import 'package:reservatior/features/client/channel/presentation/widgets/channel_list_widget.dart';
import 'package:reservatior/features/client/channel/presentation/widgets/channel_form_widget.dart';
...
class ChannelAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(channelListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_channel_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(channelListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: ChannelListWidget(items: filteredData as List<Channel>),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(channelListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: ChannelFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(channelCreateProvider.notifier).state = val;
            else
              ref.read(channelUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
