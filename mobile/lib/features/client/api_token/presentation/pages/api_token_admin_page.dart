...
import 'package:reservatior/features/client/api_token/presentation/widgets/api_token_list_widget.dart';
import 'package:reservatior/features/client/api_token/presentation/widgets/api_token_form_widget.dart';
...
class ApiTokenAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(apiTokenListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_apitoken_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(apiTokenListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: ApiTokenListWidget(items: filteredData as List<ApiToken>),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(apiTokenListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: ApiTokenFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(apiTokenCreateProvider.notifier).state = val;
            else
              ref.read(apiTokenUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
