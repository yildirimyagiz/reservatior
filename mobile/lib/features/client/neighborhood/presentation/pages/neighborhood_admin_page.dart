...
import 'package:reservatior/features/client/neighborhood/presentation/widgets/neighborhood_list_widget.dart';
import 'package:reservatior/features/client/neighborhood/presentation/widgets/neighborhood_form_widget.dart';
...
class NeighborhoodAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(neighborhoodListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_neighborhood_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(neighborhoodListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: NeighborhoodListWidget(
                items: filteredData as List<Neighborhood>,
              ),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(neighborhoodListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: NeighborhoodFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(neighborhoodCreateProvider.notifier).state = val;
            else
              ref.read(neighborhoodUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
