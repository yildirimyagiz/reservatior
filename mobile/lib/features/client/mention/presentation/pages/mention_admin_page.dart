...
import 'package:reservatior/features/client/mention/presentation/widgets/mention_list_widget.dart';
import 'package:reservatior/features/client/mention/presentation/widgets/mention_form_widget.dart';
...
class MentionAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(mentionListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_mention_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(mentionListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: MentionListWidget(items: filteredData as List<Mention>),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(mentionListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: MentionFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(mentionCreateProvider.notifier).state = val;
            else
              ref.read(mentionUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
