...
class DashboardWidgetListWidget extends StatelessWidget {
  ...
  const DashboardWidgetListWidget({super.key, required this.items});
  ...
  Widget build(BuildContext context) {
    if (items.isEmpty)
      return Center(
        child: Padding(
          ...
          child: Text('mobile.auto.no_dashboard_widget_items_found'.tr(),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        final item = items[i];
        final heroTag = 'hero_dashboard_widget_${item.id}';
        String title = 'Item';
        try {
