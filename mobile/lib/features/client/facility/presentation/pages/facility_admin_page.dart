...
import 'package:reservatior/features/client/facility/presentation/widgets/facility_list_widget.dart';
import 'package:reservatior/features/client/facility/presentation/widgets/facility_form_widget.dart';
...
class FacilityAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(facilityListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_facility_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(facilityListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: FacilityListWidget(items: filteredData as List<Facility>),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(facilityListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: FacilityFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(facilityCreateProvider.notifier).state = val;
            else
              ref.read(facilityUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
