...
import 'package:reservatior/features/client/report/presentation/widgets/report_list_widget.dart';
import 'package:reservatior/features/client/report/presentation/widgets/report_form_widget.dart';
...
class ReportAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(reportListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_report_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(reportListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: ReportListWidget(items: filteredData as List<Report>),
            ),
          );
        },
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: ReportFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(reportCreateProvider.notifier).state = val;
            else
              ref.read(reportUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
