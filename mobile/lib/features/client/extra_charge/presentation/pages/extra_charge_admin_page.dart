...
import 'package:reservatior/features/client/extra_charge/presentation/widgets/extra_charge_list_widget.dart';
import 'package:reservatior/features/client/extra_charge/presentation/widgets/extra_charge_form_widget.dart';
...
class ExtraChargeAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(extraChargeListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_extracharge_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(extraChargeListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: ExtraChargeListWidget(
                items: filteredData as List<ExtraCharge>,
              ),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(extraChargeListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: ExtraChargeFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(extraChargeCreateProvider.notifier).state = val;
            else
              ref.read(extraChargeUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
