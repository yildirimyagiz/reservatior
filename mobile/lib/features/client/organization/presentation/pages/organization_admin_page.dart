...
import 'package:reservatior/features/client/organization/presentation/widgets/organization_list_widget.dart';
import 'package:reservatior/features/client/organization/presentation/widgets/organization_form_widget.dart';
...
class OrganizationAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(organizationListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_organization_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(organizationListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: OrganizationListWidget(
                items: filteredData as List<Organization>,
              ),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(organizationListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: OrganizationFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(organizationCreateProvider.notifier).state = val;
            else
              ref.read(organizationUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
