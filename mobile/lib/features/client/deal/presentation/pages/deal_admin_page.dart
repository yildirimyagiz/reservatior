...
import 'package:reservatior/features/client/deal/presentation/widgets/deal_list_widget.dart';
import 'package:reservatior/features/client/deal/presentation/widgets/deal_form_widget.dart';
...
class DealAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(dealListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_deal_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(dealListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: DealListWidget(items: filteredData as List<Deal>),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(dealListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: DealFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(dealCreateProvider.notifier).state = val;
            else
              ref.read(dealUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
