...
import 'package:reservatior/features/client/health_check/presentation/widgets/health_check_list_widget.dart';
import 'package:reservatior/features/client/health_check/presentation/widgets/health_check_form_widget.dart';
...
class HealthCheckAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(healthCheckListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_healthcheck_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(healthCheckListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: HealthCheckListWidget(
                items: filteredData as List<HealthCheck>,
              ),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(healthCheckListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: HealthCheckFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(healthCheckCreateProvider.notifier).state = val;
            else
              ref.read(healthCheckUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
