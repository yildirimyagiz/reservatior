...
class SubscriptionListWidget extends StatelessWidget {
  ...
  const SubscriptionListWidget({super.key, required this.items});
  ...
  Widget build(BuildContext context) {
    if (items.isEmpty)
      return Center(
        child: Padding(
          ...
          child: Text('mobile.auto.no_subscription_items_found'.tr(),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        final item = items[i];
        final heroTag = 'hero_subscription_${item.id}';
        String title = 'Item';
        try {
