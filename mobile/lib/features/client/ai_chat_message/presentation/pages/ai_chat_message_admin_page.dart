...
import 'package:reservatior/shared/providers/ai_chat_message_provider.dart';
import 'package:reservatior/shared/providers/auth_provider.dart';
import 'package:reservatior/shared/models/models.dart';
import 'package:reservatior/core/theme/app_theme.dart';
import 'package:reservatior/features/client/ai_chat_message/presentation/widgets/ai_chat_message_list_widget.dart';
import 'package:reservatior/features/client/ai_chat_message/presentation/widgets/ai_chat_message_form_widget.dart';
...
class AiChatMessageAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(aiChatMessageListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_aichatmessage_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(aiChatMessageListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: AiChatMessageListWidget(
                items: filteredData as List<AiChatMessage>,
              ),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(aiChatMessageListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: AiChatMessageFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(aiChatMessageCreateProvider.notifier).state = val;
            else
              ref.read(aiChatMessageUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
