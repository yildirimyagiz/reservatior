...
import 'package:reservatior/features/client/listing/presentation/widgets/listing_list_widget.dart';
import 'package:reservatior/features/client/listing/presentation/widgets/listing_form_widget.dart';
...
class ListingAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(listingListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_listing_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(listingListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: ListingListWidget(items: filteredData as List<Listing>),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(listingListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: ListingFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(listingCreateProvider.notifier).state = val;
            else
              ref.read(listingUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
