...
class AvailabilityListWidget extends StatelessWidget {
  ...
  const AvailabilityListWidget({super.key, required this.items});
  ...
  Widget build(BuildContext context) {
    if (items.isEmpty)
      return Center(
        child: Padding(
          ...
          child: Text('mobile.auto.no_availability_items_found'.tr(),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        final item = items[i];
        final heroTag = 'hero_availability_${item.id}';
        String title = 'Item';
        try {
