...
import 'package:reservatior/features/client/agent/presentation/widgets/agent_list_widget.dart';
import 'package:reservatior/features/client/agent/presentation/widgets/agent_form_widget.dart';
...
class AgentAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(agentListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_agent_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(agentListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: AgentListWidget(items: filteredData as List<Agent>),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(agentListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: AgentFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(agentCreateProvider.notifier).state = val;
            else
              ref.read(agentUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
