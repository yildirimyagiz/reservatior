...
import 'package:reservatior/shared/providers/photo_provider.dart';
import 'package:reservatior/shared/providers/auth_provider.dart';
import 'package:reservatior/shared/models/models.dart';
import 'package:reservatior/core/theme/app_theme.dart';
import 'package:reservatior/features/client/photo/presentation/widgets/photo_list_widget.dart';
import 'package:reservatior/features/client/photo/presentation/widgets/photo_form_widget.dart';
...
class PhotoAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(photoListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_photo_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(photoListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: PhotoListWidget(items: filteredData as List<Photo>),
            ),
          );
        },
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: PhotoFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(photoCreateProvider.notifier).state = val;
            else
              ref.read(photoUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
