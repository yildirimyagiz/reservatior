...
import 'package:reservatior/features/client/signature_request/presentation/widgets/signature_request_list_widget.dart';
import 'package:reservatior/features/client/signature_request/presentation/widgets/signature_request_form_widget.dart';
...
class SignatureRequestAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(signatureRequestListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_signaturerequest_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(signatureRequestListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: SignatureRequestListWidget(
                items: filteredData as List<SignatureRequest>,
              ),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(signatureRequestListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: SignatureRequestFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(signatureRequestCreateProvider.notifier).state = val;
            else
              ref.read(signatureRequestUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
