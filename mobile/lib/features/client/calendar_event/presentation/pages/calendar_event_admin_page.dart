...
import 'package:reservatior/features/client/calendar_event/presentation/widgets/calendar_event_list_widget.dart';
import 'package:reservatior/features/client/calendar_event/presentation/widgets/calendar_event_form_widget.dart';
...
class CalendarEventAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(calendarEventListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_calendarevent_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(calendarEventListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: CalendarEventListWidget(
                items: filteredData as List<CalendarEvent>,
              ),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(calendarEventListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: CalendarEventFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(calendarEventCreateProvider.notifier).state = val;
            else
              ref.read(calendarEventUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
