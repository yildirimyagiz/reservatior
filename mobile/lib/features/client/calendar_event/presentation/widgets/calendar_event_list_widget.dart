...
class CalendarEventListWidget extends StatelessWidget {
  ...
  const CalendarEventListWidget({super.key, required this.items});
  ...
  Widget build(BuildContext context) {
    if (items.isEmpty)
      return Center(
        child: Padding(
          ...
          child: Text('mobile.auto.no_calendar_event_items_found'.tr(),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        final item = items[i];
        final heroTag = 'hero_calendar_event_${item.id}';
        String title = 'Item';
        try {
