...
import 'package:reservatior/features/client/lead/presentation/widgets/lead_list_widget.dart';
import 'package:reservatior/features/client/lead/presentation/widgets/lead_form_widget.dart';
...
class LeadAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(leadListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_lead_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(leadListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: LeadListWidget(items: filteredData as List<Lead>),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(leadListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: LeadFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(leadCreateProvider.notifier).state = val;
            else
              ref.read(leadUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
