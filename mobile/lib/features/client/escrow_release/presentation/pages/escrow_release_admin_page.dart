...
import 'package:reservatior/features/client/escrow_release/presentation/widgets/escrow_release_list_widget.dart';
import 'package:reservatior/features/client/escrow_release/presentation/widgets/escrow_release_form_widget.dart';
...
class EscrowReleaseAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(escrowReleaseListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_escrowrelease_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(escrowReleaseListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: EscrowReleaseListWidget(
                items: filteredData as List<EscrowRelease>,
              ),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(escrowReleaseListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: EscrowReleaseFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(escrowReleaseCreateProvider.notifier).state = val;
            else
              ref.read(escrowReleaseUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
