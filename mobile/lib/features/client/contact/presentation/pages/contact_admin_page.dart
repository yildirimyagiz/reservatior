...
import 'package:reservatior/features/client/contact/presentation/widgets/contact_list_widget.dart';
import 'package:reservatior/features/client/contact/presentation/widgets/contact_form_widget.dart';
...
class ContactAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final itemsAsync = ref.watch(contactListProvider);

    return Scaffold(
      backgroundColor: AppColors.darkBg,
      appBar: AppBar(
        title: Text('mobile.auto.feature_contact_title'.tr()),
      ...
      body: itemsAsync.when(
        data: (data) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.refresh(contactListProvider),
            child: SingleChildScrollView(
              padding: const EdgeInsets.symmetric(vertical: 16),
              child: ContactListWidget(items: filteredData as List<Contact>),
            ),
          );
        },
        ...
        error: (e, s) => Center(
          child: Column(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => ref.refresh(contactListProvider),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: ContactFormWidget(
          ...
          onSubmit: (val) {
            if (item == null)
              ref.read(contactCreateProvider.notifier).state = val;
            else
              ref.read(contactUpdateProvider.notifier).state = {
                'id': item.id,
                'data': val,
              };
            Navigator.pop(ctx);
