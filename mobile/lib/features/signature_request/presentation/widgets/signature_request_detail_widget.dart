...
// ── SignatureRequest Detail Widget  |  11 fields

class SignatureRequestDetailWidget extends StatelessWidget {
  final SignatureRequest item;
  const SignatureRequestDetailWidget({super.key, required this.item});
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      if (item.status != null)
        ...
        _row('Sign Url', item.signUrl?.toString() ?? 'N/A', Icons.text_fields),
        _row('Signed Document Url', item.signedDocumentUrl?.toString() ?? 'N/A', Icons.text_fields),
