...
// ── SolicitorManagement Detail Widget  |  14 fields

class SolicitorManagementDetailWidget extends StatelessWidget {
  final SolicitorManagement item;
  const SolicitorManagementDetailWidget({super.key, required this.item});
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      if (item.status != null)
        ...
        _row('Solicitor Type', item.solicitorType?.toString() ?? 'N/A', Icons.text_fields),
        _row('Status', item.status?.toString() ?? 'N/A', Icons.info_outline),
        _row('Engaged At', _fmt(item.engagedAt), Icons.calendar_today),
        _row('Completed At', _fmt(item.completedAt), Icons.calendar_today),
        _row('Fee', item.fee?.toString() ?? 'N/A', Icons.attach_money),
