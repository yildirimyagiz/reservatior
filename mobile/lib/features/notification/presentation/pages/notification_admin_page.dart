...
// ================================================================
// Notification Admin Page  |  15 fields
// Auto-generated — edit with care
// ================================================================

class NotificationAdminPage extends ConsumerWidget {
  const NotificationAdminPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(notificationLoadingProvider);
    return Scaffold(
      appBar: AppBar(
      ...
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'NotificationFAB',
        onPressed: () => _showForm(context, ref),
        icon: const Icon(Icons.add), label: const Text('New Notification'),
      ),
    );
  }
}
...
class _NotificationBody extends ConsumerStatefulWidget {
  const _NotificationBody({super.key});
  @override ConsumerState<_NotificationBody> createState() => __NotificationBodyState();
}

class __NotificationBodyState extends ConsumerState<_NotificationBody> {
      ...
      Expanded(child: async.when(
        data: (items) {
          final list = _q.isEmpty
              ? items
              : items.where((item) => ((item.orgId?.toString() ?? '') + " " + (item.userId?.toString() ?? '') + " " + (item.title?.toString() ?? '') + " " + (item.body?.toString() ?? '') + " " + (item.ruleKey?.toString() ?? '')).toLowerCase().contains(_q)).toList();
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.inbox_outlined, size: 56, color: Colors.grey[350]),
              const SizedBox(height: 12),
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Notifications yet',
        ...
        error: (e, _) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 12),
          ElevatedButton.icon(onPressed: () => ref.invalidate(notificationListProvider),
              icon: const Icon(Icons.refresh), label: const Text('Retry')),
        ])),
      )),
    ]);
  }
  Color _stColor(Notification item) {
...
void _showDetail(BuildContext context, Notification item) {
      ...
      child: Scaffold(
        ...
        body: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _row('Deliveries', item.deliveries?.toString() ?? 'N/A', Icons.text_fields),
              _row('Rule Key', item.ruleKey?.toString() ?? 'N/A', Icons.text_fields),
              _row('Rule Config', item.ruleConfig?.toString() ?? 'N/A', Icons.text_fields),
...
void _showForm(BuildContext context, WidgetRef ref, {Notification? item}) {
  showDialog(context: context, builder: (ctx) => _NotificationForm(item: item, ref: ref));
}

class _NotificationForm extends ConsumerStatefulWidget {
  final Notification? item;
  final WidgetRef ref;
  const _NotificationForm({super.key, this.item, required this.ref});
  @override ConsumerState<_NotificationForm> createState() => __NotificationFormState();
}

class __NotificationFormState extends ConsumerState<_NotificationForm> {
  ...
  void _submit() {
    ...
    final data = <String, dynamic>{
      ...
      if (_deliveries?.isNotEmpty == true) 'deliveries': _deliveries,
      if (_ruleKey?.isNotEmpty == true) 'ruleKey': _ruleKey,
      if (_ruleConfig?.isNotEmpty == true) 'ruleConfig': _ruleConfig,
    };
    if (widget.item == null) {
      widget.ref.read(notificationCreateStateProvider.notifier).state = Notification.fromJson(data);
    } else {
      widget.ref.read(notificationUpdateStateProvider.notifier).state = {
        'id': widget.item!.id,
        'notification': Notification.fromJson({...widget.item!.toJson(), ...data}),
      };
    }
            ...
            child: ListView(
              ...
              children: [
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item.userPreferences?.toString() ?? '',
                    maxLines: 1,
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item.deliveries?.toString() ?? '',
                    maxLines: 1,
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item.ruleKey?.toString() ?? '',
                    maxLines: 1,
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item.ruleConfig?.toString() ?? '',
                    maxLines: 1,
                ...
                FilledButton.icon(
                  ...
                  label: Text(isEdit ? 'Save Changes' : 'Create Notification'),
...
void _confirmDel(BuildContext context, WidgetRef ref, Notification item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    ...
    actions: [
      ...
      FilledButton.icon(
        onPressed: () {
          ref.read(notificationDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
