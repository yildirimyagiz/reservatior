...
import '../widgets/escrow_release_form_widget.dart';

// ── EscrowRelease Client Page

class EscrowReleaseClientPage extends ConsumerStatefulWidget {
  const EscrowReleaseClientPage({super.key});
  @override ConsumerState<EscrowReleaseClientPage> createState() => _EscrowReleaseClientPageState();
}

class _EscrowReleaseClientPageState extends ConsumerState<EscrowReleaseClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Escrow Releases'),
        backgroundColor: Colors.teal[700], foregroundColor: Colors.white,
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(escrowReleaseListProvider)),
        ],
      ),
          ...
          data: (items) {
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Escrow Releases', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
      ...
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'EscrowReleaseClientFAB',
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              const Text('Escrow Release Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
          ...
          Expanded(child: ListView(controller: sc, padding: const EdgeInsets.all(16), children: [
                  ...
                  _row('Approval Completed At', _fmt(item.approvalCompletedAt), Icons.calendar_today),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Escrow Release', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            constraints: BoxConstraints(maxHeight: MediaQuery.of(ctx).size.height * 0.7),
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16),
              child: EscrowReleaseFormWidget(
                onSubmit: (newItem) {
                  ref.read(escrowReleaseCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
