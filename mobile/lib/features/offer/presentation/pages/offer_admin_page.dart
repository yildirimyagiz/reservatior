...
// ================================================================
// Offer Admin Page  |  16 fields
// Auto-generated — edit with care
// ================================================================

class OfferAdminPage extends ConsumerWidget {
  const OfferAdminPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(offerLoadingProvider);
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(icon: const Icon(Icons.refresh), tooltip: 'Refresh',
              onPressed: () => ref.invalidate(offerListProvider)),
        ],
      ),
      body: const _OfferBody(),
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'OfferFAB',
        onPressed: () => _showForm(context, ref),
        icon: const Icon(Icons.add), label: const Text('New Offer'),
      ),
    );
  }
}
...
class _OfferBody extends ConsumerStatefulWidget {
  const _OfferBody({super.key});
  @override ConsumerState<_OfferBody> createState() => __OfferBodyState();
}

class __OfferBodyState extends ConsumerState<_OfferBody> {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(offerListProvider);
      ...
      Expanded(child: async.when(
        data: (items) {
          final list = _q.isEmpty
              ? items
              : items.where((item) => ((item.increaseId?.toString() ?? '') + " " + (item.guestId?.toString() ?? '') + " " + (item.specialRequirements?.toString() ?? '') + " " + (item.notes?.toString() ?? '') + " " + (item.reservationId?.toString() ?? '') + " " + (item.propertyId?.toString() ?? '')).toLowerCase().contains(_q)).toList();
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.inbox_outlined, size: 56, color: Colors.grey[350]),
              const SizedBox(height: 12),
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Offers yet',
                  style: TextStyle(color: Colors.grey[500])),
            ]));
          }
          return RefreshIndicator(
            onRefresh: () async => ref.invalidate(offerListProvider),
              ...
              itemBuilder: (ctx, i) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    leading: CircleAvatar(backgroundColor: _stColor(item), foregroundColor: Colors.white, child: Text(item.specialRequirements != null && item.specialRequirements!.toString().isNotEmpty ? item.specialRequirements!.toString()[0].toUpperCase() : '?'),),
                    title: Text(item.specialRequirements ?? 'Unknown', style: const TextStyle(fontWeight: FontWeight.w600)),
        ...
        error: (e, _) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 12),
          ElevatedButton.icon(onPressed: () => ref.invalidate(offerListProvider),
              icon: const Icon(Icons.refresh), label: const Text('Retry')),
        ])),
      )),
    ]);
  }
  Color _stColor(Offer item) {
...
void _showDetail(BuildContext context, Offer item) {
      ...
      child: Scaffold(
        ...
        body: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _row('Increase Id', item.increaseId?.toString() ?? 'N/A', Icons.link),
              _row('Id', item.id?.toString() ?? 'N/A', Icons.tag),
              _row('Offer Type', item.offerType?.toString() ?? 'N/A', Icons.text_fields),
              _row('Status', item.status?.toString() ?? 'N/A', Icons.info_outline),
              _row('Base Price', item.basePrice?.toString() ?? 'N/A', Icons.attach_money),
              _row('Discount Rate', item.discountRate?.toString() ?? 'N/A', Icons.attach_money),
              _row('Final Price', item.finalPrice?.toString() ?? 'N/A', Icons.attach_money),
              ...
              _row('Special Requirements', item.specialRequirements?.toString() ?? 'N/A', Icons.text_fields),
...
void _showForm(BuildContext context, WidgetRef ref, {Offer? item}) {
  showDialog(context: context, builder: (ctx) => _OfferForm(item: item, ref: ref));
}

class _OfferForm extends ConsumerStatefulWidget {
  final Offer? item;
  final WidgetRef ref;
  const _OfferForm({super.key, this.item, required this.ref});
  @override ConsumerState<_OfferForm> createState() => __OfferFormState();
}

class __OfferFormState extends ConsumerState<_OfferForm> {
  ...
  void _submit() {
    ...
    if (widget.item == null) {
      widget.ref.read(offerCreateStateProvider.notifier).state = Offer.fromJson(data);
    } else {
      widget.ref.read(offerUpdateStateProvider.notifier).state = {
        'id': widget.item!.id,
        'offer': Offer.fromJson({...widget.item!.toJson(), ...data}),
      };
    }
            ...
            child: ListView(
              ...
              children: [
                  TextFormField(
                    ...
                    initialValue: widget.item.increaseId?.toString() ?? '',
                    maxLines: 1,
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item.offerType?.toString() ?? '',
                    maxLines: 1,
                  ...
                  TextFormField(
                    ...
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    initialValue: widget.item.discountRate?.toString() ?? '',
                  ...
                  TextFormField(
                    ...
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    initialValue: widget.item.finalPrice?.toString() ?? '',
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item.specialRequirements?.toString() ?? '',
                    maxLines: 1,
                ...
                FilledButton.icon(
                  ...
                  label: Text(isEdit ? 'Save Changes' : 'Create Offer'),
...
void _confirmDel(BuildContext context, WidgetRef ref, Offer item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    ...
    actions: [
      ...
      FilledButton.icon(
        onPressed: () {
          ref.read(offerDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
