...
import '../../../../shared/providers/ai_chat_handoff_provider.dart';
import '../../../../gen_models/models_library.dart';

// ================================================================
// AIChatHandoff Admin Page  |  9 fields
// Auto-generated — edit with care
// ================================================================

class AIChatHandoffAdminPage extends ConsumerWidget {
  const AIChatHandoffAdminPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(aiChatHandoffLoadingProvider);
    return Scaffold(
      appBar: AppBar(
        title: const Text('Ai Chat Handoff Management'),
        ...
        actions: [
          ...
          IconButton(icon: const Icon(Icons.refresh), tooltip: 'Refresh',
              onPressed: () => ref.invalidate(aiChatHandoffListProvider)),
        ],
      ),
      body: const _AIChatHandoffBody(),
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'AIChatHandoffFAB',
        onPressed: () => _showForm(context, ref),
        icon: const Icon(Icons.add), label: const Text('New Ai Chat Handoff'),
      ),
    );
  }
}
...
class _AIChatHandoffBody extends ConsumerStatefulWidget {
  const _AIChatHandoffBody();
  @override ConsumerState<_AIChatHandoffBody> createState() => __AIChatHandoffBodyState();
}

class __AIChatHandoffBodyState extends ConsumerState<_AIChatHandoffBody> {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(aiChatHandoffListProvider);
      ...
      Padding(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            hintText: 'Search Ai Chat Handoffs…',
      ...
      Expanded(child: async.when(
        data: (items) {
          final list = _q.isEmpty
              ? items
              : items.where((AIChatHandoff item) {
                  final searchText = [
                    item.orgId ?? '',
                    item.sessionId ?? '',
                    item.handoffReason ?? '',
          ...
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Ai Chat Handoffs yet',
                  style: TextStyle(color: Colors.grey[500])),
            ]));
          }
          return RefreshIndicator(
            onRefresh: () async => ref.invalidate(aiChatHandoffListProvider),
        ...
        error: (e, _) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 12),
          ElevatedButton.icon(onPressed: () => ref.invalidate(aiChatHandoffListProvider),
...
void _showDetail(BuildContext context, AIChatHandoff item) {
  showDialog(context: context, builder: (ctx) => Dialog(
    child: ConstrainedBox(
      constraints: const BoxConstraints(maxWidth: 500, maxHeight: 640),
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Ai Chat Handoff Details'),
        ...
        body: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _row('Handoff At', _formatDate(item.handoffAt), Icons.calendar_today),
...
void _showForm(BuildContext context, WidgetRef ref, {AIChatHandoff? item}) {
  showDialog(context: context, builder: (ctx) => _AIChatHandoffForm(item: item, ref: ref));
}

class _AIChatHandoffForm extends ConsumerStatefulWidget {
  final AIChatHandoff? item;
  final WidgetRef ref;
  const _AIChatHandoffForm({this.item, required this.ref});
  @override ConsumerState<_AIChatHandoffForm> createState() => __AIChatHandoffFormState();
}

class __AIChatHandoffFormState extends ConsumerState<_AIChatHandoffForm> {
  ...
  void _submit() {
    ...
    if (widget.item == null) {
      widget.ref.read(aiChatHandoffCreateStateProvider.notifier).state = AIChatHandoff.fromJson(data);
    } else {
      widget.ref.read(aiChatHandoffUpdateStateProvider.notifier).state = {
        'id': widget.item!.id,
        'aiChatHandoff': AIChatHandoff.fromJson({...widget.item!.toJson(), ...data}),
      };
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      child: ConstrainedBox(
        ...
        child: Scaffold(
          appBar: AppBar(
            title: Text(isEdit ? 'Edit Ai Chat Handoff' : 'New Ai Chat Handoff'),
            automaticallyImplyLeading: false,
            ...
            child: ListView(
              ...
              children: [
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item?.handoffReason?.toString() ?? '',
                    maxLines: 1,
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item?.handoffTo?.toString() ?? '',
                    maxLines: 1,
                  ...
                  InkWell(
                    ...
                    child: InputDecorator(
                      decoration: const InputDecoration(
                        labelText: 'Handoff At',
                        prefixIcon: Icon(Icons.calendar_today),
                        border: OutlineInputBorder(),
                      ),
                      child: Text(_handoffAt != null ? _formatDate(_handoffAt) : 'Tap to select date'),
                    ),
                  ),
                  InkWell(
                    onTap: () async {
                ...
                FilledButton.icon(
                  ...
                  label: Text(isEdit ? 'Save Changes' : 'Create Ai Chat Handoff'),
...
void _confirmDel(BuildContext context, WidgetRef ref, AIChatHandoff item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    icon: const Icon(Icons.warning_amber_rounded, color: Colors.orange, size: 40),
    title: const Text('Delete Ai Chat Handoff?'),
    content: const Text('This action cannot be undone.'),
    actions: [
      TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
      FilledButton.icon(
        onPressed: () {
          ref.read(aiChatHandoffDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
