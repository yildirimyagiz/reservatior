...
class ExpenseAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(expenseLoadingProvider);
    return Scaffold(
      appBar: AppBar(
      ...
      body: const _ExpenseBody(),
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'ExpenseFAB',
        onPressed: () => _showForm(context, ref),
        icon: const Icon(Icons.add), label: const Text('New Expense'),
      ),
    );
  }
}
...
class _ExpenseBody extends ConsumerStatefulWidget {
  const _ExpenseBody({super.key});
  @override ConsumerState<_ExpenseBody> createState() => __ExpenseBodyState();
}

class __ExpenseBodyState extends ConsumerState<_ExpenseBody> {
      ...
      Expanded(child: async.when(
        data: (items) {
          final list = _q.isEmpty
              ? items
              : items.where((item) => ((item.propertyId?.toString() ?? '') + " " + (item.tenantId?.toString() ?? '') + " " + (item.agencyId?.toString() ?? '') + " " + (item.currencyId?.toString() ?? '') + " " + (item.notes?.toString() ?? '') + " " + (item.facilityId?.toString() ?? '') + " " + (item.includedServiceId?.toString() ?? '') + " " + (item.extraChargeId?.toString() ?? '')).toLowerCase().contains(_q)).toList();
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.inbox_outlined, size: 56, color: Colors.grey[350]),
              const SizedBox(height: 12),
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Expenses yet',
        ...
        error: (e, _) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 12),
          ElevatedButton.icon(onPressed: () => ref.invalidate(expenseListProvider),
              icon: const Icon(Icons.refresh), label: const Text('Retry')),
        ])),
      )),
    ]);
  }
  Color _stColor(Expense item) {
...
void _showDetail(BuildContext context, Expense item) {
      ...
      child: Scaffold(
        ...
        body: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _row('Paid Date', _formatDate(item.paidDate), Icons.calendar_today),
...
void _showForm(BuildContext context, WidgetRef ref, {Expense? item}) {
  showDialog(context: context, builder: (ctx) => _ExpenseForm(item: item, ref: ref));
}

class _ExpenseForm extends ConsumerStatefulWidget {
  final Expense? item;
  final WidgetRef ref;
  const _ExpenseForm({super.key, this.item, required this.ref});
  @override ConsumerState<_ExpenseForm> createState() => __ExpenseFormState();
}

class __ExpenseFormState extends ConsumerState<_ExpenseForm> {
  ...
  void _submit() {
    ...
    if (widget.item == null) {
      widget.ref.read(expenseCreateStateProvider.notifier).state = Expense.fromJson(data);
    } else {
      widget.ref.read(expenseUpdateStateProvider.notifier).state = {
        'id': widget.item!.id,
        'expense': Expense.fromJson({...widget.item!.toJson(), ...data}),
      };
    }
              ...
              children: [
                  ...
                  InkWell(
                    ...
                    child: InputDecorator(
                      ...
                      child: Text(_paidDate != null ? _formatDate(_paidDate) : 'Tap to select date'),
                    ),
                  ),
                  TextFormField(
                ...
                FilledButton.icon(
                  ...
                  label: Text(isEdit ? 'Save Changes' : 'Create Expense'),
...
void _confirmDel(BuildContext context, WidgetRef ref, Expense item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    ...
    actions: [
      ...
      FilledButton.icon(
        onPressed: () {
          ref.read(expenseDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
