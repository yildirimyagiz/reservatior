...
import '../widgets/contract_form_widget.dart';

// ── Contract Client Page

class ContractClientPage extends ConsumerStatefulWidget {
  const ContractClientPage({super.key});
  @override ConsumerState<ContractClientPage> createState() => _ContractClientPageState();
}

class _ContractClientPageState extends ConsumerState<ContractClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(contractListProvider)),
        ],
      ),
          ...
          data: (items) {
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Contracts', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Expanded(child: ListView(controller: sc, padding: const EdgeInsets.all(16), children: [
                  ...
                  _row('Effective From', _fmt(item.effectiveFrom), Icons.calendar_today),
                  _row('Effective To', _fmt(item.effectiveTo), Icons.calendar_today),
                  _row('Next Renewal At', _fmt(item.nextRenewalAt), Icons.calendar_today),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Contract', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            ...
            child: SingleChildScrollView(
              ...
              child: ContractFormWidget(
                onSubmit: (newItem) {
                  ref.read(contractCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
