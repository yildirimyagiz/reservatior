...
// ================================================================
// ContractVersion Admin Page  |  9 fields
// Auto-generated — edit with care
// ================================================================

class ContractVersionAdminPage extends ConsumerWidget {
  const ContractVersionAdminPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(contractVersionLoadingProvider);
    return Scaffold(
      appBar: AppBar(
        title: const Text('Contract Version Management'),
        ...
        actions: [
          ...
          IconButton(icon: const Icon(Icons.refresh), tooltip: 'Refresh',
              onPressed: () => ref.invalidate(contractVersionListProvider)),
        ],
      ),
      body: const _ContractVersionBody(),
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'ContractVersionFAB',
...
class _ContractVersionBody extends ConsumerStatefulWidget {
  const _ContractVersionBody();
  @override ConsumerState<_ContractVersionBody> createState() => __ContractVersionBodyState();
}

class __ContractVersionBodyState extends ConsumerState<_ContractVersionBody> {
        ...
        data: (items) {
          ...
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Contract Versions yet',
...
void _showDetail(BuildContext context, ContractVersion item) {
  showDialog(context: context, builder: (ctx) => Dialog(
    child: ConstrainedBox(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Contract Version Details'),
...
void _showForm(BuildContext context, WidgetRef ref, {ContractVersion? item}) {
  showDialog(context: context, builder: (ctx) => _ContractVersionForm(item: item, ref: ref));
}

class _ContractVersionForm extends ConsumerStatefulWidget {
  final ContractVersion? item;
  final WidgetRef ref;
  const _ContractVersionForm({this.item, required this.ref});
  @override ConsumerState<_ContractVersionForm> createState() => __ContractVersionFormState();
}

class __ContractVersionFormState extends ConsumerState<_ContractVersionForm> {
  ...
  void _submit() {
    ...
    if (widget.item == null) {
      widget.ref.read(contractVersionCreateStateProvider.notifier).state = ContractVersion.fromJson(data);
    } else {
      widget.ref.read(contractVersionUpdateStateProvider.notifier).state = {
        'id': widget.item!.id,
        'contractVersion': ContractVersion.fromJson({...widget.item!.toJson(), ...data}),
      };
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      child: ConstrainedBox(
        ...
        child: Scaffold(
          appBar: AppBar(
            title: Text(isEdit ? 'Edit Contract Version' : 'New Contract Version'),
            automaticallyImplyLeading: false,
          ...
          body: Form(
            ...
            child: ListView(
              ...
              children: [
                  TextFormField(
                    decoration: const InputDecoration(labelText: 'Contract Id', prefixIcon: Icon(Icons.link), border: OutlineInputBorder()),
                  ...
                  TextFormField(
                    decoration: const InputDecoration(labelText: 'Document Url', prefixIcon: Icon(Icons.link), border: OutlineInputBorder()),
                    initialValue: widget.item?.documentUrl?.toString() ?? '',
                    maxLines: 1,
                    onSaved: (v) => _documentUrl = v?.isEmpty == true ? null : v,
                  ),
                  TextFormField(
                    decoration: const InputDecoration(labelText: 'Checksum', prefixIcon: Icon(Icons.text_fields), border: OutlineInputBorder()),
                ...
                FilledButton.icon(
                  ...
                  label: Text(isEdit ? 'Save Changes' : 'Create Contract Version'),
...
void _confirmDel(BuildContext context, WidgetRef ref, ContractVersion item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    icon: const Icon(Icons.warning_amber_rounded, color: Colors.orange, size: 40),
    title: const Text('Delete Contract Version?'),
    content: const Text('This action cannot be undone.'),
    actions: [
      TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
      FilledButton.icon(
        onPressed: () {
          ref.read(contractVersionDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
