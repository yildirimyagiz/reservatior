...
// ================================================================
// DashboardConfiguration Admin Page  |  12 fields
...
class DashboardConfigurationAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(dashboardConfigurationLoadingProvider);
    return Scaffold(
      appBar: AppBar(
        title: const Text('Dashboard Configuration Management'),
        ...
        actions: [
          ...
          IconButton(icon: const Icon(Icons.refresh), tooltip: 'Refresh',
              onPressed: () => ref.invalidate(dashboardConfigurationListProvider)),
        ],
      ),
        ...
        data: (items) {
          ...
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Dashboard Configurations yet',
...
void _showDetail(BuildContext context, DashboardConfiguration item) {
  showDialog(context: context, builder: (ctx) => Dialog(
    child: ConstrainedBox(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Dashboard Configuration Details'),
...
class __DashboardConfigurationFormState extends ConsumerState<_DashboardConfigurationForm> {
  ...
  void _submit() {
    ...
    if (widget.item == null) {
      widget.ref.read(dashboardConfigurationCreateStateProvider.notifier).state = DashboardConfiguration.fromJson(data);
    } else {
      widget.ref.read(dashboardConfigurationUpdateStateProvider.notifier).state = {
        'id': widget.item!.id,
        'dashboardConfiguration': DashboardConfiguration.fromJson({...widget.item!.toJson(), ...data}),
      };
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      child: ConstrainedBox(
        ...
        child: Scaffold(
          appBar: AppBar(
            title: Text(isEdit ? 'Edit Dashboard Configuration' : 'New Dashboard Configuration'),
            automaticallyImplyLeading: false,
            ...
            child: ListView(
              ...
              children: [
                ...
                FilledButton.icon(
                  ...
                  label: Text(isEdit ? 'Save Changes' : 'Create Dashboard Configuration'),
...
void _confirmDel(BuildContext context, WidgetRef ref, DashboardConfiguration item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    ...
    title: const Text('Delete Dashboard Configuration?'),
    content: const Text('This action cannot be undone.'),
    actions: [
      TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
      FilledButton.icon(
        onPressed: () {
          ref.read(dashboardConfigurationDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
