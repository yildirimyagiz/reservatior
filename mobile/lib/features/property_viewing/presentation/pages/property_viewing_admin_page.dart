...
// ================================================================
// PropertyViewing Admin Page
// ================================================================

class PropertyViewingAdminPage extends ConsumerWidget {
  const PropertyViewingAdminPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(propertyViewingLoadingProvider);
    return Scaffold(
      appBar: AppBar(
        title: const Text('Property Viewing Management'),
        ...
        actions: [
          ...
          IconButton(icon: const Icon(Icons.refresh), tooltip: 'Refresh',
              onPressed: () => ref.invalidate(propertyViewingListProvider)),
        ],
      ),
      body: const _PropertyViewingBody(),
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'PropertyViewingFAB',
...
class _PropertyViewingBody extends ConsumerStatefulWidget {
  const _PropertyViewingBody({super.key});
  @override ConsumerState<_PropertyViewingBody> createState() => __PropertyViewingBodyState();
}

class __PropertyViewingBodyState extends ConsumerState<_PropertyViewingBody> {
        ...
        data: (items) {
          ...
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Property Viewings yet',
              ...
              itemBuilder: (ctx, i) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    title: Text(item.viewingType?.toString() ?? 'Unknown', style: const TextStyle(fontWeight: FontWeight.w600)),
...
void _showDetail(BuildContext context, PropertyViewing item) {
  showDialog(context: context, builder: (ctx) => Dialog(
    child: ConstrainedBox(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Property Viewing Details'),
...
void _showForm(BuildContext context, WidgetRef ref, {PropertyViewing? item}) {
  showDialog(
    context: context,
    builder: (ctx) => Dialog(
      child: ConstrainedBox(
        ...
        child: Scaffold(
          appBar: AppBar(
            title: Text(item != null ? 'Edit Property Viewing' : 'New Property Viewing'),
            automaticallyImplyLeading: false,
            actions: [IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(ctx))],
          ),
          body: SingleChildScrollView(
...
void _confirmDel(BuildContext context, WidgetRef ref, PropertyViewing item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    icon: const Icon(Icons.warning_amber_rounded, color: Colors.orange, size: 40),
    title: const Text('Delete Property Viewing?'),
    content: const Text('This action cannot be undone.'),
    actions: [
      TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
      FilledButton.icon(
        onPressed: () {
          ref.read(propertyViewingDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
