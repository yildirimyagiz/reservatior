...
import '../widgets/m_l_configuration_form_widget.dart';

// ── MLConfiguration Client Page

class MLConfigurationClientPage extends ConsumerStatefulWidget {
  const MLConfigurationClientPage({super.key});
  @override ConsumerState<MLConfigurationClientPage> createState() => _MLConfigurationClientPageState();
}

class _MLConfigurationClientPageState extends ConsumerState<MLConfigurationClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My M L Configurations'),
        backgroundColor: Colors.teal[700], foregroundColor: Colors.white,
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(mLConfigurationListProvider)),
        ],
      ),
          ...
          data: (items) {
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No M L Configurations', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
      ...
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'MLConfigurationClientFAB',
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              const Text('M L Configuration Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New M L Configuration', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            constraints: BoxConstraints(maxHeight: MediaQuery.of(ctx).size.height * 0.7),
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16),
              child: MLConfigurationFormWidget(
                onSubmit: (newItem) {
                  ref.read(mLConfigurationCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
