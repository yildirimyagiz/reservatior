...
class SessionListWidget extends StatelessWidget {
  ...
  const SessionListWidget({super.key, required this.items, this.onTap, this.onEdit, this.onDelete});
    ...
    return ListView.builder(
      ...
      itemBuilder: (ctx, i) {
        ...
        return Card(
          ...
          child: ListTile(
            leading: CircleAvatar(child: Text(item.tokenHash != null && item.tokenHash!.toString().isNotEmpty ? item.tokenHash!.toString()[0].toUpperCase() : '?'),),
            title: Text(item.tokenHash ?? 'Unknown', style: const TextStyle(fontWeight: FontWeight.w600)),
