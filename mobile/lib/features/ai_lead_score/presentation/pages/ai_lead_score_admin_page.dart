...
import '../../../../shared/providers/ai_lead_score_provider.dart';
import '../../../../gen_models/models_library.dart';

// ================================================================
// AILeadScore Admin Page  |  11 fields
// Auto-generated — edit with care
// ================================================================

class AILeadScoreAdminPage extends ConsumerWidget {
  const AILeadScoreAdminPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(aiLeadScoreLoadingProvider);
    return Scaffold(
      appBar: AppBar(
        title: const Text('Ai Lead Score Management'),
        ...
        actions: [
          ...
          IconButton(icon: const Icon(Icons.refresh), tooltip: 'Refresh',
              onPressed: () => ref.invalidate(aiLeadScoreListProvider)),
        ],
      ),
      body: const _AILeadScoreBody(),
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'AILeadScoreFAB',
        onPressed: () => _showForm(context, ref),
        icon: const Icon(Icons.add), label: const Text('New Ai Lead Score'),
      ),
    );
  }
}
...
class _AILeadScoreBody extends ConsumerStatefulWidget {
  const _AILeadScoreBody();
  @override ConsumerState<_AILeadScoreBody> createState() => __AILeadScoreBodyState();
}

class __AILeadScoreBodyState extends ConsumerState<_AILeadScoreBody> {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(aiLeadScoreListProvider);
      ...
      Padding(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            hintText: 'Search Ai Lead Scores…',
      ...
      Expanded(child: async.when(
        data: (items) {
          final list = _q.isEmpty
              ? items
              : items.where((item) => ((item.orgId?.toString() ?? '') + " " + (item.modelId?.toString() ?? '') + " " + (item.leadId?.toString() ?? '') + " " + (item.status?.toString() ?? '')).toLowerCase().contains(_q)).toList();
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.inbox_outlined, size: 56, color: Colors.grey[350]),
              const SizedBox(height: 12),
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Ai Lead Scores yet',
                  style: TextStyle(color: Colors.grey[500])),
            ]));
          }
          return RefreshIndicator(
            onRefresh: () async => ref.invalidate(aiLeadScoreListProvider),
        ...
        error: (e, _) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 12),
          ElevatedButton.icon(onPressed: () => ref.invalidate(aiLeadScoreListProvider),
              icon: const Icon(Icons.refresh), label: const Text('Retry')),
        ])),
      )),
    ]);
  }
  Color _stColor(AILeadScore item) {
...
void _showDetail(BuildContext context, AILeadScore item) {
  showDialog(context: context, builder: (ctx) => Dialog(
    child: ConstrainedBox(
      constraints: const BoxConstraints(maxWidth: 500, maxHeight: 640),
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Ai Lead Score Details'),
        ...
        body: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _row('Lead Id', item.leadId?.toString() ?? 'N/A', Icons.link),
              _row('Score', item.score?.toString() ?? 'N/A', Icons.numbers),
              _row('Score Breakdown', item.scoreBreakdown?.toString() ?? 'N/A', Icons.text_fields),
              _row('Confidence', item.confidence?.toString() ?? 'N/A', Icons.numbers),
              _row('Scored At', _formatDate(item.scoredAt), Icons.calendar_today),
              _row('Features Used', item.featuresUsed?.toString() ?? 'N/A', Icons.text_fields),
...
void _showForm(BuildContext context, WidgetRef ref, {AILeadScore? item}) {
  showDialog(context: context, builder: (ctx) => _AILeadScoreForm(item: item, ref: ref));
}

class _AILeadScoreForm extends ConsumerStatefulWidget {
  final AILeadScore? item;
  final WidgetRef ref;
  const _AILeadScoreForm({super.key, this.item, required this.ref});
  @override ConsumerState<_AILeadScoreForm> createState() => __AILeadScoreFormState();
}

class __AILeadScoreFormState extends ConsumerState<_AILeadScoreForm> {
  ...
  void _submit() {
    ...
    if (widget.item == null) {
      widget.ref.read(aiLeadScoreCreateStateProvider.notifier).state = AILeadScore.fromJson(data);
    } else {
      widget.ref.read(aiLeadScoreUpdateStateProvider.notifier).state = {
        'id': widget.item!.id,
        'aiLeadScore': AILeadScore.fromJson({...widget.item!.toJson(), ...data}),
      };
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      child: ConstrainedBox(
        ...
        child: Scaffold(
          appBar: AppBar(
            title: Text(isEdit ? 'Edit Ai Lead Score' : 'New Ai Lead Score'),
            automaticallyImplyLeading: false,
            ...
            child: ListView(
              ...
              children: [
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item?.leadId?.toString() ?? '',
                    maxLines: 1,
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item?.scoreBreakdown?.toString() ?? '',
                    maxLines: 1,
                  ...
                  InkWell(
                    ...
                    child: InputDecorator(
                      ...
                      child: Text(_scoredAt != null ? _formatDate(_scoredAt) : 'Tap to select date'),
                    ),
                  ),
                  TextFormField(
                    decoration: InputDecoration(labelText: 'Features Used', prefixIcon: Icon(Icons.text_fields), border: OutlineInputBorder()),
                    initialValue: widget.item?.featuresUsed?.toString() ?? '',
                    maxLines: 1,
                ...
                FilledButton.icon(
                  ...
                  label: Text(isEdit ? 'Save Changes' : 'Create Ai Lead Score'),
...
void _confirmDel(BuildContext context, WidgetRef ref, AILeadScore item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    icon: const Icon(Icons.warning_amber_rounded, color: Colors.orange, size: 40),
    title: const Text('Delete Ai Lead Score?'),
    content: const Text('This action cannot be undone.'),
    actions: [
      TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
      FilledButton.icon(
        onPressed: () {
          ref.read(aiLeadScoreDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
