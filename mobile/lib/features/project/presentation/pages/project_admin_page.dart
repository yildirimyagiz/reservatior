...
// ================================================================
// Project Admin Page  |  21 fields
// Auto-generated — edit with care
// ================================================================

class ProjectAdminPage extends ConsumerWidget {
  const ProjectAdminPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(projectLoadingProvider);
    return Scaffold(
      appBar: AppBar(
...
class _ProjectBody extends ConsumerStatefulWidget {
  ...
  @override ConsumerState<_ProjectBody> createState() => __ProjectBodyState();
}

class __ProjectBodyState extends ConsumerState<_ProjectBody> {
      ...
      Expanded(child: async.when(
        data: (items) {
          final list = _q.isEmpty
              ? items
              : items.where((item) => ((item.orgId?.toString() ?? '') + " " + (item.name?.toString() ?? '') + " " + (item.description?.toString() ?? '') + " " + (item.projectType?.toString() ?? '') + " " + (item.propertyId?.toString() ?? '') + " " + (item.address?.toString() ?? '') + " " + (item.status?.toString() ?? '') + " " + (item.currency?.toString() ?? '') + " " + (item.managerId?.toString() ?? '') + " " + (item.contractorId?.toString() ?? '') + " " + (item.createdBy?.toString() ?? '')).toLowerCase().contains(_q)).toList();
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.inbox_outlined, size: 56, color: Colors.grey[350]),
              const SizedBox(height: 12),
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Projects yet',
        ...
        error: (e, _) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 12),
          ElevatedButton.icon(onPressed: () => ref.invalidate(projectListProvider),
              icon: const Icon(Icons.refresh), label: const Text('Retry')),
        ])),
      )),
    ]);
  }
  Color _stColor(Project item) {
...
void _showDetail(BuildContext context, Project item) {
      ...
      child: Scaffold(
        ...
        body: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _row('Estimated End Date', _formatDate(item.estimatedEndDate), Icons.calendar_today),
              _row('Actual End Date', _formatDate(item.actualEndDate), Icons.calendar_today),
...
void _showForm(BuildContext context, WidgetRef ref, {Project? item}) {
  showDialog(context: context, builder: (ctx) => _ProjectForm(item: item, ref: ref));
}

class _ProjectForm extends ConsumerStatefulWidget {
  final Project? item;
  final WidgetRef ref;
  const _ProjectForm({super.key, this.item, required this.ref});
  @override ConsumerState<_ProjectForm> createState() => __ProjectFormState();
}

class __ProjectFormState extends ConsumerState<_ProjectForm> {
  ...
  void _submit() {
    ...
    if (widget.item == null) {
      widget.ref.read(projectCreateStateProvider.notifier).state = Project.fromJson(data);
    } else {
      widget.ref.read(projectUpdateStateProvider.notifier).state = {
        'id': widget.item!.id,
        'project': Project.fromJson({...widget.item!.toJson(), ...data}),
      };
    }
            ...
            child: ListView(
              ...
              children: [
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item.projectType?.toString() ?? '',
                    maxLines: 1,
                  ...
                  InkWell(
                    ...
                    child: InputDecorator(
                      ...
                      child: Text(_estimatedEndDate != null ? _formatDate(_estimatedEndDate) : 'Tap to select date'),
                    ),
                  ),
                  InkWell(
                    onTap: () async {
                    ...
                    child: InputDecorator(
                      ...
                      child: Text(_actualEndDate != null ? _formatDate(_actualEndDate) : 'Tap to select date'),
                    ),
                  ),
                  TextFormField(
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item.managerId?.toString() ?? '',
                    maxLines: 1,
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item.contractorId?.toString() ?? '',
                    maxLines: 1,
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item.milestones?.toString() ?? '',
                    maxLines: 1,
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item.phases?.toString() ?? '',
                    maxLines: 1,
...
void _confirmDel(BuildContext context, WidgetRef ref, Project item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    ...
    actions: [
      ...
      FilledButton.icon(
        onPressed: () {
          ref.read(projectDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
