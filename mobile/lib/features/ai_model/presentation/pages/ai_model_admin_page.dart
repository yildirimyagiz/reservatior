...
// ================================================================
// AIModel Admin Page  |  15 fields
// Auto-generated — edit with care
// ================================================================

class AIModelAdminPage extends ConsumerWidget {
  const AIModelAdminPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(aiModelLoadingProvider);
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(icon: const Icon(Icons.refresh), tooltip: 'Refresh',
              onPressed: () => ref.invalidate(aiModelListProvider)),
        ],
      ),
      body: const _AIModelBody(),
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'AIModelFAB',
        onPressed: () => _showForm(context, ref),
        icon: const Icon(Icons.add), label: const Text('New Ai Model'),
      ),
    );
  }
}
...
class _AIModelBody extends ConsumerStatefulWidget {
  const _AIModelBody();
  @override ConsumerState<_AIModelBody> createState() => __AIModelBodyState();
}

class __AIModelBodyState extends ConsumerState<_AIModelBody> {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(aiModelListProvider);
    return Column(children: [
      ...
      Expanded(child: async.when(
        data: (items) {
          ...
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Ai Models yet',
                  style: TextStyle(color: Colors.grey[500])),
            ]));
          }
          return RefreshIndicator(
            onRefresh: () async => ref.invalidate(aiModelListProvider),
              ...
              itemBuilder: (ctx, i) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    leading: CircleAvatar(backgroundColor: _stColor(item), foregroundColor: Colors.white, child: Text(item.modelName != null && item.modelName!.toString().isNotEmpty ? item.modelName!.toString()[0].toUpperCase() : '?'),),
        ...
        error: (e, _) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 12),
          ElevatedButton.icon(onPressed: () => ref.invalidate(aiModelListProvider),
              icon: const Icon(Icons.refresh), label: const Text('Retry')),
        ])),
      )),
    ]);
  }
  Color _stColor(AIModel item) {
...
void _showDetail(BuildContext context, AIModel item) {
  showDialog(context: context, builder: (ctx) => Dialog(
    child: ConstrainedBox(
      constraints: const BoxConstraints(maxWidth: 500, maxHeight: 640),
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Ai Model Details'),
        ...
        body: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _row('Endpoint Url', item.endpointUrl?.toString() ?? 'N/A', Icons.link),
...
void _showForm(BuildContext context, WidgetRef ref, {AIModel? item}) {
  showDialog(context: context, builder: (ctx) => _AIModelForm(item: item, ref: ref));
}

class _AIModelForm extends ConsumerStatefulWidget {
  final AIModel? item;
  final WidgetRef ref;
  const _AIModelForm({this.item, required this.ref});
  @override ConsumerState<_AIModelForm> createState() => __AIModelFormState();
}

class __AIModelFormState extends ConsumerState<_AIModelForm> {
  ...
  void _submit() {
    ...
    if (widget.item == null) {
      widget.ref.read(aiModelCreateStateProvider.notifier).state = AIModel.fromJson(data);
    } else {
      widget.ref.read(aiModelUpdateStateProvider.notifier).state = {
        'id': widget.item!.id,
        'aiModel': AIModel.fromJson({...widget.item!.toJson(), ...data}),
      };
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      child: ConstrainedBox(
        ...
        child: Scaffold(
          appBar: AppBar(
            title: Text(isEdit ? 'Edit Ai Model' : 'New Ai Model'),
            automaticallyImplyLeading: false,
          ...
          body: Form(
            ...
            child: ListView(
              ...
              children: [
                  ...
                  TextFormField(
                    decoration: InputDecoration(labelText: 'Endpoint Url', prefixIcon: Icon(Icons.link), border: OutlineInputBorder()),
                    initialValue: widget.item?.endpointUrl?.toString() ?? '',
                    maxLines: 1,
                ...
                FilledButton.icon(
                  ...
                  label: Text(isEdit ? 'Save Changes' : 'Create Ai Model'),
...
void _confirmDel(BuildContext context, WidgetRef ref, AIModel item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    ...
    actions: [
      ...
      FilledButton.icon(
        onPressed: () {
          ref.read(aiModelDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
