...
// ── SignatureSigner Detail Widget  |  12 fields

class SignatureSignerDetailWidget extends StatelessWidget {
  final SignatureSigner item;
  const SignatureSignerDetailWidget({super.key, required this.item});
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      if (item.status != null)
        ...
        _row('Signature Request Id', item.signatureRequestId?.toString() ?? 'N/A', Icons.link),
        _row('Participant Type', item.participantType?.toString() ?? 'N/A', Icons.text_fields),
