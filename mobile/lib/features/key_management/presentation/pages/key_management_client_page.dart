...
import '../widgets/key_management_form_widget.dart';

// ── KeyManagement Client Page

class KeyManagementClientPage extends ConsumerStatefulWidget {
  const KeyManagementClientPage({super.key});
  @override ConsumerState<KeyManagementClientPage> createState() => _KeyManagementClientPageState();
}

class _KeyManagementClientPageState extends ConsumerState<KeyManagementClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Key Managements'),
        backgroundColor: Colors.teal[700], foregroundColor: Colors.white,
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(keyManagementListProvider)),
        ],
      ),
          ...
          data: (items) {
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Key Managements', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
                ...
                itemBuilder: (ctx, i) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      leading: CircleAvatar(backgroundColor: _stColor(item.keyStatus), foregroundColor: Colors.white, child: Text(item.keyType != null && item.keyType!.toString().isNotEmpty ? item.keyType!.toString()[0].toUpperCase() : '?'),),
      ...
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'KeyManagementClientFAB',
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              const Text('Key Management Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Key Management', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            constraints: BoxConstraints(maxHeight: MediaQuery.of(ctx).size.height * 0.7),
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16),
              child: KeyManagementFormWidget(
                onSubmit: (newItem) {
                  ref.read(keyManagementCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
