...
class PropertyListWidget extends StatelessWidget {
  ...
  final void Function(Property)? onEdit;
  final void Function(Property)? onDelete;
  const PropertyListWidget({super.key, required this.items, this.onTap, this.onEdit, this.onDelete});
    ...
    return ListView.builder(
      ...
      itemBuilder: (ctx, i) {
        ...
        return Card(
          ...
          child: ListTile(
            leading: CircleAvatar(backgroundColor: _stColor(item.listingStatus), foregroundColor: Colors.white, child: Text(item.name != null && item.name!.toString().isNotEmpty ? item.name!.toString()[0].toUpperCase() : '?'),),
