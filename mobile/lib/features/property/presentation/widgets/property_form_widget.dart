...
class PropertyFormWidget extends StatefulWidget {
  ...
  @override State<PropertyFormWidget> createState() => _PropertyFormWidgetState();
}
...
class _PropertyFormWidgetState extends State<PropertyFormWidget> {
  ...
  void _submit() {
    ...
    final data = <String, dynamic>{
        ...
        if (_neighborhoodId?.isNotEmpty == true) 'neighborhoodId': _neighborhoodId,
        ...
        if (_stateCode?.isNotEmpty == true) 'stateCode': _stateCode,
        if (_propertyCategory?.isNotEmpty == true) 'propertyCategory': _propertyCategory,
        if (_listingType?.isNotEmpty == true) 'listingType': _listingType,
        if (_listingStatus?.isNotEmpty == true) 'listingStatus': _listingStatus,
        if (_listingPrice != null) 'listingPrice': _listingPrice,
        if (_originalPrice != null) 'originalPrice': _originalPrice,
        if (_priceHistory?.isNotEmpty == true) 'priceHistory': _priceHistory,
        if (_schoolDistrict?.isNotEmpty == true) 'schoolDistrict': _schoolDistrict,
        if (_hoaFee != null) 'hoaFee': _hoaFee,
        if (_hoaFeeFrequency?.isNotEmpty == true) 'hoaFeeFrequency': _hoaFeeFrequency,
        ...
        if (_floodZone?.isNotEmpty == true) 'floodZone': _floodZone,
        if (_zoningCode?.isNotEmpty == true) 'zoningCode': _zoningCode,
        ...
        if (_basementType?.isNotEmpty == true) 'basementType': _basementType,
        if (_basementFinishedSqFt != null) 'basementFinishedSqFt': _basementFinishedSqFt,
        if (_garageType?.isNotEmpty == true) 'garageType': _garageType,
        if (_garageCapacity != null) 'garageCapacity': _garageCapacity,
        if (_parkingSpaces != null) 'parkingSpaces': _parkingSpaces,
        if (_parkingType?.isNotEmpty == true) 'parkingType': _parkingType,
        if (_poolType?.isNotEmpty == true) 'poolType': _poolType,
        if (_heatingType?.isNotEmpty == true) 'heatingType': _heatingType,
        if (_coolingType?.isNotEmpty == true) 'coolingType': _coolingType,
        if (_fireplaceType?.isNotEmpty == true) 'fireplaceType': _fireplaceType,
        if (_fireplaceCount != null) 'fireplaceCount': _fireplaceCount,
        if (_viewType?.isNotEmpty == true) 'viewType': _viewType,
        if (_waterfrontType?.isNotEmpty == true) 'waterfrontType': _waterfrontType,
        if (_waterfrontFeet != null) 'waterfrontFeet': _waterfrontFeet,
        if (_constructionType?.isNotEmpty == true) 'constructionType': _constructionType,
        if (_roofType?.isNotEmpty == true) 'roofType': _roofType,
        if (_roofYear != null) 'roofYear': _roofYear,
        if (_sidingType?.isNotEmpty == true) 'sidingType': _sidingType,
        if (_zipPlus4?.isNotEmpty == true) 'zipPlus4': _zipPlus4,
        if (_countyFIPS?.isNotEmpty == true) 'countyFIPS': _countyFIPS,
        if (_censusTract?.isNotEmpty == true) 'censusTract': _censusTract,
        if (_mlsArea?.isNotEmpty == true) 'mlsArea': _mlsArea,
        if (_propertyClass?.isNotEmpty == true) 'propertyClass': _propertyClass,
        if (_buildingClass?.isNotEmpty == true) 'buildingClass': _buildingClass,
        ...
        if (_electricityProvider?.isNotEmpty == true) 'electricityProvider': _electricityProvider,
        if (_gasProvider?.isNotEmpty == true) 'gasProvider': _gasProvider,
        if (_waterProvider?.isNotEmpty == true) 'waterProvider': _waterProvider,
        if (_internetProvider?.isNotEmpty == true) 'internetProvider': _internetProvider,
        if (_trashService?.isNotEmpty == true) 'trashService': _trashService,
        ...
        if (_energyRating?.isNotEmpty == true) 'energyRating': _energyRating,
        if (_zoningDescription?.isNotEmpty == true) 'zoningDescription': _zoningDescription,
        if (_landUse?.isNotEmpty == true) 'landUse': _landUse,
        if (_buildingRestrictions?.isNotEmpty == true) 'buildingRestrictions': _buildingRestrictions,
        if (_futureDevelopment?.isNotEmpty == true) 'futureDevelopment': _futureDevelopment,
              ...
              InkWell(
                onTap: () async {
                  final d = await showDatePicker(
                    context: context, initialDate: _moldInspectionDate ?? DateTime.now(),
                    firstDate: DateTime(2000), lastDate: DateTime(2100),
                  );
                  if (d != null) setState(() => _moldInspectionDate = d);
                },
                child: InputDecorator(
                  decoration: const InputDecoration(labelText: 'Mold Inspection Date',
                    prefixIcon: Icon(Icons.calendar_today), border: OutlineInputBorder()),
                  child: Text(_moldInspectionDate != null ? _fmt(_moldInspectionDate) : 'Tap to select date'),
                ),
              ),
              const SizedBox(height: 12),
              InkWell(
                onTap: () async {
                  final d = await showDatePicker(
                    context: context, initialDate: _asbestosInspectionDate ?? DateTime.now(),
                    firstDate: DateTime(2000), lastDate: DateTime(2100),
                  );
                  if (d != null) setState(() => _asbestosInspectionDate = d);
                },
                child: InputDecorator(
                  decoration: const InputDecoration(labelText: 'Asbestos Inspection Date',
                    prefixIcon: Icon(Icons.calendar_today), border: OutlineInputBorder()),
                  child: Text(_asbestosInspectionDate != null ? _fmt(_asbestosInspectionDate) : 'Tap to select date'),
                ),
              ),
              const SizedBox(height: 12),
              InkWell(
                onTap: () async {
                  final d = await showDatePicker(
                    context: context, initialDate: _radonTestDate ?? DateTime.now(),
                    firstDate: DateTime(2000), lastDate: DateTime(2100),
                  );
                  if (d != null) setState(() => _radonTestDate = d);
                },
                child: InputDecorator(
                  decoration: const InputDecoration(labelText: 'Radon Test Date',
                    prefixIcon: Icon(Icons.calendar_today), border: OutlineInputBorder()),
                  child: Text(_radonTestDate != null ? _fmt(_radonTestDate) : 'Tap to select date'),
                ),
              ),
              const SizedBox(height: 12),
              InkWell(
                onTap: () async {
                  final d = await showDatePicker(
                    context: context, initialDate: _pestControlDate ?? DateTime.now(),
                    firstDate: DateTime(2000), lastDate: DateTime(2100),
                  );
                  if (d != null) setState(() => _pestControlDate = d);
                },
                child: InputDecorator(
                  decoration: const InputDecoration(labelText: 'Pest Control Date',
                    prefixIcon: Icon(Icons.calendar_today), border: OutlineInputBorder()),
                  child: Text(_pestControlDate != null ? _fmt(_pestControlDate) : 'Tap to select date'),
                ),
              ),
              const SizedBox(height: 12),
              InkWell(
                onTap: () async {
                  final d = await showDatePicker(
                    context: context, initialDate: _fireInspectionDate ?? DateTime.now(),
                    firstDate: DateTime(2000), lastDate: DateTime(2100),
                  );
                  if (d != null) setState(() => _fireInspectionDate = d);
                },
                child: InputDecorator(
                  decoration: const InputDecoration(labelText: 'Fire Inspection Date',
                    prefixIcon: Icon(Icons.calendar_today), border: OutlineInputBorder()),
                  child: Text(_fireInspectionDate != null ? _fmt(_fireInspectionDate) : 'Tap to select date'),
                ),
              ),
              const SizedBox(height: 12),
              InkWell(
                onTap: () async {
                  final d = await showDatePicker(
                    context: context, initialDate: _elevatorInspectionDate ?? DateTime.now(),
                    firstDate: DateTime(2000), lastDate: DateTime(2100),
                  );
                  if (d != null) setState(() => _elevatorInspectionDate = d);
                },
                child: InputDecorator(
                  decoration: const InputDecoration(labelText: 'Elevator Inspection Date',
                    prefixIcon: Icon(Icons.calendar_today), border: OutlineInputBorder()),
                  child: Text(_elevatorInspectionDate != null ? _fmt(_elevatorInspectionDate) : 'Tap to select date'),
                ),
              ),
              const SizedBox(height: 12),
              InkWell(
                onTap: () async {
                  final d = await showDatePicker(
                    context: context, initialDate: _poolInspectionDate ?? DateTime.now(),
                    firstDate: DateTime(2000), lastDate: DateTime(2100),
                  );
                  if (d != null) setState(() => _poolInspectionDate = d);
                },
                child: InputDecorator(
                  decoration: const InputDecoration(labelText: 'Pool Inspection Date',
                    prefixIcon: Icon(Icons.calendar_today), border: OutlineInputBorder()),
                  child: Text(_poolInspectionDate != null ? _fmt(_poolInspectionDate) : 'Tap to select date'),
                ),
              ),
              const SizedBox(height: 12),
              InkWell(
                onTap: () async {
                  final d = await showDatePicker(
                    context: context, initialDate: _lastCodeComplianceDate ?? DateTime.now(),
                    firstDate: DateTime(2000), lastDate: DateTime(2100),
                  );
                  if (d != null) setState(() => _lastCodeComplianceDate = d);
                },
                child: InputDecorator(
                  decoration: const InputDecoration(labelText: 'Last Code Compliance Date',
                    prefixIcon: Icon(Icons.calendar_today), border: OutlineInputBorder()),
                  child: Text(_lastCodeComplianceDate != null ? _fmt(_lastCodeComplianceDate) : 'Tap to select date'),
                ),
              ),
              const SizedBox(height: 12),
              StatefulBuilder(
