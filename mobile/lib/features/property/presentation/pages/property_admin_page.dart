...
class PropertyAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(propertyLoadingProvider);
    return Scaffold(
      appBar: AppBar(
...
class _PropertyBody extends ConsumerStatefulWidget {
  const _PropertyBody({super.key});
  @override ConsumerState<_PropertyBody> createState() => __PropertyBodyState();
}

class __PropertyBodyState extends ConsumerState<_PropertyBody> {
        ...
        data: (items) {
          ...
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Propertys yet',
...
void _showForm(BuildContext context, WidgetRef ref, {Property? item}) {
  showDialog(
    context: context,
    builder: (ctx) => Dialog(
      child: ConstrainedBox(
        ...
        child: Scaffold(
          appBar: AppBar(
            title: Text(item != null ? 'Edit Property' : 'New Property'),
            automaticallyImplyLeading: false,
            actions: [IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(ctx))],
          ),
          body: SingleChildScrollView(
...
void _confirmDel(BuildContext context, WidgetRef ref, Property item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    ...
    actions: [
      ...
      FilledButton.icon(
        onPressed: () {
          ref.read(propertyDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
