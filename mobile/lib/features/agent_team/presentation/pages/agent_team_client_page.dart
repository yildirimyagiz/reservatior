...
import '../widgets/agent_team_form_widget.dart';

// ── AgentTeam Client Page

class AgentTeamClientPage extends ConsumerStatefulWidget {
  const AgentTeamClientPage({super.key});
  @override ConsumerState<AgentTeamClientPage> createState() => _AgentTeamClientPageState();
}

class _AgentTeamClientPageState extends ConsumerState<AgentTeamClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Agent Teams'),
        backgroundColor: Colors.teal[700], foregroundColor: Colors.white,
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(agentTeamListProvider)),
        ],
      ),
  ...
  return '$orgId $name $leaderId'.toLowerCase().contains(_q);
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Agent Teams', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              const Text('Agent Team Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Agent Team', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            constraints: BoxConstraints(maxHeight: MediaQuery.of(ctx).size.height * 0.7),
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16),
              child: AgentTeamFormWidget(
                onSubmit: (newItem) {
                  ref.read(agentTeamCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
