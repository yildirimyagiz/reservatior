...
class ReservationListWidget extends StatelessWidget {
  ...
  const ReservationListWidget({super.key, required this.items, this.onTap, this.onEdit, this.onDelete});
    ...
    return ListView.builder(
      ...
      itemBuilder: (ctx, i) {
        ...
        return Card(
          ...
          child: ListTile(
            leading: CircleAvatar(backgroundColor: _stColor(item.status), foregroundColor: Colors.white, child: Text(item.specialRequests != null && item.specialRequests!.toString().isNotEmpty ? item.specialRequests!.toString()[0].toUpperCase() : '?'),),
            title: Text(item.specialRequests ?? 'Unknown', style: const TextStyle(fontWeight: FontWeight.w600)),
