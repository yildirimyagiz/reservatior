...
import '../widgets/offline_sync_queue_form_widget.dart';

// ── OfflineSyncQueue Client Page

class OfflineSyncQueueClientPage extends ConsumerStatefulWidget {
  const OfflineSyncQueueClientPage({super.key});
  @override ConsumerState<OfflineSyncQueueClientPage> createState() => _OfflineSyncQueueClientPageState();
}

class _OfflineSyncQueueClientPageState extends ConsumerState<OfflineSyncQueueClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Offline Sync Queues'),
        backgroundColor: Colors.teal[700], foregroundColor: Colors.white,
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(offlineSyncQueueListProvider)),
        ],
      ),
          ...
          data: (items) {
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Offline Sync Queues', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
                ...
                itemBuilder: (ctx, i) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      leading: CircleAvatar(backgroundColor: _stColor(item.syncStatus), foregroundColor: Colors.white, child: Text(item.entityType != null && item.entityType!.toString().isNotEmpty ? item.entityType!.toString()[0].toUpperCase() : '?'),),
      ...
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'OfflineSyncQueueClientFAB',
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              const Text('Offline Sync Queue Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
          ...
          Expanded(child: ListView(controller: sc, padding: const EdgeInsets.all(16), children: [
                  ...
                  _row('Synced At', _fmt(item.syncedAt), Icons.calendar_today),
          ])),
        ]),
      ),
    );
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Offline Sync Queue', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            constraints: BoxConstraints(maxHeight: MediaQuery.of(ctx).size.height * 0.7),
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16),
              child: OfflineSyncQueueFormWidget(
                onSubmit: (newItem) {
                  ref.read(offlineSyncQueueCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
