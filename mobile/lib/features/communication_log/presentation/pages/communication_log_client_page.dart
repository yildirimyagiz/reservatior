...
import '../widgets/communication_log_form_widget.dart';

// ── CommunicationLog Client Page

class CommunicationLogClientPage extends ConsumerStatefulWidget {
  const CommunicationLogClientPage({super.key});
  @override ConsumerState<CommunicationLogClientPage> createState() => _CommunicationLogClientPageState();
}

class _CommunicationLogClientPageState extends ConsumerState<CommunicationLogClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Communication Logs'),
        backgroundColor: Colors.teal[700], foregroundColor: Colors.white,
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(communicationLogListProvider)),
        ],
      ),
          ...
          data: (items) {
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Communication Logs', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
      ...
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'CommunicationLogClientFAB',
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              const Text('Communication Log Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
          ...
          Expanded(child: ListView(controller: sc, padding: const EdgeInsets.all(16), children: [
                  ...
                  _row('Delivered At', _fmt(item.deliveredAt), Icons.calendar_today),
                  ...
                  _row('Edited At', _fmt(item.editedAt), Icons.calendar_today),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Communication Log', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            constraints: BoxConstraints(maxHeight: MediaQuery.of(ctx).size.height * 0.7),
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16),
              child: CommunicationLogFormWidget(
                onSubmit: (newItem) {
                  ref.read(communicationLogCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
