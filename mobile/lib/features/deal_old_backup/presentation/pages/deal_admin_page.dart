...
class __DealBodyState extends ConsumerState<_DealBody> {
      ...
      Expanded(child: async.when(
        data: (items) {
          final list = _q.isEmpty
              ? items
              : items.where((item) => ((item.orgId?.toString() ?? '') + " " + (item.listingId?.toString() ?? '') + " " + (item.propertyId?.toString() ?? '') + " " + (item.clientId?.toString() ?? '') + " " + (item.agentId?.toString() ?? '') + " " + (item.locationId?.toString() ?? '') + " " + (item.dealType?.toString() ?? '') + " " + (item.financingType?.toString() ?? '') + " " + (item.createdBy?.toString() ?? '')).toLowerCase().contains(_q)).toList();
          if (list.isEmpty) {
              ...
              itemBuilder: (ctx, i) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    leading: CircleAvatar(backgroundColor: _stColor(item), foregroundColor: Colors.white, child: Text(item.dealType != null && item.dealType!.toString().isNotEmpty ? item.dealType!.toString()[0].toUpperCase() : '?'),),
                    title: Text(item.dealType ?? 'Unknown', style: const TextStyle(fontWeight: FontWeight.w600)),
                ...
                if (item.dealStatus != null)
                  Container(
                    ...
                    child: Text(item.dealStatus!.toString(), style: TextStyle(fontSize: 11, color: _stColor(item), fontWeight: FontWeight.w600)),
                  ),
  ...
  Color _stColor(Deal item) {
    final s = item.dealStatus?.toString().toLowerCase() ?? '';
      ...
      child: Scaffold(
        ...
        body: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _row('Deal Status', item.dealStatus?.toString() ?? 'N/A', Icons.info_outline),
              _row('Deal Type', item.dealType?.toString() ?? 'N/A', Icons.text_fields),
              ...
              _row('Down Payment', item.downPayment?.toString() ?? 'N/A', Icons.numbers),
              _row('Earnest Money', item.earnestMoney?.toString() ?? 'N/A', Icons.numbers),
              ...
              _row('Title Contingency', (item.titleContingency == true ? 'Yes' : 'No'), Icons.toggle_on),
            ...
            child: ListView(
              ...
              children: [
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item.dealStatus?.toString() ?? '',
                    maxLines: 1,
