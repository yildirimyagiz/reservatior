...
import '../widgets/achievement_form_widget.dart';

// ── Achievement Client Page

class AchievementClientPage extends ConsumerStatefulWidget {
  const AchievementClientPage({super.key});
  @override ConsumerState<AchievementClientPage> createState() => _AchievementClientPageState();
}

class _AchievementClientPageState extends ConsumerState<AchievementClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(achievementListProvider)),
        ],
      ),
  ...
  return '$userId $bonusReward $organizationId'.toLowerCase().contains(_q);
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Achievements', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              const Text('Achievement Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Achievement', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            ...
            child: SingleChildScrollView(
              ...
              child: AchievementFormWidget(
                onSubmit: (newItem) {
                  ref.read(achievementCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
