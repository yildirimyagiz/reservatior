...
// ================================================================
// DepositProtection Admin Page  |  15 fields
...
class DepositProtectionAdminPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(depositProtectionLoadingProvider);
    return Scaffold(
      appBar: AppBar(
        title: const Text('Deposit Protection Management'),
        ...
        actions: [
          ...
          IconButton(icon: const Icon(Icons.refresh), tooltip: 'Refresh',
              onPressed: () => ref.invalidate(depositProtectionListProvider)),
        ],
      ),
...
class __DepositProtectionBodyState extends ConsumerState<_DepositProtectionBody> {
        ...
        data: (items) {
          ...
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Deposit Protections yet',
              ...
              itemBuilder: (ctx, i) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    leading: CircleAvatar(backgroundColor: _stColor(item), foregroundColor: Colors.white, child: Text(item.provider != null && item.provider!.toString().isNotEmpty ? item.provider!.toString()[0].toUpperCase() : '?'),),
  ...
  Color _stColor(DepositProtection item) {
...
void _showDetail(BuildContext context, DepositProtection item) {
  showDialog(context: context, builder: (ctx) => Dialog(
    child: ConstrainedBox(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Deposit Protection Details'),
...
class __DepositProtectionFormState extends ConsumerState<_DepositProtectionForm> {
  ...
  void _submit() {
    ...
    if (widget.item == null) {
      widget.ref.read(depositProtectionCreateStateProvider.notifier).state = DepositProtection.fromJson(data);
    } else {
      widget.ref.read(depositProtectionUpdateStateProvider.notifier).state = {
        'id': widget.item!.id,
        'depositProtection': DepositProtection.fromJson({...widget.item!.toJson(), ...data}),
      };
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      child: ConstrainedBox(
        ...
        child: Scaffold(
          appBar: AppBar(
            title: Text(isEdit ? 'Edit Deposit Protection' : 'New Deposit Protection'),
            automaticallyImplyLeading: false,
            ...
            child: ListView(
              ...
              children: [
                ...
                FilledButton.icon(
                  ...
                  label: Text(isEdit ? 'Save Changes' : 'Create Deposit Protection'),
...
void _confirmDel(BuildContext context, WidgetRef ref, DepositProtection item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    ...
    title: const Text('Delete Deposit Protection?'),
    content: const Text('This action cannot be undone.'),
    actions: [
      TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
      FilledButton.icon(
        onPressed: () {
          ref.read(depositProtectionDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
