...
// ================================================================
// Booking Admin Page  |  17 fields
// Auto-generated — edit with care
// ================================================================

class BookingAdminPage extends ConsumerWidget {
  const BookingAdminPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(bookingLoadingProvider);
    return Scaffold(
      appBar: AppBar(
...
class _BookingBody extends ConsumerStatefulWidget {
  const _BookingBody();
  @override ConsumerState<_BookingBody> createState() => __BookingBodyState();
}

class __BookingBodyState extends ConsumerState<_BookingBody> {
      ...
      Expanded(child: async.when(
        data: (items) {
          final list = _q.isEmpty
              ? items
              : items.where((item) => '${item.orgId ?? ''} ${item.listingId ?? ''} ${item.contactId ?? ''} ${item.reservationId ?? ''} ${item.currency ?? ''} ${item.notes ?? ''} ${item.createdBy ?? ''}'.toLowerCase().contains(_q)).toList();
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.inbox_outlined, size: 56, color: Colors.grey[350]),
              const SizedBox(height: 12),
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Bookings yet',
              ...
              itemBuilder: (ctx, i) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    leading: CircleAvatar(backgroundColor: _stColor(item), foregroundColor: Colors.white, child: Text(item.currency != null && item.currency!.isNotEmpty ? item.currency![0].toUpperCase() : '?'),),
        ...
        error: (e, _) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 12),
          ElevatedButton.icon(onPressed: () => ref.invalidate(bookingListProvider),
              icon: const Icon(Icons.refresh), label: const Text('Retry')),
        ])),
      )),
    ]);
  }
  Color _stColor(Booking item) {
...
void _showDetail(BuildContext context, Booking item) {
      ...
      child: Scaffold(
        ...
        body: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _row('Adults', item.adults?.toString() ?? 'N/A', Icons.numbers),
              _row('Children', item.children?.toString() ?? 'N/A', Icons.numbers),
              _row('Price Total', item.priceTotal?.toString() ?? 'N/A', Icons.attach_money),
...
void _showForm(BuildContext context, WidgetRef ref, {Booking? item}) {
  showDialog(context: context, builder: (ctx) => _BookingForm(item: item, ref: ref));
}

class _BookingForm extends ConsumerStatefulWidget {
  final Booking? item;
  final WidgetRef ref;
  const _BookingForm({this.item, required this.ref});
  @override ConsumerState<_BookingForm> createState() => __BookingFormState();
}

class __BookingFormState extends ConsumerState<_BookingForm> {
  ...
  void _submit() {
    ...
    if (widget.item == null) {
      widget.ref.read(bookingCreateStateProvider.notifier).state = Booking.fromJson(data);
    } else {
      widget.ref.read(bookingUpdateStateProvider.notifier).state = {
        'id': widget.item!.id,
        'booking': Booking.fromJson({...widget.item!.toJson(), ...data}),
      };
    }
            ...
            child: ListView(
              ...
              children: [
                  TextFormField(
                    ...
                    initialValue: widget.item?.listingId ?? '',
                    maxLines: 1,
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item?.contactId ?? '',
                    maxLines: 1,
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item?.reservationId ?? '',
                    maxLines: 1,
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item?.adults?.toString() ?? '',
                  ...
                  TextFormField(
                    ...
                    keyboardType: TextInputType.number,
                    initialValue: widget.item?.children?.toString() ?? '',
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item?.priceTotal?.toString() ?? '',
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item?.currency ?? '',
                    maxLines: 1,
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item?.paymentStatus ?? '',
                    maxLines: 1,
...
void _confirmDel(BuildContext context, WidgetRef ref, Booking item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    ...
    actions: [
      ...
      FilledButton.icon(
        onPressed: () {
          ref.read(bookingDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
