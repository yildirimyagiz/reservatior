...
import '../widgets/dashboard_widget_form_widget.dart';

// ── DashboardWidget Client Page

class DashboardWidgetClientPage extends ConsumerStatefulWidget {
  const DashboardWidgetClientPage({super.key});
  @override ConsumerState<DashboardWidgetClientPage> createState() => _DashboardWidgetClientPageState();
}

class _DashboardWidgetClientPageState extends ConsumerState<DashboardWidgetClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Dashboard Widgets'),
        backgroundColor: Colors.teal[700], foregroundColor: Colors.white,
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(dashboardWidgetListProvider)),
        ],
      ),
          ...
          data: (items) {
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Dashboard Widgets', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
      ...
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'DashboardWidgetClientFAB',
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              const Text('Dashboard Widget Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Dashboard Widget', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            constraints: BoxConstraints(maxHeight: MediaQuery.of(ctx).size.height * 0.7),
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16),
              child: DashboardWidgetFormWidget(
                onSubmit: (newItem) {
                  ref.read(dashboardWidgetCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
