...
import '../widgets/automation_task_form_widget.dart';

// ── AutomationTask Client Page

class AutomationTaskClientPage extends ConsumerStatefulWidget {
  const AutomationTaskClientPage({super.key});
  @override ConsumerState<AutomationTaskClientPage> createState() => _AutomationTaskClientPageState();
}

class _AutomationTaskClientPageState extends ConsumerState<AutomationTaskClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Automation Tasks'),
        backgroundColor: Colors.teal[700], foregroundColor: Colors.white,
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(automationTaskListProvider)),
        ],
      ),
          ...
          data: (items) {
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Automation Tasks', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
                ...
                itemBuilder: (ctx, i) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      leading: CircleAvatar(backgroundColor: _stColor(item.status), foregroundColor: Colors.white, child: Text(item.taskType != null && item.taskType!.toString().isNotEmpty ? item.taskType!.toString()[0].toUpperCase() : '?'),),
                      title: Text(item.taskType ?? 'Unknown', style: const TextStyle(fontWeight: FontWeight.w600)),
                      subtitle: Text('item.status?.toString( ?? "N/A"}'),
      ...
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'AutomationTaskClientFAB',
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              const Text('Automation Task Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
          ...
          Expanded(child: ListView(controller: sc, padding: const EdgeInsets.all(16), children: [
                  ...
                  _row('Last Run', _fmt(item.lastRun), Icons.calendar_today),
                  _row('Next Run', _fmt(item.nextRun), Icons.calendar_today),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Automation Task', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            constraints: BoxConstraints(maxHeight: MediaQuery.of(ctx).size.height * 0.7),
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16),
              child: AutomationTaskFormWidget(
                onSubmit: (newItem) {
                  ref.read(automationTaskCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
