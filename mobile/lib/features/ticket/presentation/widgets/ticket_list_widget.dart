...
class TicketListWidget extends StatelessWidget {
  ...
  final void Function(Ticket)? onEdit;
  final void Function(Ticket)? onDelete;
  const TicketListWidget({super.key, required this.items, this.onTap, this.onEdit, this.onDelete});
    ...
    return ListView.builder(
      ...
      itemBuilder: (ctx, i) {
        ...
        return Card(
          ...
          child: ListTile(
            leading: CircleAvatar(backgroundColor: _stColor(item.status), foregroundColor: Colors.white, child: Text(item.subject != null && item.subject!.toString().isNotEmpty ? item.subject!.toString()[0].toUpperCase() : '?'),),
