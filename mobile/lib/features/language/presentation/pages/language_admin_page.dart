...
// ================================================================
// Language Admin Page  |  11 fields
// Auto-generated — edit with care
// ================================================================

class LanguageAdminPage extends ConsumerWidget {
  const LanguageAdminPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final loading = ref.watch(languageLoadingProvider);
    return Scaffold(
      appBar: AppBar(
      ...
      body: const _LanguageBody(),
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'LanguageFAB',
        onPressed: () => _showForm(context, ref),
        icon: const Icon(Icons.add), label: const Text('New Language'),
      ),
    );
  }
}
...
class _LanguageBody extends ConsumerStatefulWidget {
  const _LanguageBody({super.key});
  @override ConsumerState<_LanguageBody> createState() => __LanguageBodyState();
}

class __LanguageBodyState extends ConsumerState<_LanguageBody> {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(languageListProvider);
      ...
      Expanded(child: async.when(
        data: (items) {
          final list = _q.isEmpty
              ? items
              : items.where((item) => ((item.code?.toString() ?? '') + " " + (item.name?.toString() ?? '') + " " + (item.nativeName?.toString() ?? '') + " " + (item.agencyId?.toString() ?? '') + " " + (item.agentId?.toString() ?? '') + " " + (item.userId?.toString() ?? '')).toLowerCase().contains(_q)).toList();
          if (list.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.inbox_outlined, size: 56, color: Colors.grey[350]),
              const SizedBox(height: 12),
              Text(_q.isNotEmpty ? 'No results for "$_q"' : 'No Languages yet',
        ...
        error: (e, _) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 12),
          ElevatedButton.icon(onPressed: () => ref.invalidate(languageListProvider),
...
void _showDetail(BuildContext context, Language item) {
      ...
      child: Scaffold(
        ...
        body: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _row('Native Name', item.nativeName?.toString() ?? 'N/A', Icons.person),
              _row('Is R T L', (item.isRTL == true ? 'Yes' : 'No'), Icons.toggle_on),
...
void _showForm(BuildContext context, WidgetRef ref, {Language? item}) {
  showDialog(context: context, builder: (ctx) => _LanguageForm(item: item, ref: ref));
}

class _LanguageForm extends ConsumerStatefulWidget {
  final Language? item;
  final WidgetRef ref;
  const _LanguageForm({super.key, this.item, required this.ref});
  @override ConsumerState<_LanguageForm> createState() => __LanguageFormState();
}

class __LanguageFormState extends ConsumerState<_LanguageForm> {
  ...
  void _submit() {
    ...
    if (widget.item == null) {
      widget.ref.read(languageCreateStateProvider.notifier).state = Language.fromJson(data);
    } else {
      widget.ref.read(languageUpdateStateProvider.notifier).state = {
        'id': widget.item!.id,
        'language': Language.fromJson({...widget.item!.toJson(), ...data}),
      };
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      child: ConstrainedBox(
        ...
        child: Scaffold(
          appBar: AppBar(
            title: Text(isEdit ? 'Edit Language' : 'New Language'),
            automaticallyImplyLeading: false,
            ...
            child: ListView(
              ...
              children: [
                  ...
                  TextFormField(
                    ...
                    initialValue: widget.item.nativeName?.toString() ?? '',
                    maxLines: 1,
                ...
                FilledButton.icon(
                  ...
                  label: Text(isEdit ? 'Save Changes' : 'Create Language'),
...
void _confirmDel(BuildContext context, WidgetRef ref, Language item) {
  showDialog(context: context, builder: (ctx) => AlertDialog(
    ...
    actions: [
      ...
      FilledButton.icon(
        onPressed: () {
          ref.read(languageDeleteStateProvider.notifier).state = item.id;
          Navigator.pop(ctx);
        },
