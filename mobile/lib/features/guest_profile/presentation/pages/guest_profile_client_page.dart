...
import '../widgets/guest_profile_form_widget.dart';

// ── GuestProfile Client Page

class GuestProfileClientPage extends ConsumerStatefulWidget {
  const GuestProfileClientPage({super.key});
  @override ConsumerState<GuestProfileClientPage> createState() => _GuestProfileClientPageState();
}

class _GuestProfileClientPageState extends ConsumerState<GuestProfileClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Guest Profiles'),
        backgroundColor: Colors.teal[700], foregroundColor: Colors.white,
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(guestProfileListProvider)),
        ],
      ),
          ...
          data: (items) {
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Guest Profiles', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Guest Profile', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            constraints: BoxConstraints(maxHeight: MediaQuery.of(ctx).size.height * 0.7),
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16),
              child: GuestProfileFormWidget(
                onSubmit: (newItem) {
                  ref.read(guestProfileCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
