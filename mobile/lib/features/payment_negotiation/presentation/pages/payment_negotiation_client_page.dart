...
import '../widgets/payment_negotiation_form_widget.dart';

// ── PaymentNegotiation Client Page

class PaymentNegotiationClientPage extends ConsumerStatefulWidget {
  const PaymentNegotiationClientPage({super.key});
  @override ConsumerState<PaymentNegotiationClientPage> createState() => _PaymentNegotiationClientPageState();
}

class _PaymentNegotiationClientPageState extends ConsumerState<PaymentNegotiationClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Payment Negotiations'),
        backgroundColor: Colors.teal[700], foregroundColor: Colors.white,
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(paymentNegotiationListProvider)),
        ],
      ),
          ...
          data: (items) {
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Payment Negotiations', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
                ...
                itemBuilder: (ctx, i) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      leading: CircleAvatar(backgroundColor: _stColor(item.status), foregroundColor: Colors.white, child: Text(item.validationNotes != null && item.validationNotes!.toString().isNotEmpty ? item.validationNotes!.toString()[0].toUpperCase() : '?'),),
      ...
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'PaymentNegotiationClientFAB',
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              const Text('Payment Negotiation Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
          ...
          Expanded(child: ListView(controller: sc, padding: const EdgeInsets.all(16), children: [
                  ...
                  _row('Agreed At', _fmt(item.agreedAt), Icons.calendar_today),
                  _row('Expires At', _fmt(item.expiresAt), Icons.calendar_today),
                  _row('Reminder Sent At', _fmt(item.reminderSentAt), Icons.calendar_today),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Payment Negotiation', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            constraints: BoxConstraints(maxHeight: MediaQuery.of(ctx).size.height * 0.7),
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16),
              child: PaymentNegotiationFormWidget(
                onSubmit: (newItem) {
                  ref.read(paymentNegotiationCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
