...
import '../widgets/ai_market_analysis_form_widget.dart';

// ── AIMarketAnalysis Client Page

class AIMarketAnalysisClientPage extends ConsumerStatefulWidget {
  const AIMarketAnalysisClientPage({super.key});
  @override ConsumerState<AIMarketAnalysisClientPage> createState() => _AIMarketAnalysisClientPageState();
}

class _AIMarketAnalysisClientPageState extends ConsumerState<AIMarketAnalysisClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Ai Market Analysiss'),
        backgroundColor: Colors.teal[700], foregroundColor: Colors.white,
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(aiMarketAnalysisListProvider)),
        ],
      ),
  ...
  return '$orgId $analysisType $location $analysisPeriod $status'.toLowerCase().contains(_q);
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Ai Market Analysiss', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
                ...
                itemBuilder: (ctx, i) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      leading: CircleAvatar(backgroundColor: _stColor(item.status), foregroundColor: Colors.white, child: Text(item.analysisType != null && item.analysisType!.toString().isNotEmpty ? item.analysisType!.toString()[0].toUpperCase() : '?'),),
      ...
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'AIMarketAnalysisClientFAB',
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              const Text('Ai Market Analysis Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Ai Market Analysis', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            ...
            child: SingleChildScrollView(
              ...
              child: AIMarketAnalysisFormWidget(
                onSubmit: (newItem) {
                  ref.read(aiMarketAnalysisCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
