...
// ── PropertyPromotion Detail Widget  |  15 fields

class PropertyPromotionDetailWidget extends StatelessWidget {
  final PropertyPromotion item;
  const PropertyPromotionDetailWidget({super.key, required this.item});
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      if (item.status != null)
        ...
        _row('Promotion Type', item.promotionType?.toString() ?? 'N/A', Icons.text_fields),
        ...
        _row('Is Auto Renew', (item.isAutoRenew == true ? 'Yes' : 'No'), Icons.toggle_on),
        _row('Payment History Id', item.paymentHistoryId?.toString() ?? 'N/A', Icons.link),
