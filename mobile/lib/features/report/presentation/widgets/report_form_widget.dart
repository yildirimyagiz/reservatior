...
// ── Report Form Widget  |  Fields: userId, name, description, reportType, config, schedule, recipients, lastRunAt, isActive
...
class ReportFormWidget extends StatefulWidget {
  ...
  @override State<ReportFormWidget> createState() => _ReportFormWidgetState();
}
...
class _ReportFormWidgetState extends State<ReportFormWidget> {
  ...
  void _submit() {
    ...
    final data = <String, dynamic>{
        ...
        if (_recipients?.isNotEmpty == true) 'recipients': _recipients,
              ...
              InkWell(
                onTap: () async {
                  final d = await showDatePicker(
                    context: context, initialDate: _lastRunAt ?? DateTime.now(),
                    firstDate: DateTime(2000), lastDate: DateTime(2100),
                  );
                  if (d != null) setState(() => _lastRunAt = d);
                },
                child: InputDecorator(
                  decoration: const InputDecoration(labelText: 'Last Run At',
                    prefixIcon: Icon(Icons.calendar_today), border: OutlineInputBorder()),
                  child: Text(_lastRunAt != null ? _fmt(_lastRunAt) : 'Tap to select date'),
                ),
              ),
              const SizedBox(height: 12),
              StatefulBuilder(
