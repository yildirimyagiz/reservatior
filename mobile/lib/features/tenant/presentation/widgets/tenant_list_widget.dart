...
class TenantListWidget extends StatelessWidget {
  ...
  final void Function(Tenant)? onTap;
  final void Function(Tenant)? onEdit;
  final void Function(Tenant)? onDelete;
  const TenantListWidget({super.key, required this.items, this.onTap, this.onEdit, this.onDelete});
    ...
    return ListView.builder(
      ...
      itemBuilder: (ctx, i) {
        ...
        return Card(
          ...
          child: ListTile(
            leading: CircleAvatar(backgroundColor: _stColor(item.paymentStatus), foregroundColor: Colors.white, child: Text(item.email != null && item.email!.toString().isNotEmpty ? item.email!.toString()[0].toUpperCase() : '?'),),
