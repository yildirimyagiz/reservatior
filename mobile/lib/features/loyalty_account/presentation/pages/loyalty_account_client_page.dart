...
import '../widgets/loyalty_account_form_widget.dart';

// ── LoyaltyAccount Client Page

class LoyaltyAccountClientPage extends ConsumerStatefulWidget {
  const LoyaltyAccountClientPage({super.key});
  @override ConsumerState<LoyaltyAccountClientPage> createState() => _LoyaltyAccountClientPageState();
}

class _LoyaltyAccountClientPageState extends ConsumerState<LoyaltyAccountClientPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Loyalty Accounts'),
        backgroundColor: Colors.teal[700], foregroundColor: Colors.white,
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: () => ref.invalidate(loyaltyAccountListProvider)),
        ],
      ),
          ...
          data: (items) {
            ...
            if (list.isEmpty) {
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No Loyalty Accounts', style: TextStyle(color: Colors.grey[500])),
              ]));
            }
      ...
      floatingActionButton: FloatingActionButton.extended(
        heroTag: 'LoyaltyAccountClientFAB',
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              const Text('Loyalty Account Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 16),
            child: Align(alignment: Alignment.centerLeft,
              child: Text('New Loyalty Account', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))),
          const Divider(),
          ConstrainedBox(
            constraints: BoxConstraints(maxHeight: MediaQuery.of(ctx).size.height * 0.7),
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16),
              child: LoyaltyAccountFormWidget(
                onSubmit: (newItem) {
                  ref.read(loyaltyAccountCreateStateProvider.notifier).state = newItem;
                  Navigator.pop(ctx);
                },
              ),
            ),
